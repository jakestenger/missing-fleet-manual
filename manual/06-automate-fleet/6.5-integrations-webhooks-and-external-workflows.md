...
![Explanation](../_assets/icons/explanation-light.svg) Fleet can tell something else that a thing happened. That is the whole feature, and almost every difficulty in this chapter comes from the fact that **the seven mechanisms which do it are not variations on one design.**
...
![Explanation](../_assets/icons/explanation-light.svg) Three ways to learn that something happened, and the first is not always the right one.
...
![Reference](../_assets/icons/reference-light.svg) Where each one is configured, and what scope it can be configured at.
...
**Three of them cannot be scoped to a fleet at all, and the calendar callback destination cannot be configured globally.** Note that the Google Calendar credentials themselves *are* global; it is only the callback destination that is per fleet.

The activity webhook is what changed at 4.91. On Fleet Premium it now takes a per-fleet form alongside the global one, and the two fire on different events. Its contract is under *The activity webhook, global or per fleet*.
...
![Reference](../_assets/icons/reference-light.svg) The comparison the chapter exists for.
...
The global setting POSTs every activity, on Free and Premium alike. The per-fleet webhook is Fleet Premium, **it fires only for activities linked to a host**, and it delivers to the fleet those hosts belong to. Configure it from the Hosts page, or in GitOps under a fleet's `webhook_settings.host_activities_webhook`; the "No fleet" host set takes the same setting. An activity that touches no host, a policy edited or a user signing in, reaches the global webhook.
...
![Troubleshooting](../_assets/icons/troubleshooting-light.svg) Fleet gives you less than you would like here, so the receiver has to carry it.
...
![How-to](../_assets/icons/howto-light.svg) What you can and cannot deduplicate on.
...
![Reference](../_assets/icons/reference-light.svg) The configuration keys that control each integration and log destination are in [a.3](../09-appendices/a.3-configuration-model-and-precedence.md). Whether a webhook or integration setting is reachable through the UI, fleetctl, the API, or GitOps is in [a.5](../09-appendices/a.5-interface-index.md).
