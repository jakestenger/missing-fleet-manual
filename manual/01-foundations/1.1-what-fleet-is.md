...
`preview` also rewrites your `fleetctl` configuration. If `~/.fleet/config` doesn't exist yet, it creates one and sets the sandbox as your `default` context. If a config file already exists, it adds a separate `preview` context instead and prints a line saying so, but that message is the only signal: the CLI's `--context` flag still defaults to `default` for later commands, so reaching the sandbox again needs `fleetctl --context preview ...`.
