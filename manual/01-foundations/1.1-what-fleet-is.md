...
![How-to](../_assets/icons/howto.svg) `fleetctl preview` (alias `sandbox`) starts a throwaway Fleet server under Docker on your own machine: no cloud account, no Terraform, nothing to tear down afterward beyond `fleetctl preview reset`. It's the fastest way to see the product before choosing a deployment path in Part II.
