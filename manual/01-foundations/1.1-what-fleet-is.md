...
|---|---|---|
| Report storage | Keeps the newest result per host per query, overwriting the previous one, so the report page always shows current answers and never a trend | The global `query_reports_disabled` setting, the query's own `discard_data`, a query that is not snapshot logging, or the report row cap, which defaults to 1000 rows per query |
...
**The fourth question exists because the third one does not answer it.** A change passes through distinct states: desired, queued, delivered, applied, and verified. Fleet's audit trail records that you asked, and it is written when the request is successfully queued, not when a device does anything. A command can sit in a queue for days waiting for a device to check in, or be delivered and refused by the device, with a clean audit entry the whole time. Part VIII is largely the discipline of telling those states apart, and this manual tries never to let "Fleet says it did it" stand in for "the device did it".
...
**SIEMs, data lakes, and security tooling** are good places to store and analyze the data Fleet generates. Fleet emits device data, report results, logs and audit records to a configured destination, and that destination is where longer retention, correlation and response belong. Fleet's own storage is not built to be queried as a history.
...
3. **Your edition does not include it.** Fleet Premium gates a great many features individually, and a licence gate presents as an absence rather than as an error about licensing, which is what makes it easy to mistake for one of the other three.
...
|---|---|---|---|
| macOS | fleetd and osquery | Orbit: scripts, installer-based software, fleetd updates, disk-encryption workflows | Apple MDM: profiles, MDM commands, App Store and VPP apps |
