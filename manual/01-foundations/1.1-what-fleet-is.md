...
Fleet helps you see the state of the devices you manage and change them in controlled, auditable ways. Many organizations use one tool to inventory devices and another to manage them through operating-system management channels, commonly called mobile device management (MDM). Fleet joins both jobs around one device record and one **control plane**, a single place that both sees a device's state and changes it.
...
Fleet reads device data through **osquery**, its query engine. It runs agent-side work through **fleetd**, the installed agent bundle. For profiles and device commands, it uses the device-management protocol built into each operating system.
...
**A host's vitals, inventory, policy results and stored report rows are the latest thing that host told Fleet, not live truth about the machine.** Configuration you set is different: a profile or a policy is stored because you wrote it, so it is current by definition. Fleet keeps a little real history too: for host uptime and for vulnerabilities, a change appends instead of overwriting. An offline laptop will happily show a browser version it stopped running weeks ago, which is why Fleet keeps freshness clocks beside the data: a host's last-seen time, the fetch time on a single report row. Read the clock that applies with the value, and check that one applies. A few values, such as a custom host vital an administrator sets through the API, carry no timestamp at all, so nothing on the page says when they were last true.

**Fleet keeps only the latest result of a scheduled report; it does not build a running history.** Three different behaviours decide what is kept, what is forwarded, and what is audited. Confuse them and an estate can end up retaining nothing at all:
...
Result forwarding, the middle row, is where data is most often lost. Configuring a log destination is not the same as sending reports to it: the destination is set once for the server, and each query decides for itself whether its results are forwarded. For the log route you need both settings, and [2.8](../02-administer-and-deploy-fleet/2.8-activity-audit-logs-and-log-delivery.md) covers the delivery machinery those settings drive.

Log delivery is not the only route to a history. The REST API returns the stored report rows, so you can export them yourself on a schedule; log delivery just saves you writing and running that exporter. `fleetctl` is not that route: `fleetctl report` runs a fresh live report and `fleetctl get reports` lists definitions, and neither returns the stored rows.
...
Knowing that four hundred devices are out of date only helps if you can say *which* four hundred, in terms Fleet can act on. That is what labels, policies, fleets, and vulnerability data are for: turning an observation into a defined set of devices.
...
One rule holds across all of them: **the audit entry records that Fleet accepted your instruction, not that a device did anything with it.** A command can sit queued for days waiting for a check-in, or be delivered and refused, with a clean audit entry the whole time ([8.1](../08-troubleshooting/8.1-diagnostic-method.md)). Part VIII is largely the discipline of telling those states apart, and this manual tries never to let "Fleet says it did it" stand in for "the device did it".
...
Those are the five stages: connect devices, know them, manage them, automate the work, operate the service.

**Verification is not a sixth stage.** The fourth question is answered by going back round: you manage, then you *know* again, and the difference between what you asked for and what the devices report is the answer. That is why the lifecycle loops from Manage back to Know rather than ending, and why Part IV (reading) and Part V (changing) are best read together.
...
<!-- IMAGE-TODO: assets/1.1-fleet-environment-boundaries.webp
     ...
     environment overview paragraphs to one paragraph explaining ownership, preserving the
     provisioning and log-delivery links and stream-specific conditions. Keep current prose and this
     ...
     CANDIDATE: ../../research/visual-reviews/part-0-1/assets/1.1-fleet-environment-boundaries.webp
...
![Fleet maintains its own administrator accounts, observes devices, and exports independent data streams alongside existing identity and configuration systems.](assets/1.1-fleet-environment-boundaries.webp)
-->
...
**This table is not an edition promise.** Fleet Premium's gates follow individual features, not one Free/Premium rule, and the shape is not intuitive. Fleets and software installation require Premium. Scripts exist on Free, but fleet-scoped execution does not. Configuration profiles gate separately for fleet scope and for label scope, and gate their *contents* independently again, while MDM commands carry gates specific to particular commands.

Using a Fleet variable in a configuration profile needs Premium on **Apple and Windows**, from one shared check, with Apple applying further per-variable and certificate validation on top of that gate rather than in place of it. **Android is a third profile platform and does not work that way**: it accepts supported Fleet variables on Free, and gates particular profile *content* instead, currently `systemUpdate`.

Each feature chapter states its own gate. **[a.2](../09-appendices/a.2-platform-capability-matrix.md) collects them in one place**, so use the matrix for the platform, licence, and prerequisite answer, then return to the owning chapter for the procedure.
...
![How-to](../_assets/icons/howto.svg) `fleetctl preview` (alias `sandbox`) starts a throwaway Fleet server under Docker on your own machine: no cloud account and no Terraform, and teardown is `fleetctl preview reset`, which removes the containers and Orbit's update files but leaves the config dir and named volumes to remove separately. It's the fastest way to see the product before choosing a deployment path in Part II.
...
**Drop `--no-hosts` only if you mean to enroll the machine you're running this on.** Without it, `preview` doesn't just start simulated Linux containers: it downloads and runs a real Orbit and osqueryd locally, pointed at the sandbox, and enrolls your actual workstation as a host. That's deliberate, since it's the fastest way to see a real host in the UI, but it's worth knowing before running it on a machine you didn't mean to enroll anywhere.

`preview` also rewrites your `fleetctl` configuration. If `~/.fleet/config` doesn't exist yet, it creates one and sets the sandbox as your `default` context. If a config file already exists, it adds a separate `preview` context instead and prints a line saying so, but that message is the only signal: the CLI's `--context` flag still defaults to `default` for later commands, so reaching the sandbox again needs `--context preview` on the command itself, e.g. `fleetctl get hosts --context preview` (`--context` is a per-command flag, not a global one, so it has to come after the subcommand).
