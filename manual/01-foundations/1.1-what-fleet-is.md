...
|---|---|---|
| Report storage | Keeps the newest result per host per query, replacing that host's previous rows, so the report page shows the latest answer per host and never a trend | The global `query_reports_disabled` setting, which also **deletes** the rows already stored; the query's own `discard_data`; a query that is not snapshot logging; and the **report row cap**, 1000 rows per query by default. The cap does not behave simply: a host whose single snapshot exceeds 1000 rows stores nothing at all, and a cleanup job trims across hosts keeping the most recent rows, so an older host's answer can disappear rather than persist. **Once a report is marked clipped, treat both its coverage and its freshness as unknown** and read the per-row timestamps rather than the page |
...
The trap is the middle row. Configuring a log destination is not the same as sending reports to it: the destination is set once for the server, and each query decides for itself whether its results are forwarded. Log delivery is not the only route to a history, incidentally, since you can also export the stored results periodically through the REST API or `fleetctl`; it is the route that scales without you writing the exporter. If you want the log route, you need both settings, and [2.5](../02-administer-and-deploy-fleet/2.5-activity-audit-logs-and-log-delivery.md) covers the delivery machinery those settings drive.
...
**When a capability is missing for a platform, work through four major checks, of which the operating system is only the first.** They are not the only possible causes, but they account for the ones a reader can act on:
...
3. **Your edition does not include it.** Fleet Premium gates a great many features individually. Fleet's validators generally return an explicit licence error rather than failing obscurely, so trying the operation and reading the error is often the cheapest of the four to rule out. What can hide it is the interface, which may simply not offer a control rather than offering one that refuses.
