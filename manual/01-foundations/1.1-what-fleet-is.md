...
`preview` also rewrites your `fleetctl` configuration. If `~/.fleet/config` doesn't exist yet, it creates one and sets the sandbox as your `default` context. If a config file already exists, it adds a separate `preview` context instead and prints a line saying so, but that message is the only signal: the CLI's `--context` flag still defaults to `default` for later commands, so reaching the sandbox again needs `fleetctl --context preview ...`. **If your existing `~/.fleet/config` is present but fails to parse, `preview` overwrites it and every context it held is gone.** Back the file up first if you already use `fleetctl` against a real deployment. [A.7](../09-appendices/a.7-fleetctl-command-reference.md#the-exit-zero-register) tracks this as Z30, alongside the full authorization chain `preview` runs inside the sandbox it creates.
