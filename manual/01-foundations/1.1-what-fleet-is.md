...
**When a capability is missing for a platform, work through these checks, of which the operating system is only the first.** They are the common causes and not the complete set; server configuration and your own authorization can refuse an action too, and neither is a property of the platform:
