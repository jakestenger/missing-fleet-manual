...
<!-- IMAGE-REDO: 1.3-fleet-scope-delegation.webp
     WHY: reviewed 2026-08-24. Legible, and the green "Manage this fleet" connector
     against the grey "No access" line is exactly right; keep both. **The structure is
     wrong, though, and it contradicts the chapter.** It draws Global scope as a parent with
     a tree connector down to the two fleets, which says everything global flows down into
     them. That is only true for policies, reports and labels. Configuration profiles and
     software do not inherit at all: a host in a fleet gets that fleet's and no others. Redraw
     so the global panel connects to the fleets only through the three things that do reach
     them, each connector labelled, and add a short line making plain that profiles and
     software do not travel that path. Getting this backwards teaches the wrong model.
