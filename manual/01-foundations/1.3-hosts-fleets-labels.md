...
![Explanation](../_assets/icons/explanation.svg) A host belongs to one named fleet or to the **Unassigned** scope. A host in one fleet receives the configurations and payloads assigned to that fleet and no other, while Unassigned is its own useful scope for devices that are being evaluated, enrolled, or awaiting assignment.
...
![Explanation](../_assets/icons/explanation.svg) fleet-scoped managed items begin with their fleet scope; global items begin with the organization-wide scope. Label targeting then narrows the recipients within that scope.
