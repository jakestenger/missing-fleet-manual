...
That asymmetry is the thing to hold on to, because getting it backwards is expensive in both directions. Copying global policies into each new fleet gives every host the same check twice. Assuming a profile will be inherited from global gives it to nobody.
...
Two behaviours are worth knowing here rather than in the reference, because neither is what the model so far would predict.
...
**Two devices sharing a hardware identifier merge rather than fail.** If a serial or UUID is duplicated, whether through cloning, imaging, or a replaced mainboard, both machines match the same host record and take turns claiming it. Fleet logs a warning and lets the newer enrollment overwrite the existing data, so the symptom is one host record whose details keep changing rather than an error anyone notices.
...
| Software | yes | no |
| Enrollment secrets, agent options, roles | yes | no |
...
**One consequence is irreversible and worth checking before you click.** If the destination fleet does not enforce disk encryption, Fleet deletes the host's escrowed recovery key on transfer. It is escrowed again once encryption is enforced, but in the interval **Fleet no longer holds a recovery key for that disk**, and whether anything else does depends on what else your organisation runs. This applies across platforms, not only macOS. Confirm the destination enforces encryption before moving encrypted hosts into it.

The other losses in the list above are recoverable in the sense that the data can be regenerated by the host reporting again; this one is not, because the key was the only copy Fleet had.
...
**Fleet protects against exactly that for profiles and software.** Where an exclusion depends on a label created more recently than the host's last label report, delivery is withheld rather than allowed. The failure mode is "nobody got it yet" rather than "the wrong hosts got it permanently", which is the right trade.
...
The consequence to plan around, for the two items that do guard: attaching a newly created dynamic label as an exclusion means the item reaches nobody until hosts report, which can be an hour, or indefinitely for a host that is offline. Create the label, let it populate, then attach it.

And the correction to the sentence above about a profile not un-installing itself: **it does**. When label membership changes so that a host falls out of a profile's scope, Fleet queues the installed profile for removal rather than leaving it in place. The reason to withhold delivery is that the round trip is slow and visible to the user, not that the state would be permanent.
...
**No item supports all four, and no two item types support the same set.** This is the single most-copied wrong assumption about Fleet targeting, and it produces API and GitOps payloads that Fleet rejects outright:
...
- A group that needs its own **administrators**, enrollment secrets or agent options wants **a fleet**, because those are fleet-scoped and nothing else can express them. Needing its own profiles or software is weaker evidence than it looks: both can be label-targeted inside an existing fleet, and doing so avoids a second configuration set.
...
That is a heuristic and it has a clear exception, so do not apply it mechanically. **Agent options are fleet-scoped and labels play no part in selecting them**, so an update-channel pilot needs a fleet however temporary it is ([3.7](../03-connect-devices/3.7-manage-fleetd-orbit-and-updates.md)). The question that actually settles it is not how long the group lasts but whether what you want to give it can be targeted by label at all.
...
[a.6](../09-appendices/a.6-glossary-and-release-compatibility.md) is where the full mapping of which surface uses which name will live; it is an outline at present, so for now the rules above are the record.
