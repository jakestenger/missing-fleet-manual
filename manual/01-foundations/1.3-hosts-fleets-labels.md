...
<!-- IMAGE-REDO: 1.3-fleet-scope-delegation.webp
     WHY: reviewed 2026-08-24. The inheritance error is fixed: connectors are labelled
     individually and "Profiles and software do not inherit" is stated. Three things to tidy.
     **The label connectors stop in mid-air** instead of reaching the fleet panels, which
     reads as labels not arriving, the opposite of what is meant. The "Profiles and software
     do not inherit" line sits directly beneath those connectors, so it looks like an
     annotation on labels rather than a general statement; move it clear or set it apart. And
     "No access" now runs fleet to fleet, when it should run from the Lab employees box to
     the Corporate workstations fleet, since it is a statement about people.
...
<!-- IMAGE-REDO: assets/1.3-fleets-scope-labels-target.webp
     WHY: reviewed 2026-08-24. The shared laptop is filled green and the overlap now
     reads, which was the main thing. Two problems. **The three panels are at full saturation
     and two of them are completely empty**, so the loudest colours in the picture carry no
     information. Drop all three to a 20 to 25 percent tint, and put two or three laptop
     outlines inside Kiosks and Unassigned so the colour has something to describe. There are
     also two captions saying nearly the same thing, top and bottom; keep only the bottom.
