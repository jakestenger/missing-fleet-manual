...
| Agent options, the **osquery** part | fleet options win where set | **yes**, a fleet with no osquery options of its own falls back to the global ones |
| Agent options, the **Orbit** part: `update_channels`, `command_line_flags`, extensions | read from the fleet's own document | **no**, apart from the script execution timeout. A fleet that does not name a channel does not inherit the global one ([3.7](../03-connect-devices/3.7-manage-fleetd-orbit-and-updates.md)) |
...
That is a heuristic and it has a clear exception, so do not apply it mechanically. **Update channels are chosen per fleet and labels play no part in choosing them**, so an update-channel pilot needs a fleet however temporary it is ([3.7](../03-connect-devices/3.7-manage-fleetd-orbit-and-updates.md)). That is narrower than "labels never touch agent options": agent options also carry Orbit extensions, and those *can* be filtered by label, with the label filter itself gated on Premium. The question that actually settles it is not how long the group lasts but whether the particular thing you want to give the group can be targeted by label.
