...
<!-- IMAGE-REDO: assets/1.3-fleets-scope-labels-target.webp
     WHY: reviewed 2026-08-24. The tint is right and Kiosks and Unassigned now have hosts in
     them, so both earlier faults are fixed. **A new one replaced them, and it breaks the
     concept.** The two dashed label outlines now sit side by side as separate groups, so no
     host is in both. The entire point of this picture, and the sentence under it, is that a
     host can be in any number of labels at once. Draw the two outlines genuinely
     overlapping, in the way a Venn diagram overlaps, so that exactly one laptop falls inside
     both. That shared laptop is the green one, and it is the only reason the diagram
     exists.
