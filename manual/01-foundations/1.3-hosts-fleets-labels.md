...
| Agent options | fleet options win where set | **yes**, a fleet with no options of its own falls back to the global ones |
...
**No two item types share the same set of modes or the same combination rule.** Policies are the only item with all four. This is the single most-copied wrong assumption about Fleet targeting, and it produces API and GitOps payloads that Fleet rejects outright:
