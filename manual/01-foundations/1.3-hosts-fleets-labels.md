...
<!-- IMAGE-REDO: 1.3-fleet-scope-delegation.webp
     WHY: measured off-palette: three saturated hue families. Not individually inspected.
...
<!-- IMAGE-REDO: assets/1.3-fleets-scope-labels-target.webp
     WHY: the three fleet panels are tinted blue, green and purple. Purple is not a Fleet
     colour and the brand allows one accent. The image also carries two captions saying
     nearly the same thing, and the host shared between the two dashed label outlines, which
     is the entire point of the picture, is not visibly shared.
     ...
     visibly share exactly one laptop, and that shared laptop should be tinted with the
     single Fleet Green accent so the overlap is unmistakable. Inside the bottom of
