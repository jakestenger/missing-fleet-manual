...
**Two devices sharing a hardware identifier usually merge rather than fail.** If a serial or UUID is duplicated, whether through cloning, imaging, or a replaced mainboard, both machines match the same host record and take turns claiming it. Fleet logs a warning and lets the newer enrollment overwrite the existing data, so the usual symptom is one host record whose details keep changing rather than an error anyone notices.
