...
**Fleet does keep an archive of superseded keys**, so an active key going is not always the last copy: when a host reports a key that differs from the one on file, the old one is written to an archive table. A host whose key has been rotated at least once therefore has something archived; a host whose first and only key was just deleted does not. Treat the archive as a possible mitigation to ask about rather than as a recovery path you can count on.

The other losses in the list above are recoverable in the sense that the host regenerates them by reporting again. This one is not, because the host does not re-report a key that Fleet is no longer asking for.
