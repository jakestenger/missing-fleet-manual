...
![Explanation](../_assets/icons/explanation-light.svg) A dynamic label’s optional **platform** limits which hosts receive its query. A macOS label is never evaluated on Windows. The platform is fixed at creation; create a new label to change it.
...
An Ubuntu host matches both `Ubuntu (Linux)` and the generic `Linux` platform option.

Before 4.91, Linux-wide dynamic labels needed an unrestricted platform and a distribution test in the query. Those queries still work; the generic `Linux` option lets new labels avoid querying other platforms.
