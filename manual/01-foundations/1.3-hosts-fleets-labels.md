...
![Explanation](../_assets/icons/explanation.svg) A host belongs to one named fleet or to **Unassigned**. That placement helps determine the configuration it receives. Different item types use scope differently, so check the behavior of the item you are assigning:
...
![Explanation](../_assets/icons/explanation.svg) Use global policies, reports, and labels for requirements shared across the organization. Use fleet-scoped items for a group’s configuration and operating needs.
...
![Reference](../_assets/icons/reference.svg) A transfer lets a device move to a new operating baseline while keeping its host identity. Before transferring it, review the results and memberships that belong to its current fleet so you can preserve any history you need.
...
![Explanation](../_assets/icons/explanation.svg) A dynamic label’s optional **platform** limits which hosts receive its query. A macOS label is never evaluated on Windows. The platform is fixed at creation; create a new label to change it.
...
![Reference](../_assets/icons/reference.svg) Fleet’s current vocabulary uses **fleets** and **reports** where older material used **teams** and **queries**. The interface, API, command line, and GitOps gained the new names through aliases, so you will still encounter the older identifiers when reading data or maintaining automation.
