...
verified_source: "product claims re-checked at fleet-v4.90.0 (7c428c6e46) during the 2026-08-27 Part I review; corrections recorded in the section ledger. The `linux` label platform option, added in 4.91, verified against fleet-v4.91.0 (35fc1c0244)"
further_reading:
...
Two exceptions apply to re-enrollment: it preserves the host's current fleet, and duplicate device identifiers can merge host records. Both matter before you treat the enroll secret as a move instruction.

**Re-enrolling a host does not move it between fleets.** An enroll secret belongs to a fleet, and that assignment applies when the host is genuinely new. Re-enrolling an existing host with a different fleet's secret leaves it exactly where it is. Moving it is a deliberate transfer, whatever the installer suggests.
...
**With TPM-backed host identity certificates the behaviour inverts.** Fleet does not support duplicate identifiers on that path and rejects the enrollment outright, saying that the certificate's host does not match the enrolled host. So the same underlying problem presents as a silent merge in one deployment and a loud enrollment failure in another, and which one you get depends on a setting made elsewhere.
...
**"Has no options of its own" means the whole document, not the setting.** Two different things sound the same here: a fleet with no agent options at all falls back to the global document entire, while a fleet that *has* a document and leaves a setting out of it does not inherit that setting from anywhere. The two cases are one line apart in Fleet and produce opposite results.

**fleets** are for devices that share an owner, an operating model, or a lifecycle. A fleet carries its own policies, software, profiles, reports, agent options, enrollment secrets and roles; it does not shield its hosts from globally scoped policies, reports and labels.
...
**"Global" does not mean the same thing for every kind of item.** Some item types merge the global set with the fleet's set. Others match one scope exactly and ignore everything else.
...
Someone who has only worked with policies expects a global profile to reach everything, and it does not. Someone who has only worked with profiles expects a global policy not to apply inside a fleet, and it does.
...
**Withdrawn is not the same as removed.** Profiles are reconciled, so a profile the host no longer qualifies for is queued for removal from the device. Software is not: deleting the fleet removes the *availability* of its packages, and anything already installed stays installed on every host. Deleting a fleet is therefore not a way to claw software back.
...
![Reference](../_assets/icons/reference.svg) Moving a host between fleets keeps some of its state and deletes the rest.
...
**One consequence needs checking before you click, because the host cannot repair it.** If the destination fleet does not enforce disk encryption, Fleet **deletes the host's active escrowed recovery key** on transfer. It is escrowed again once encryption is enforced, and in the interval Fleet has no active key to give you. This applies across platforms, not only macOS. Confirm the destination enforces encryption before moving encrypted hosts into it.

**Fleet keeps an archive, and it holds more than superseded keys.** When a host reports a non-empty key that differs from the active value, Fleet writes **the incoming key** into an archive table before it inserts or updates the active row. An absent active key counts as different, so **the first key a host ever reports is archived too**, not only later ones. The read path then falls back to the newest decryptable archived copy when the active row is missing, which is why an administrator asking for the key of a transferred host can still be given one.

The archive is therefore a real recovery path. It is still not a reason to skip the check below, because the archived copy depends on Fleet having ingested a key at all and on that copy still being decryptable with the server's current private key ([1.6](1.6-the-fleet-server.md)).

The other losses in the list above are recoverable in the sense that the host regenerates them by reporting again. This one is different: the host does not re-report a key that Fleet has stopped asking for, so recovery depends on the archived copy rather than on anything the device will do.

**Two permissions are checked, not one.** Transferring a host authorizes the action against the destination fleet *and* against the fleet it is leaving, so an administrator scoped to only one end of the move will be refused. Check that before delegating transfers: the refusal names the action rather than the missing scope.


<!-- IMAGE-TODO: assets/1.3-host-transfer-state.webp
     ...
     kept/lost/rebuilt explanations; retain the complete preservation list, irreversible loss
     warning, and archived-key retrieval qualifications as text. Keep current prose and this TODO
     ...
     CANDIDATE: ../../research/visual-reviews/part-0-1/assets/1.3-host-transfer-state.webp
...
![A fleet transfer preserves the host identity, deletes old fleet-specific results, and reconciles destination configuration separately.](assets/1.3-host-transfer-state.webp)
-->
...
**Two independent properties describe a label, and collapsing them into one list of four "kinds" gets it wrong**, because they live in different database columns and an automation that reads the wrong one will classify a label incorrectly.
...
#### A dynamic label can be restricted to a platform

![Explanation](../_assets/icons/explanation.svg) Beside its query, a dynamic label carries an optional **platform**. It is not a third kind of label: it narrows which hosts are ever asked the query, so a label restricted to macOS is not evaluated on a Windows host at all rather than being evaluated and coming back false. **It is fixed when you create the label and cannot be changed afterwards**, so a platform chosen too narrowly means building a new label rather than editing this one.

Only a dynamic label takes one. Setting a platform on a manual or host-vitals label is rejected, because neither asks the host anything.

| Platform | Matches |
|---|---|
| All platforms (unset) | Every host, whatever it runs |
| macOS | Hosts reporting `darwin` |
| Windows | Hosts reporting `windows` |
| **Linux** | Hosts on **any** Linux distribution. New in Fleet 4.91 |
| Ubuntu (Linux) | Ubuntu hosts specifically |
| CentOS (Linux) | Hosts reporting Red Hat whose operating-system string identifies CentOS |

**A Linux host matches its distribution and the generic option, not one or the other.** An Ubuntu machine is asked the queries of `Ubuntu (Linux)` labels and of `Linux` labels both, so the generic option adds coverage rather than replacing the specific ones.

**Before 4.91 the only Linux choices were those two distributions**, which meant a label meant to cover the whole Linux estate had to be left unrestricted and carry the distribution test inside its own query, on every host including the Macs and Windows machines that could never match. If you have labels built that way, the query still works; what changes is that you no longer have to write them that way.
...
**Profiles and software behave differently here, and the difference is why the guard matters more for one than the other.**
...
Its effect on targeting cuts both ways. An **include** keeps delivering to a host that should have fallen out, which shows up as over-delivery. An **exclude** keeps excluding a host that should have fallen back in, which shows up as nothing at all: an item quietly not reaching a host that now qualifies for it. The second is the one that goes undiagnosed, because there is no failure to investigate.
...
These are two steps in a fixed order. The scope establishes the pool of candidate hosts. The labels then filter that pool. *A label can never widen the pool beyond the scope it started from.*
...
![Reference](../_assets/icons/reference.svg) Fleet renamed **teams** to **fleets**, and **queries** to **reports**, in the interface, API, command line, and GitOps. The rename is an alias layer rather than a migration, so it added the new words without retiring the old ones: both vocabularies are live at once, and you will meet the older one constantly.
...
- A device that changes organizational ownership wants **a fleet transfer**. The long-term scope changed, which is exactly the case fleets exist for.
...
That is a heuristic and it has a clear exception, so do not apply it mechanically. **Update channels are chosen per fleet and labels play no part in choosing them**, so an update-channel pilot needs a fleet however temporary it is ([3.8](../03-connect-devices/3.8-manage-fleetd-orbit-and-updates.md)). That is narrower than "labels never touch agent options": agent options also carry Orbit extensions, and those *can* be filtered by label, with the label filter itself gated on Premium. The question that settles it is not how long the group lasts but whether the particular thing you want to give the group can be targeted by label.
...
<!-- IMAGE-REDO: assets/1.3-fleets-scope-labels-target.webp
     ...
     CANDIDATE: ../../research/visual-reviews/part-0-1/assets/1.3-fleets-scope-labels-target.webp
     Prepared 2026-09-08; awaiting joint review. Keep this marker pending until accepted.
-->
