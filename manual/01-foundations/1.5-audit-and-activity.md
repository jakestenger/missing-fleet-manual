...
And **"streamed" does not always mean "arrived"**. A destination that accepts a batch and silently drops an oversized record inside it, which Firehose does above its per-record limit, returns success; Fleet then marks those activities streamed. So a row can be flagged as delivered with no copy at the far end, and the only way to know is to reconcile counts at the destination rather than trusting Fleet's flag.
