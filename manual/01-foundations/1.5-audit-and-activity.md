...
**Streamed to a log destination**, which is how the record ends up alongside your identity, infrastructure, and security events, minus the host-only rows described below. This is a Fleet Premium capability, and it is the one that puts Fleet's events in durable storage next to everything else you correlate against.
