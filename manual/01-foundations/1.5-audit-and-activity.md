...
verified_source: "product claims re-checked at fleet-v4.90.0 (7c428c6e46) during the 2026-08-27 Part I review; the per-fleet activity webhook added in 4.91 re-checked at fleet-v4.91.0 (35fc1c0244), full contract in 6.5; the `user_mfa_requested` activity added in 4.91 re-checked at the same tag; corrections recorded in the section ledger"
further_reading:
...
Fleet gives administrators a record of the change requested in Fleet and the result reported by the device. Read together, those records give the whole of an endpoint action: what was requested, who requested it, how it progressed, and what the host reported.

## Activity records and host results

![Explanation](../_assets/icons/explanation.svg) **The activity record is an enumerated event stream.** A two-record model, administrative requests on one side and device outcomes on the other, is easier to reach for and it is wrong: it tells you an activity row proves an administrator asked for something, and plenty of them do not.

`activity_past` contains a defined list of things Fleet considers worth recording, drawn from four different places:
...
**These are not four exclusive bins.** A `ran_script` row is an observed outcome *and* may be automation, since it is written when the host's result comes back and separately reports whether a policy or the setup experience started it. An `installed_software` row can carry the user who asked for it, or no user at all when it came from self-service or an automation.
...
<!-- IMAGE-TODO: assets/1.5-activity-actor-retention.webp
     ...
     actor-attribution caveats around the activity examples. Retain the four-source table,
     overlapping event categories, and distinction between activities and detailed host results.
     ...
     qualifications.
     CANDIDATE: ../../research/visual-reviews/part-0-1/assets/1.5-activity-actor-retention.webp
...
![Device events may have no actor; deleting an account severs an activity's user link while retaining its stored name and email.](assets/1.5-activity-actor-retention.webp)
-->

## What the record covers, and where it stops

![Explanation](../_assets/icons/explanation.svg) One boundary decides which questions this record can answer.

**Every entry is written by Fleet's server, but they do not all record something the server chose to do.** Some record a server action; others are written when a device's report arrives, which is why an unenrollment or a returned script result has a row of its own. Creating a policy, assigning a profile, transferring a host, changing someone's role and signing in are all written as they happen. A per-host **script result is written when the host's result comes back**, not when the script was sent, which is why a script queued against a single offline host produces no `ran_script` row until that host returns. Batch runs are different: requesting one writes its batch activity straight away, and scheduling one likewise, whatever state the target hosts are in.

**Fleet does not record anything it never saw.** An action taken on the machine, outside every channel Fleet holds, leaves no entry: someone uninstalling the agent locally, deleting a profile by hand, turning off encryption at the keyboard, or another management tool removing a package.

The distinction is between *device-originated* and *invisible*. A device that unenrolls from MDM tells Fleet so, and that becomes an activity. A device whose agent is deleted tells Fleet nothing, and it does not. So "how did the agent get removed from this laptop?" has no answer here unless Fleet removed it, and the answer is on the device, in [8.4](../08-troubleshooting/8.4-host-side-investigation.md). "When did this Mac leave MDM?" may well have one.
...
[1.4](1.4-identity-and-roles.md) notes that reading a host is enough to reveal its disk encryption recovery key, at whatever scope that read reaches. This is the other half of it.
...
The read-only role can see the secret. It cannot see it quietly. Whether that trade suits your organisation is yours to decide; Fleet gives you the visibility to decide it on.

### Sign-ins are recorded too, including the failures

Successful sign-ins appear as `user_logged_in`, and **password-stage** failures as `user_failed_login`. A run of failures against one account belongs in the same place as the rest of your security events, which is what delivery, below, is for.

**Second-factor events are recorded thinly.** Settle what is there before anyone builds an alert on it. Fleet can perform email-based MFA itself rather than delegating it: after the password validates, Fleet sends the code and returns without a login activity. The `user_logged_in` row appears only when the code is later accepted. **Fleet 4.91 added one signal at the front of that gap**, a `user_mfa_requested` activity written when the password validates and the mail goes out, carrying the address and the requesting IP. It records that a valid password was used; it does not record what happened next. **A wrong or expired code still writes nothing, so `user_failed_login` does not cover second-factor failures**, and because Fleet ran the challenge rather than an identity provider, the IdP has no record either. Someone can still fail the second factor indefinitely without leaving a failure anywhere, though each attempt is now preceded by a visible request ([8.12](../08-troubleshooting/8.12-audit-logs.md#8124-activity-types-the-categories)).
...
When an action is intentionally gradual, the activity record identifies the rollout, and **it identifies its scope only partially**: a profile activity carries the profile's identity and its fleet, and a batch script activity carries a host count and a fleet, but neither carries the labels that were targeted or the hosts that were resolved. A batch profile edit may not even carry the profile name. So the activity tells you a rollout happened and roughly how wide it was; for who was actually targeted you have to read the item's own targeting ([1.3](1.3-hosts-fleets-labels.md)).
...
**Pushed per event, through an activity webhook**, on Free and Premium alike. Fleet can POST each activity to a URL as it happens. The organization-wide setting sends every activity; Fleet Premium adds a per-fleet activity webhook (new in 4.91) that fires only for activities linked to that fleet's hosts, set from the Hosts page, so one fleet's events can be routed to their own channel. [6.5](../06-automate-fleet/6.5-integrations-webhooks-and-external-workflows.md#the-activity-webhook-global-or-per-fleet) is the receiver contract for both.
...
![Reference](../_assets/icons/reference.svg) Activity retention is configurable. The expiry settings are off by default, so an untouched deployment keeps everything and the database grows to match. That is defensible, but it should be deliberate.

**Host-linked activities are kept while any host link remains, with one exception.** On an Apple **Automated Device Enrollment (ADE)** re-enrollment, Fleet consults `preserve_host_activities_on_reenrollment`. When it is off, the reset **unlinks that host's activities without deleting the host**. An activity can link to several hosts, and expiry accepts a row only once **no** host link remains, so what becomes eligible is the subset that had no other host attached. The default is decided once, by the migration that introduced the setting, on a single test: **on if any user account already existed when that migration ran, off if none did**. That means on for a deployment that upgraded into it and off for a fresh install, so two Fleet servers of the same version can behave differently depending on their history. If you rely on a redeployed Mac keeping its investigation history, check that setting rather than assuming it.

**If you need records for longer than you want them sitting in Fleet's database, treat retention and delivery as one decision rather than two**, so that expiry does not remove something before it has been delivered. Coordination will not buy you a way out of duplication: a host-linked row stays in Fleet for as long as the link exists, whether or not it has also been streamed. The settings are in [2.7](../02-administer-and-deploy-fleet/2.7-organization-and-server-settings.md).

Three things constrain the obvious "stream it out, then trim locally" plan. Settle all three before you enable expiry. **Streaming does not carry host-only rows**, so trimming on the assumption that everything left Fleet discards those permanently. **Expiry cannot trim host-linked activities at all**: its query selects only activities with no host link remaining, so the rows attached to your hosts stay regardless of the window you set. What expiry actually prunes is the unlinked remainder.

**"Streamed" does not always mean "arrived"**. A destination can accept a batch while an individual record inside it never lands, and Fleet marks the whole batch streamed on the destination's acknowledgement. So a row can be flagged as delivered with no copy at the far end, and the only way to know is to reconcile counts at the destination rather than trusting Fleet's flag.
