...
![Explanation](../_assets/icons/explanation-light.svg) The activity stream records specific event types. An entry may describe an administrator’s request or a result reported by a device.
...
![Explanation](../_assets/icons/explanation-light.svg) Fleet writes activities for the event types it implements, using server actions and incoming device reports.
...
![Explanation](../_assets/icons/explanation-light.svg) The record itself is available on every Fleet deployment; how you reach it differs.
...
![Reference](../_assets/icons/reference-light.svg) Choose an activity-retention policy that supports your operational and audit needs. Expiry settings are off by default, so Fleet retains the records and the database grows as activity accumulates. You can keep that history or configure expiry once you have decided which records to retain elsewhere.
...
![How-to](../_assets/icons/howto-light.svg) Build regular review around a simple sequence:
