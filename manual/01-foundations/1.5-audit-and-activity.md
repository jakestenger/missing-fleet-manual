...
| Kind | Example | Actor |
|---|---|---|
| Administrative changes | Assigning a profile, editing a policy, changing a role | A person, or an API-only service identity, which the API distinguishes |
...
**Fleet records what Fleet did, and when it knew it.** Every entry is written by the Fleet server, but not always at the moment it acts. Creating a policy, assigning a profile, transferring a host, changing someone's role and signing in are all written as they happen. A **script run is written when the host's result comes back**, not when the script was sent, which is why a script queued against an offline host produces no row until that host returns.
...
**Streamed to a log destination**, which is how the record ends up alongside your identity, infrastructure, and security events, minus the host-only rows described below. This is a Fleet Premium capability, and it is the one that answers retention and correlation properly.
...
**Host-linked activities are kept while any host link remains, and there is one exception that surprises people.** On an Apple ADE re-enrollment, Fleet consults `preserve_host_activities_on_reenrollment`. When it is off, the reset **unlinks that host's activities without deleting the host**. An activity can link to several hosts, and expiry accepts a row only once **no** host link remains, so what becomes eligible is the subset that had no other host attached. The setting defaults **off on a fresh installation** and on for a deployment upgraded into it, so two Fleet servers of the same version can behave differently here depending on their history. If you rely on a redeployed Mac keeping its investigation history, check that setting rather than assuming it.

**If you need records for longer than you want them sitting in Fleet's database, treat retention and delivery as one decision rather than two**, so you do not end up exporting the evidence and keeping it as well. The settings are in [2.4](../02-administer-and-deploy-fleet/2.4-organization-and-server-settings.md).

Two things constrain the obvious "stream it out, then trim locally" plan, and both matter before you enable expiry. **Streaming does not carry host-only rows**, so trimming on the assumption that everything left Fleet discards those permanently. And **expiry cannot trim host-linked activities at all**: its query selects only activities with no host link remaining, so the rows attached to your hosts stay regardless of the window you set. What expiry actually prunes is the unlinked remainder.
