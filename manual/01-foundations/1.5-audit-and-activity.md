...
| Kind | Example | Actor |
|---|---|---|
| Administrative changes | Assigning a profile, editing a policy, changing a role | The person who made it |
| Requested work | Running a script, sending an MDM command | The person who requested it |
| Automation | A policy automation running a script | Fleet, flagged as Fleet-initiated |
| **Observed outcomes** | A device checking out of MDM, an enrollment completing, software reporting itself installed through the agent | **Often nobody** |

That last row is the one the tidy model hides. An Apple device sending `CheckOut` creates an `mdm_unenrolled` activity with no user at all; enrollment creates `mdm_enrolled` the same way; software results coming back through Orbit create `installed_software`. **An activity with no actor is a legitimate record of something that happened, not a corrupted one**, and reading it as a missing administrator will send you looking for a person who was never involved.
...
**In Fleet**, through the interface and the Activities API. This is the immediate view, bounded by whatever retention you have configured, and an account alone is not enough to see it: **the global feed is readable only by global roles**. A fleet-scoped operator is refused there while still being able to read an individual host's feed, which authorizes against access to that host and its fleet instead. If someone reports the activity page as empty or forbidden, check their scope before checking retention.

**Pushed per event, through an activity webhook.** Fleet can POST each activity to a URL as it happens. There is exactly one such setting, in the organization's own configuration; **there is no per-fleet activity webhook**, so a request to notify one fleet's channel separately has no answer at this release.

Treat it as a notification rather than as a copy of the record. Fleet fires the POST asynchronously *before* the row is stored, the payload omits the stored activity's ID and its Fleet-initiated flag, and delivery is best-effort: only a `429` is retried, for at most thirty minutes, and every other error is permanent and silent. It is the right tool for putting a change in a chat channel or opening a ticket, and the wrong one for anything that has to be complete.

**Streamed to a log destination**, which is how most of the record ends up alongside your identity, infrastructure, and security events. This is a Fleet Premium capability, and it is the one that answers retention and correlation properly.

**"Most" is doing real work in that sentence.** Streaming reuses the query behind the global activity feed, and that query excludes host-only rows unconditionally. Two kinds are host-only at this release: an `installed_certificate` activity always, and a `ran_script` activity when it belongs to a batch execution, which is to say the per-host rows for a batch script run. Those appear on the host's own feed in Fleet and never reach your destination. **Do not trim Fleet's local history on the assumption that the stream holds everything**, and do not claim coverage in a compliance context without checking whether the activity types you rely on are among these.
...
**Host-linked activities are usually kept until the host is deleted, and there is one exception that surprises people.** On an Apple ADE re-enrollment, Fleet consults `preserve_host_activities_on_reenrollment`. When it is off, the reset **unlinks the host's activities without deleting the host**, and those now-orphaned rows become eligible for expiry cleanup. The setting defaults **off on a fresh installation** and on for a deployment upgraded into it, so two Fleet servers of the same version can behave differently here depending on their history. If you rely on a redeployed Mac keeping its investigation history, check that setting rather than assuming it.
...
2. Confirm the intended fleet and label scope **from the item itself rather than from the activity**, since the activity payload does not carry label targeting or the resolved host set.
