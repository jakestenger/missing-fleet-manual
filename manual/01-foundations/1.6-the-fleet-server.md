...
<!-- IMAGE-REDO: assets/1.6-fleet-server-architecture.webp
     WHY: measured off-palette, and this prompt has now been deleted from the file three
     times while the image stayed. It renders as nothing, so it disappears in an edit pass.
     Please leave it in place; it is the only record of what the picture is meant to show.
     ...
     coordination", "Object storage, packages and files". Put the single Fleet Green accent
     on the MySQL box, because it is the only one whose loss is unrecoverable.
...
<!-- IMAGE-REDO: assets/1.6-fleet-service-state-model.webp
     WHY: measured off-palette. Deleted from this file three times along with the one above;
     ...
     Put the single Fleet Green accent on the MySQL card only.
