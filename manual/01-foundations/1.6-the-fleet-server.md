...
**Some of it is not, and the exceptions are worth knowing before you plan Redis maintenance.** Fleet holds the pending recipients for failing-policy automations in Redis sets, and the worker that sends those notifications reads them from there; lose the set before delivery and the owed notification is not reconstructed from anywhere. Fleet's recorded server errors live in Redis too, so an outage erases the evidence of itself. And where asynchronous host processing is enabled, Redis carries pending last-seen, label, policy and scheduled-query-stat updates on their way to MySQL, with write failures logged while the check-in carries on regardless.

None of that makes Redis a system of record. It does mean "losing Redis costs a retry" is too comfortable: it can also cost notifications you were owed and diagnostics you wanted.
...
**The breadth matters when you are diagnosing staleness.** A stale pack, a query definition that will not update, an MDM asset Fleet insists is the old one, an app name that will not change: all of these have the same shape as the agent-options case and none of them is fixed by looking at agent options. Suspect the cache before you suspect the write.
...
The database-backed two keep working across any number of instances, because every instance reaches the same database. What they cost is database growth, and carve blocks in particular grow it quickly.

The filesystem-backed two do not survive a second instance at all. Local disk is not shared, so an installer uploaded through one instance is invisible to the others and to whichever one the load balancer hands the download to. Fleet logs that it is using a local store and that this is not suitable for production, once for installers and once for icons. That log line is not boilerplate.
...
Failover is a genuine interruption. Promoting a replica requires Fleet instances to reconnect, and live reports in flight do not survive it. Enrolled hosts generally continue, because osquery buffers the logs it generates and retries, which is the same guarantee Fleet documents for an upgrade window; it is a claim about host logging rather than about every device workflow, and the ADE caveat in the upgrade section applies here too.
...
- **Pending failing-policy notifications are lost**, because the recipients waiting to be sent are held in Redis sets rather than reconstructed from MySQL. This is the one item on this list that is not a retry.
...
**Redis is not backed up**, and no supported procedure asks you to. Be aware of what that means rather than treating it as costless: a Redis loss takes pending failing-policy notifications and the recorded error history with it, neither of which is reconstructed. Plan Redis maintenance as a small deliberate data loss rather than as a restart.
