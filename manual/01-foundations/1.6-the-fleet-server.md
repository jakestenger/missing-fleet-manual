...
**Some of it is not reconstructible, and the distinction that matters is between Redis being unavailable and Redis losing what it held.** Three things sit in Redis that nothing else holds a copy of:
...
| Recorded server errors | Not recordable and not readable during the outage | The record of what went wrong before it is gone too |
...
None of that makes Redis a system of record. It does mean "losing Redis costs a retry" is too comfortable for the second column: it can also cost notifications you were owed and diagnostics you wanted.
...
<!-- IMAGE-REDO: assets/1.6-fleet-service-state-model.webp
     ...
     NOTE 2026-08-27: the rendered image still summarises Redis as "loss costs a retry" and
     object storage as "loss costs a re-upload". The chapter now retracts both: Redis holds
     pending failing-policy notifications and the recorded error history that nothing
     reconstructs, and object storage can hold file-carve blocks that nobody can re-upload.
     When regenerating, change those two summaries to "loss costs a retry, except pending
     notifications and the error record" and "loss costs a re-upload, except carve evidence".
...
- **Error recording stops and the existing record goes with it**, because Fleet's recorded server errors are themselves Redis data, with their own TTL, and reading them can flush them. The outage is invisible in the place you would look for it afterwards, and so is whatever was recorded before it.
