...
**Redis holds work in progress**, and most of it is disposable: live report campaigns, the host authentication cache, rate limits, in-flight SSO state, and the cursors reconcilers use to track where they had reached. A live report running when Redis restarts is gone. Run it again.

**Some of it is not reconstructible, and Redis being unavailable is different from Redis losing what it held.** Plenty sits in Redis that nothing else holds a copy of, live-report campaigns and in-flight SSO state among it. The loss of these three is quiet enough to matter afterwards:
...
A stale pack, a query definition that will not update, an app name that will not change: these have the same shape as the agent-options case and none of them is fixed by looking at agent options. Suspect the cache before you suspect the write.

**MDM assets are the exception.** They are cached too, but the cache key includes the asset's checksum and Fleet fetches the current hashes each time, so an updated asset does not read from the old entry. A stale MDM asset is not a cache-expiry problem, and looking for one there is a dead end.

That is what an "I changed it and only some hosts got the new value" report often turns out to be. A configuration change that applies inconsistently and then settles within the cache's expiry is this, and nothing is broken.

### What happens if you configure no object storage

**Your licence turns some of these features off before storage is even a question.** Software installers, software title icons and Apple bootstrap packages are all Fleet Premium: the base service returns a licence error on upload, and their stores are only constructed when the licence is Premium. File carves and organisation logos are not gated that way, and carve storage is initialised outside that block entirely. So on Free, three of the five rows below describe artifacts you cannot create.
...
The stored errors carry a TTL of their own, and the destructive read is the right thing to run once you have captured what you need. During an incident, a colleague who believes every read is destructive will hesitate to look at all.

## Deployment constraints this design imposes

![Reference](../_assets/icons/reference.svg) Everything above is what the split buys. This is what it costs. Two of these constraints shape a deployment plan directly.
...
Failover interrupts service: Fleet instances have to reconnect, and work in flight against the old primary does not survive it. How much else it costs is a property of your database topology and your failover mechanism rather than of Fleet, and this manual does not establish it. Treat a failover as an outage of unknown length and plan the rest from your own testing. What the upgrade section says about buffering and about ADE enrollments applies to the documented upgrade procedure specifically, and should not be carried over here as though it described every database interruption.
...
**The replica configuration inherits nothing from the primary.** Address, credentials, TLS, and pool settings are all separate, and a half-filled replica section written by someone who assumed the rest carried over gives you one of two quite different outcomes.
...
**It is not silent, though.** Fleet returns errors from failed publishes and from publishing to a channel nobody is subscribed to, logs receive errors and passes them down the output channel, and deliberately emits a specific interruption error because the interface relies on it to say the results are incomplete. So the evidence exists in the logs and in what the UI reports.

The *shape* is misleading: a live report returning fewer hosts than you targeted reads as a host coverage problem rather than a Redis one. Read the campaign's errors before concluding the hosts were offline. The fix is to lift the limit for pub/sub clients.

It is harder to find than it should be for two reasons. A managed Redis service may split the setting across several differently named parameters, so searching for the Redis name, finding nothing, and concluding it does not apply is an easy and wrong path. The limit is also about result volume, not host count. A fleet-wide report against a wide table is a completely different sizing exercise from a policy check against the same hosts.
...
Fleet instances are interchangeable, and hold no state another instance needs. One piece of state does not travel by itself: the object-storage fallback, whose sharing and persistence depend on the filesystem you give it. Configuring a bucket removes that question rather than answering it.
...
**Desired state flows outward.** It starts with an intention: an administrator in the UI, a GitOps change, an API request, or a scheduled workflow. It ends with a policy, script, software item, profile, or command sitting ready for the hosts it was scoped to. Desired state ends at *available to the device*, not at *applied on the device*. The gap between those two is where a great deal of troubleshooting lives.
...
**They do meet.** Fleet writes what you want into MySQL and reads what is true out of device reports, and for the most part those are two separate flows. When policy results arrive, though, Fleet drives work off that observation directly: running a script, reporting compliance to an identity provider, installing software or a VPP app, and registering the notification that later becomes a webhook or a ticket. Most of those act on policies that have just *started* failing, though a policy set to act continuously acts on every failing result, and one identity-provider path evaluates at sign-in time instead. That is the loop closing, observed state producing desired state, and it is why a script can run on a host that nobody asked to run a script on ([5.9](../05-manage-devices/5.9-automate-remediation-with-policies.md)).
