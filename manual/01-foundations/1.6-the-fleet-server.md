...
|---|---|---|
| Pending recipients for failing-policy automations | The worker cannot read them; they are still there when it returns | The owed notifications are not reconstructed from anywhere |
| Recorded server errors | Not recordable and not readable while Redis is away. What was already stored is still there when it returns | The record of what went wrong before the incident is gone too |
...
None of that makes Redis a system of record, and an **outage** really does cost you little more than a retry. What it means is that the **right-hand** column exists at all: if the data itself goes rather than merely becoming unreachable, you lose notifications you were owed and diagnostics you wanted, and nothing reconstructs either.
...
The filesystem-backed two are **not shared** between instances. Local disk is local, so an installer uploaded through one instance is invisible to the others and to whichever one the load balancer hands the download to. Whether they also survive a restart is a question about your deployment rather than about Fleet, since the default is a temporary directory and you can point installers and icons at explicit directories instead. Fleet logs that it is using a local store and that this is not suitable for production, once for installers and once for icons. That log line is not boilerplate.
...
<!-- IMAGE-REDO: assets/1.6-fleet-service-state-model.webp
     ...
     second says "Local filesystem, not shared: Software installers; Software title
...
**They do meet, and the clearest case is worth knowing.** Fleet writes what you want into MySQL and reads what is true out of device reports, and for the most part those are two separate flows whose distance is where most of the interesting questions in this book live. But when policy results arrive, Fleet identifies the policies that have just started failing and drives work off that observation directly: running a script, applying conditional access, installing software or a VPP app, and registering the webhook notification. That is the loop closing, observed state producing desired state, and it is why a script can run on a host that nobody asked to run a script on.
