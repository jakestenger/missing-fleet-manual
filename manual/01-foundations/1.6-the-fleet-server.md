...
None of that makes Redis a system of record. It does mean "losing Redis costs a retry" is too comfortable for the **right-hand** column: an outage costs you a retry, and losing the data costs you notifications you were owed and diagnostics you wanted.

**Object storage holds the large files.** Software installers, bootstrap packages, title icons, organisation logos, and file-carve blocks. Big, immutable, read by many instances at once, and no business occupying a database.
...
- **Error recording stops**, because Fleet's recorded server errors are themselves Redis data, with their own TTL, and reading them can flush them. Anything already stored survives the outage; what you lose is the record of the outage itself, which is the period you will most want to read about. The outage is invisible in the place you would look for it afterwards, and so is whatever was recorded before it.
