...
Almost everything durable lives in MySQL, Redis, or object storage. The exception is the **server private key**, which lives in the server's own configuration and is what decrypts the sensitive material held in MySQL, so it is neither in the three stores nor recoverable from them ([7.7](../07-operate-fleet/7.7-production-readiness-checklist-and-handoff.md)). That makes a Fleet server process close to disposable: stop one and start three more and the deployment carries on, because nothing another instance depends on was only in that process. Replacing all of them at once is a different matter, since an upgrade requires every instance down for the migration, and that section says what the window costs.

Two things do live inside a process, and both are covered below: a cache in front of MySQL, and, if you have configured no object storage, files on local disk. The first mostly costs you staleness. The second is why a single-instance deployment behaves differently from a scaled one.
...
<!-- IMAGE-TODO: assets/1.6-datastore-outage-paths.webp
     ...
     NOTE: Proposed 2026-09-08; editorial brief, not technical re-verification. Shorten the opening
     symptom comparison and cache-fallback explanation. Keep the detailed outage list, data-loss
     versus temporary-outage distinction, notification risk, and async-processing conditions. Keep
     ...
     CANDIDATE: ../../research/visual-reviews/part-0-1/assets/1.6-datastore-outage-paths.webp
...
![MySQL failure stops authenticated work; Redis failure breaks some functions and shifts cache traffic to MySQL, with additional async-processing effects.](assets/1.6-datastore-outage-paths.webp)
-->
