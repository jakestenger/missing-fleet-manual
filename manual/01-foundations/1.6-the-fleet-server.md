...
Everything durable lives in MySQL, Redis, or object storage. That makes a Fleet server process close to disposable. Stop one, start three more, replace all of them during an upgrade, and the deployment carries on, because nothing another instance depends on was only in that process. Two things do live inside a process, and both are covered below: a short-lived cache in front of MySQL, and, if you have configured no object storage, files on local disk. The first is harmless. The second is why a single-instance deployment behaves differently from a scaled one.
...
Configuring no bucket does not turn the features off. Four kinds of artifact fall back, and they split along a line that decides what happens when you add a second instance.
...
A destructive read exists because it bounds memory, and it is the right thing to run once you have captured what you need. Knowing which of the two you are performing matters more during an incident than either behaviour on its own: a colleague who believes every read is destructive will hesitate to look at all.

## Deployment constraints this design imposes

![Reference](../_assets/icons/reference.svg) Everything above is what the split buys. This is what it costs. These are constraints, not preferences, and two of them shape a deployment plan before anything else does.
