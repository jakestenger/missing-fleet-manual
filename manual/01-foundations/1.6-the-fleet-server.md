...
<!-- IMAGE-REDO: assets/1.6-fleet-server-architecture.webp
     ...
     coordination", "Object storage, packages and files". Fill the MySQL box solid #009A7D with white
     text, because it is the only store whose loss is unrecoverable; the other two take the
     #D3E8F3 fill. Give the central Fleet server panel a solid #192147 fill with white text.
...
<!-- IMAGE-REDO: assets/1.6-fleet-service-state-model.webp
     ...
     Fill the MySQL card solid #009A7D with white text and give the other
     two the #D3E8F3 fill, so the ranking of what a loss costs is visible before the labels are
     read.
