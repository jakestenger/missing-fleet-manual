...
**Revealing a disk encryption recovery key is not a separate permission from reading the host it belongs to.** There is no distinct gate: anyone who can read a host can reveal its recovery key, its Recovery Lock password and its managed local account password, and that is five of the six roles at either scope. GitOps is the exception, because it can list hosts without being able to read one.
...
- It can **write MDM commands**, which is the authorization behind lock, unlock, wipe and clear passcode. It can issue them and cannot read their results.
