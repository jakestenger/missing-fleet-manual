...
The rendered picture is out of date and marked for regeneration: it still labels scope "which devices?" and draws the interface as an authorization gate. Read the four layers above rather than the image until it is replaced.
...
- It can read **certificate authority integration secrets**: the API tokens, passwords, SCEP challenges and client secrets Fleet holds for the certificate authorities you have connected. Not their private keys, which no surface returns to it.
