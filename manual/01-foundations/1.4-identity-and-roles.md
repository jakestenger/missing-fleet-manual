...
<!-- IMAGE-REDO: assets/1.4-access-gates.webp
     WHY: reviewed 2026-08-24. Much improved: the ornate gates are gone and the
     narrowing now reads. One thing left. The band tapers continuously between the bars, so
     it looks like gradual attrition rather than four discrete decisions. Make it step down
     at each bar and hold a constant height between them, so the reader sees four cuts rather
     than a funnel. The outcome box should also be a solid fill rather than a thin outline.
