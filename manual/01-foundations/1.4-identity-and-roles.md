...
-->
<!-- IMAGE REMOVED 2026-08-30: the rendered 1.4-access-gates.webp still labeled scope "which devices?" and drew the interface as an authorization gate, and shipping a knowingly wrong picture with an apology serves nobody. Regenerate from the prompt above, then restore the image here with the alt text "Diagram of the four layers a request passes: authentication, role and scope together, endpoint restrictions, and account and deployment state." and the caption: The diagram maps the four layers that decide whether a request passes Fleet's access check. -->
