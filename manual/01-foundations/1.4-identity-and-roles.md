...
Fleet accounts can inspect devices and act on them, so access mistakes have physical consequences. Identity, role, scope, and the other checks that decide whether a request is allowed are separate things.

What Fleet can do to any given device depends on the platform, the enrollment, the licence and the management channel as well as the role. Reading and acting are separate permissions, and Fleet's read-only roles exist to hand out one without the other. Grant too much and a support account can wipe a laptop. Grant too little and the people who own a device population have to escalate every task to someone who does not know their environment.

## What decides whether an action is allowed

![Explanation](../_assets/icons/explanation.svg) Four layers stand between a request and the work it asks for, and only the middle two are what people mean by "permissions". They do not all narrow: authentication and account state **stop** a request outright, while endpoint restrictions narrow one that is otherwise allowed, and only for a configured API-only account.
...
**Five separate licence checks meet here, and they fail in three different request contexts**: creating a fleet, changing fleet membership, and creating or modifying a user. Guessing which is which is the reason a Free `ErrMissingLicense` gets diagnosed against the wrong thing:
...
> **Those checks belong to the user create and modify paths, not to roles as such.** Fleet has a third route that applies roles in bulk from a spec, and it performs none of the validation above: no licence check, and no API-only check. A Free deployment can be given Premium-only roles through it, and it is the same route GitOps drives. Do not read the table as an invariant about what a role can be; read it as what these two paths enforce.
...
**A stored empty list is no restriction rather than deny-all**: an API-only account with nothing in `user_api_endpoints` reaches every registered route, gated by its role alone. So clearing a list to stage a change hands the account everything its role permits, not nothing.

An entry is also a **method and a normalized path together**, drawn from Fleet's catalogue, so allowing a path does not allow every verb on it. On Free the licence check rejects any **non-null** list, so an explicit empty array is refused there rather than treated as "no restrictions requested".

**The interface is not one of these decisions.** The UI, `fleetctl`, GitOps and the REST API are surfaces onto the same authorization, not gates of their own, and choosing one does not change what an identity may do. An interface does decide availability: an API-only account cannot use the UI at all.

**`401` and `403` are the two you will meet most, and neither is exhaustive.** Two account-level and deployment-level conditions produce refusals that have nothing to do with roles or scope:
...
**Audit-log delivery is not one of them.** Where Fleet sends its audit stream is set through the server's own configuration keys rather than through anything in the interface, so no Fleet role grants or withholds it. It is a change to the deployment, made by whoever operates the server ([2.8](../02-administer-and-deploy-fleet/2.8-activity-audit-logs-and-log-delivery.md)). An access review that treats it as an administrator's privilege will look in the wrong place.
...
**Fleet scope is about more than which hosts an identity can see, in both directions**, and an access review built only around host visibility misses both:
...
- Nor does it seal off everything global. **All six fleet-scoped roles can read global configuration**, Observer and GitOps included. That is deliberate, since a fleet-scoped user needs to know how the deployment is set up to work in it, but it means "confined to their fleet" is not literally true and should not be relied on as an isolation boundary for server settings.

### Role and scope are independent of each other

**The same set of roles exists at both levels.** A global maintainer and a fleet maintainer hold the same kind of authority; they differ only in how far it reaches.

That independence lets one person hold different roles in different fleets, which is a supported and useful arrangement. Someone can administer the fleet they own while holding read-only access to a neighbouring one they occasionally support.

It also means a role name on its own only answers half of "what can this person do". [1.3](1.3-hosts-fleets-labels.md) has the other half, and the two compose: **a fleet bounds devices and people at the same time.** That is a good reason to design fleets deliberately instead of letting them accumulate.
...
Read access is not automatically safe here, because some of what Fleet can read is a secret.

**Revealing a disk encryption recovery key is not a separate permission from reading the host it belongs to.** There is no distinct gate: anyone who can read a host can reveal its recovery key, its Recovery Lock password and its managed local account password, and that is five of the six roles at either scope. GitOps is the exception: the reveal checks the ordinary ability to list and read hosts, and GitOps has neither ([a.4](../09-appendices/a.4-roles-and-permissions-matrix.md)), holding only the narrower read of a host whose identifier it already knows.

**At fleet scope that means the hosts in their fleets. At global scope it means every host in the deployment.** A global read-only account can read every recovery key you hold.

That is deliberate and useful: recovering a locked-out user is exactly the task you want a support person to handle without also granting them the ability to change configuration. It is simply not what the phrase read-only suggests.
...
Fleet's GitOps role is write-oriented: it can apply configuration and cannot read most of what it applied. **It is not literally write-only, and the exceptions are larger than the intent suggests.**
...
**Four exceptions bear on whether a GitOps token is low-value**, and each is deliberate:
...
A credential whose purpose is to apply configuration from a pipeline can also erase a Mac, and Fleet's own comment above that rule names only administrators and maintainers.

A denied read is a `403`, not an empty success. So a GitOps token that returns nothing has not been refused: an empty result means the request was authorized and there was nothing to return, which is a different finding from a refusal and points somewhere different. Read the status code before concluding either way.
...
Fleet records a defined set of activities, and that record is what [1.5](1.5-audit-and-activity.md) is built on. It is an enumerated list of things Fleet considers worth recording rather than a log of every request, which bounds what the audit record can be relied on to prove.

**Attribution is two independent facts, not one label.** A row may name a user, which can be a person or an API-only service identity. It may be flagged Fleet-initiated, which the activity type has to declare. **It may be both**, when an automation acts on a user's behalf, and it may be neither, because Fleet accepts an activity with no user at all. Do not build an audit control on the assumption that every action traces to a person or a service account, and do not read a Fleet-initiated flag as proof there was no user.

An action attributed to a person tells you who to ask. One attributed to a well-named service identity tells you which workflow to go and read. One attributed to a token that three people share tells you very little ([8.12](../08-troubleshooting/8.12-audit-logs.md)).
...
<!-- IMAGE-TODO: assets/1.4-account-review-lifecycles.webp
     ...
     lifecycle distinctions with a short explanation and this owner/action map; retain licensing,
     restart behavior, last-admin protection, and the requirement to review service identities. Keep
     ...
     CANDIDATE: ../../research/visual-reviews/part-0-1/assets/1.4-account-review-lifecycles.webp
...
![SCIM removes eligible SSO accounts; password and API-only accounts need owner-led review, and the last global administrator is protected.](assets/1.4-account-review-lifecycles.webp)
-->
