...
![Explanation](../_assets/icons/explanation-light.svg) Fleet’s role model lets you share device-management work with the people who know those devices best. A support group can look after its own fleet, a security reviewer can inspect device information, and an automation account can carry out a defined workflow. You decide the kind of work each account can perform and where it can perform it.

Those choices also govern sensitive actions, including reading recovery credentials and wiping a laptop. Start with the responsibilities you want to delegate, then choose the role and scope that support them.
...
![Explanation](../_assets/icons/explanation-light.svg) Fleet evaluates an account’s identity and permissions whenever it handles a request. It first authenticates the account, then considers its role and scope, any endpoint restrictions, and the account or deployment conditions relevant to that operation. Following these checks in order helps you understand why an action is available to one account and refused to another.
...
Licence requirements apply to individual operations. The table shows the checks made by each listed operation; when a request fails, use its operation to identify the requirement that applies:
...
### Global Admins
![Explanation](../_assets/icons/explanation-light.svg) Some decisions are naturally organization-wide: organization configuration, identity-provider and SCIM setup, MDM connections, and other shared settings. Keep these with a small group of global administrators who can coordinate their impact across the deployment.
...
<!-- SCREENSHOT: SS-1.4-01 | Assigning a role within a fleet
...
Read access can help support staff investigate a device and recover access for its user. It also includes recovery credentials, so consider who should be able to retrieve those credentials when assigning a reader role.
...
Support staff can therefore help a locked-out user without receiving configuration-write permission. Include that recovery capability explicitly in the role’s responsibilities and in subsequent access reviews.
...
![Explanation](../_assets/icons/explanation-light.svg) Connect human accounts to your organization’s identity provider. Use SCIM and role sync where appropriate to manage account creation, changes, and departures.
...
Fleet records defined activity types, which [1.5](1.5-audit-and-activity.md) explains in detail. Some API requests produce no activity, so the record covers those implemented events rather than every request.
...
<!-- SCREENSHOT-REUSE: SS-1.5-01. Reuse the activity-feed capture from 1.5 here to illustrate named human and service-account actors; no additional capture is needed. -->

### Where account lifecycle should live

Plan how access will change when someone changes responsibilities or leaves. Giving account removal an owner and a trigger makes that part of the access model maintainable.
