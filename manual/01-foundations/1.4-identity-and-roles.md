...
These are the states an endpoint list can be in, and they are easy to get wrong in opposite directions:
...
An entry is also a **method and a normalized path together**, drawn from Fleet's catalogue, so allowing a path does not allow every verb on it. And on Free the licence check rejects any **non-null** list, so an explicit empty array is refused there rather than treated as "no restrictions requested".
...
The diagram maps the four layers that decide whether a request passes Fleet's access check.
...
- And it does not seal off everything global. **All six fleet-scoped roles can read global configuration**, Observer and GitOps included. That is deliberate, since a fleet-scoped user needs to know how the deployment is set up to work in it, but it means "confined to their fleet" is not literally true and should not be relied on as an isolation boundary for server settings.
...
In most systems read access is the safe thing to hand out. Here it is not automatically safe, because some of what Fleet can read is a secret.
...
**Four exceptions are worth knowing before you treat a GitOps token as low-value**, and each is deliberate rather than accidental:
...
The last of those is the one to weigh. A credential whose purpose is to apply configuration from a pipeline can also erase a Mac, and Fleet's own comment above that rule names only administrators and maintainers.
