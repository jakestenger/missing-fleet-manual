...
Access control carries more weight here than in a tool that only reports, because the same system that sees every device can also change every device. Those are separate permissions, and Fleet's read-only roles exist precisely to hand out one without the other. Grant too much and a support account can wipe a laptop. Grant too little and the people who own a device population have to escalate every task to someone who does not know their environment.

## What decides whether an action is allowed

![Explanation](../_assets/icons/explanation.svg) Two questions decide it, asked in order, and a third can narrow the answer further for automated identities.

**Authentication** answers who is making the request. For a person this is normally their Fleet account reached through organization sign-in. For automation it is a dedicated API-only account, whose token is a credential belonging to that account rather than an identity in its own right. Failing here is a `401`, and it happens before permissions are considered at all.

**Role and scope together** answer what that identity may do, and they are one authorization decision rather than two. The role says which kinds of work are permitted; the scope says over which hosts. Failing here is a `403`, from an identity Fleet has already recognised.

**An identity is global or fleet-scoped, never both.** Fleet rejects an account carrying a global role and a fleet role together. A global assignment works across the organization; a fleet assignment confines the same role to that fleet's hosts and resources, and one person can hold different roles in several fleets when their responsibilities differ. **Two separate licence boundaries meet here, and knowing which is which tells you where the error will come from.**

- **Fleets themselves are Fleet Premium.** Fleet Free refuses to create one outright, so fleet-scoped assignment is unavailable on Free as a consequence rather than as a rule about roles. The error arrives when you create the fleet, not when you assign someone to it.
- **Three role names are Premium at either scope**: Technician, Observer+ and GitOps. Fleet rejects a user carrying any of them, global or fleet-scoped, on a Free licence. Admin, Maintainer and Observer are not in that set.

**Endpoint restrictions** can narrow an API-only account further, to a named list of API paths. This is the only one of the three that is optional, and it only ever subtracts: granting an endpoint never confers permission the role did not already carry.

**The interface is not one of these decisions.** The UI, `fleetctl`, GitOps and the REST API are surfaces onto the same authorization, not gates of their own, and choosing one does not change what an identity may do. The one thing an interface does decide is availability: an API-only account cannot use the UI at all. Diagnosing a permission problem means asking which of the three questions above failed, not which interface was used.
...
### Global Admins
![Explanation](../_assets/icons/explanation.svg) Some decisions are naturally organization-wide: server configuration, identity-provider and SCIM setup, MDM connections, audit-log delivery, and other shared settings. Keep these with a small group of global administrators who can coordinate their impact across the deployment.
