...
3. **Fleet Desktop** is the part an end user usually sees: the menu bar item, the My Device page, and the dialogs that belong to those surfaces. It is not the only thing fleetd can put on screen. It runs as `fleet-desktop`, and only when someone is logged in, since there is nothing for it to do otherwise. **It is the one optional component**, included when the package is built with `--fleet-desktop`, so a host can be running fleetd and not have it at all ([3.8](../03-connect-devices/3.8-manage-fleetd-orbit-and-updates.md)).

The chain of custody runs one way. The operating system's service manager starts Orbit, and Orbit starts the other two. That makes Orbit the first thing to check.

**Killing Orbit does not reliably take its children with it**, most visibly on Windows, where there is no `SIGTERM` and ending the process in Task Manager can leave `osqueryd` orphaned and still holding the lock on its database. Orbit knows this: at startup it kills every pre-existing `osqueryd` process it can find before it tries to read host information, because an orphan would otherwise block it. So "Orbit is not running" does not prove "osquery is not running", and an orphaned `osqueryd` is worth looking for when a host will not start cleanly ([8.4](../08-troubleshooting/8.4-host-side-investigation.md)).
...
**ChromeOS is the exception.** There is no bundle to install. Its counterpart is a browser extension called *Fleetd for Chrome*, running as a service worker: no Orbit, no osqueryd, no Fleet Desktop. It does part of fleetd's job rather than all of it.
...
![Explanation](../_assets/icons/explanation.svg) **The division of labour describes a host running fleetd**, which is macOS, Windows and Linux; the platforms without fleetd divide the same work differently, and the applicability matrix later in this chapter says how.

Anything that answers **what is on the host right now** belongs to osquery. Live reports, scheduled results, host vitals, software inventory, file carves.
...
**Lock and wipe are the exception**: the split follows the mechanism available rather than the capability, and the two do not split the same way as each other:
...
So a Windows lock that fails is not an OMA-DM problem to start with: it needs Windows MDM configured, which Fleet does check, and it needs Orbit running with scripts enabled to execute. Fleet refuses the request when the host has reported scripts as disabled, and otherwise queues it and lets the agent try. All of these are Fleet Premium ([3.4](../03-connect-devices/3.4-enroll-linux-devices.md), [3.6](../03-connect-devices/3.6-enroll-android-devices.md) for the one free case).
...
The sixth is set apart because it authenticates differently, not because it is optional. It is a **device-authenticated API**, reached at `device/...` with a per-host token rather than a node key, which is what lets a person open My Device without holding a Fleet account. Fleet Desktop is its main consumer, and it is not the only one: **Orbit initialises the client and manages that token whether or not Fleet Desktop was included in the package**, and the macOS setup experience uses the same surface. Fleet's own architecture drawing gives it its own arrow alongside Orbit's and osquery's.

Five plus Desktop is the count for that host, not a universal property of a managed device. The matrix at the end of the section gives the count for every other platform.
...
The five below are the management families. The counts, the matrix and "which channel is failing" all refer to those five; the Fleet Desktop API sits outside that boundary deliberately, and [8.1](../08-troubleshooting/8.1-diagnostic-method.md) uses the same boundary.
...
Three of them belong to osquery, and they split the work the way osquery always has. One carries questions to the host and answers back. One fetches the host's configuration, meaning its query schedule and the `agent_options.config` block; Orbit's own settings arrive on the fourth channel instead. One ships results outward, and only outward. File carving adds a fourth osquery endpoint on the hosts that use it, so three is the normal case rather than a closed set.
...
**The families are not sealed off from each other.** A Windows host on a relaxed MDM poll is woken by Orbit, so if Orbit is wedged, queued MDM commands can sit for hours with nothing wrong in the MDM subsystem at all ([8.9](../08-troubleshooting/8.9-windows-mdm-diagnostics.md)). A synchronous script run is refused for a host Fleet considers offline, which is a judgement made from osquery check-ins, so a broken osquery channel can block Orbit work that osquery has nothing to do with.
...
The ChromeOS extension is not an osquery agent and there is no `osqueryd` on the device, yet it speaks Fleet's osquery protocol: it enrolls at the ordinary enroll endpoint and polls distributed read, answering from its own virtual tables and posting to distributed write. Those three are the only Fleet endpoints it uses. There is no config fetch and no logger upload, which is the mechanical reason **scheduled queries are not available on ChromeOS** while live reports work perfectly well.

A live report returning data is not evidence that osquery exists on the host. A platform's channel count is not a measure of how much Fleet can do with it, only of how many separate places there are to look when something breaks ([3.7](../03-connect-devices/3.7-enroll-chromeos-devices.md)).
...
Treat online as "Fleet saw recent osquery traffic", not "every management channel works": it is generous by at least a minute, and a host that has just gone quiet keeps showing online for its whole window. The exact boundary: Fleet reads that second interval from the host's `config_tls_refresh` flag, and falls back to `config_refresh` only when the host does not report the first. The two are the same setting under two names, split by an osquery rename years ago, and Fleet still checks both because old agents answer to the older one. fleetd sets `config_refresh` to 60 seconds when it starts osquery and leaves `config_tls_refresh` alone.

Which of the two a given host ends up reporting, and therefore whether its window is the 60-second grace period alone or 60 seconds plus a check-in interval, depends on how the host's osquery reports flags it was never given. That is agent behaviour rather than Fleet behaviour, and it is not documented at this release; unverified. The shape is fixed: **online is a statement about the osquery channels and nothing else**.
...
**That nesting is not uniform.** The split is not "osquery settings here, Fleet settings there", because osquery's own settings appear at both levels:
...
So `distributed_interval` is nested and `command_line_flags` is not, and both are osquery. When a value appears to be ignored, check the level it was written at before anything else.

The two intervals do different jobs. **Lowering `distributed_interval` speeds live-report pickup**, because that is the channel a live report travels. **The online window is the smaller of the two intervals plus the grace period**, so only the smaller one moves it: changing the larger value does nothing to the window until it crosses below the other. Raising the smaller one reduces check-in traffic and extends how long Fleet treats the last check-in as current. Choose values that match the responsiveness and network profile of the fleet.

**Online does not tell you whether queries will work.** The dot is weaker than "osquery is answering". Fleet stamps the host's last-seen time on *any* authenticated osquery request, and the config, distributed and logger endpoints all qualify. A host whose distributed channel is broken but which is still fetching its configuration every sixty seconds, or still shipping buffered results, keeps its last-seen time fresh and keeps showing online while every live report against it fails ([8.1](../08-troubleshooting/8.1-diagnostic-method.md)).

So online is a statement about **the osquery family as a whole**, not about any one endpoint in it, and not about Orbit or MDM at all, with one carve-out: a host that has never recorded a check-in reads as online from its creation time instead.

The MDM and Orbit paths are visible only through the status of the work they perform. A host can answer a live report while an MDM command waits for the platform's next check-in; an MDM-enrolled device reports MDM status before fleetd exists on it. The diagnostic method is therefore to identify the family associated with the requested action, then inspect that family's own evidence rather than the status dot.


<!-- IMAGE-TODO: assets/1.2-online-evidence-boundary.webp
     ...
     explanations of what online does and does not prove. Keep the exact interval formula, flag-name
     fallback, uncertain agent behavior, and YAML in text. Keep current prose and this TODO until
     ...
     CANDIDATE: ../../research/visual-reviews/part-0-1/assets/1.2-online-evidence-boundary.webp
...
![Online derives from osquery freshness, with a new-record fallback; it provides no proof that the MDM path works.](assets/1.2-online-evidence-boundary.webp)
-->
...
One consequence needs settling before the first device enrolls, because reversing it is awkward. When Android is turned on, Fleet builds the subscription's push endpoint out of the server URL as it stands at that moment, and Google delivers events to that endpoint from then on. Google therefore has to be able to reach it, which is why Fleet refuses to enable Android against `localhost` or a loopback address at all.

**Treat the URL as operationally fixed from that point.** Fleet will accept a changed `server_url` afterwards, because the setting is validated only for being present and well-formed, and this release's change handling resynchronizes Apple's enrollment profiles rather than Google's push endpoint. The result of changing it is not a rejection but a quiet outage: Fleet moves and Google keeps pushing to where Fleet used to be. This manual can establish the breakage but not the cure: the tagged source shows Fleet accepting the change and not moving the subscription, and does not establish a supported recovery, so plan on not needing one ([2.12](../02-administer-and-deploy-fleet/2.12-bind-android-enterprise.md)).
...
![Explanation](../_assets/icons/explanation.svg) Orbit updates the whole bundle, including itself, from its configured update repository. **This path does not go through the Fleet server**, so agent updates continue working when the Fleet server is unreachable, and blocking that repository silently freezes agent versions.
...
![Reference](../_assets/icons/reference.svg) **Orbit cannot reach Fleet.** The configuration poll backs off gradually from thirty seconds to a ceiling of five minutes, and one success resets it. The source establishes the backoff itself and its recovery: the interval grows toward five minutes while cycles keep failing, and returns to its normal value on the first success, at which point Orbit resumes its ordinary work. osquery is a separate process and keeps running, so the host can still answer live reports. Network errors are logged sparingly, so log volume is not a measure of severity.

Do not extend "held" into "nothing at all is ever lost". Individual features apply their own expiry and cancellation rules to work that has been waiting, and this chapter does not establish a single retention guarantee across every script, install, queued command and result class; unverified. Nor does it establish that osquery replays everything with original timestamps in every configuration, which is osquery's behaviour rather than Fleet's and depends on its buffering settings. The source does show Orbit's own poll backing off and resuming, which is a pause in the agent's work rather than a discard of it.

**Enrollment fails.** Orbit makes up to six attempts with a growing delay, roughly 10, 20, 40, 80 and 160 seconds between them, then gives up until the next attempt. `FLEETD_SILENCE_ENROLL_ERROR` is narrower than its name suggests: it suppresses a single retry-notice line, while **every failed attempt is still logged regardless**, at Orbit's ordinary info verbosity. So the variable removes one retry notice, not the enrollment attempts themselves, which are already in the log ([8.2](../08-troubleshooting/8.2-log-surfaces.md)).
...
**A failing target stops the whole update pass.** Orbit walks its targets in order, Orbit itself, then osquery, then Desktop if it is installed, and it abandons the whole pass the moment a lookup or an update for one of them returns an error. So a stuck Orbit target does not just stall Orbit; it stops osquery and Desktop from even being checked on that pass, for as long as it keeps failing. Correlated stale versions across the bundle are a symptom of one blocked target, not three separate problems ([3.8](../03-connect-devices/3.8-manage-fleetd-orbit-and-updates.md)).
