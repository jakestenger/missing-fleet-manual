...
Anything that uses **the platform's native management** belongs to the OS MDM client. Profiles, MDM commands, managed app delivery, and, on the platforms that have MDM, lock and wipe. Fleet composes the instruction; the operating system carries it out.

**Lock and wipe are the exception that proves the split is about mechanism rather than capability.** On macOS and Windows they are MDM commands. On Linux, which has no MDM, Fleet Premium performs them as scripts over the agent instead, so they land in Orbit's column and depend on scripts being enabled ([3.4](../03-connect-devices/3.4-enroll-linux-devices.md)).
...
![Explanation](../_assets/icons/explanation.svg) Fleet marks a host online when it has checked in through osquery within its normal check-in window, plus a 60-second grace period. The window is the shorter of two intervals the host itself reports: its `distributed_interval`, and its config refresh interval.
...
**That nesting is not uniform, and it catches people.** Anything osquery consumes sits under `agent_options.config`, because Fleet passes that block through to osquery more or less as written. Settings Fleet itself acts on, such as the update channels below, sit directly under `agent_options`. When a value appears to be ignored, the level it was written at is worth checking before anything else.
...
|---|---|---|
| fleetd, no MDM | Live and scheduled reports, vitals, software inventory, scripts, Orbit-driven software installs, LUKS escrow on Linux, and **lock and wipe on Linux**, which Fleet Premium runs as scripts rather than as MDM commands | Profiles, MDM commands, App Store apps. Lock and wipe on macOS and Windows, which do require MDM |
| MDM, no fleetd | Profiles, MDM commands, App Store apps, lock, wipe, and inventory on the platforms where MDM supplies it | Reports, scripts, Orbit installs, and inventory on platforms that depend on the agent for it |
...
![Explanation](../_assets/icons/explanation.svg) Fleet Desktop is the only end-user-facing surface fleetd has. It carries the My Device link, Self Service, and the dialogs that need a human answer, including the macOS MDM migration prompt.

**Not every prompt fleetd shows belongs to it.** Linux disk-encryption escrow prompts for the user's LUKS passphrase, and that prompt is Orbit's: it goes through `zenity` or `kdialog` if one is installed on the host, and does not involve Fleet Desktop. So a Linux host without Fleet Desktop can still escrow, and a Linux host without either dialog tool cannot, whatever else is installed ([3.4](../03-connect-devices/3.4-enroll-linux-devices.md)).
...
**An update fails.** Repeated failures buy a **24 hour pause**, and the pause is keyed to the specific artifact being fetched rather than to the component or the host, so everything else carries on. Two things clear it: publishing a new version, which is a different artifact, and restarting the agent, since the retry state is held in memory and does not survive the process.
