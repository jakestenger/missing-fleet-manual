...
**This recovery is macOS-only in 4.90.1.** The whole comparison sits behind a `darwin` check, so a Windows or Linux image built from a machine that had already enrolled carries the previous host's enrollment with it and nothing on the host will notice. Imaging procedures for those platforms have to remove the agent state themselves ([3.3](../03-connect-devices/3.3-enroll-windows-devices.md), [3.4](../03-connect-devices/3.4-enroll-linux-devices.md)).
