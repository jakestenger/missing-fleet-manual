...
<!-- IMAGE-REDO: assets/1.2-five-channels.webp
     WHY: reviewed 2026-08-24. **This one went backwards and should be rebuilt from the
     version before last, changing only the brace.** The brace fix landed, and everything
     around it broke. The "osqueryd" label now overlaps the host outline and is unreadable.
     The host is a tall empty rectangle that no longer reads as a laptop, and the server is a
     stack of loose lines. A large empty gap sits between arrows 3 and 4. The frame is much
     taller than 16:9 and mostly white.
     What was right two versions ago: a recognisable laptop on the left, a recognisable
     server on the right, five evenly spaced arrows reaching both, and labels clear of every
     shape. Restore that, then make the single change: **one brace spanning arrows 1 to 3**
     labelled osqueryd, not three separate braces, because those three channels share one
     process.
