...
The sixth is set apart because it is optional and because nothing an administrator does depends on it: Fleet Desktop talks to Fleet over its own `device/...` API, authenticated by a per-host token rather than by a node key, which is what lets a person open My Device without holding a Fleet account. Fleet's own architecture drawing gives it its own arrow alongside Orbit's and osquery's. It fails on its own, and when it does, Self Service and My Device stop while every management path carries on.
...
The two intervals do different jobs and it is worth not merging them. **Lowering `distributed_interval` speeds live-report pickup**, because that is the channel a live report travels. **Lowering either interval shortens Fleet's online window**, because the window is the shorter of the two plus the grace period, so it makes the online signal fresher and the host quicker to be called offline. Raising either reduces check-in traffic and extends how long Fleet treats the last check-in as current. Choose values that match the responsiveness and network profile of the fleet.
