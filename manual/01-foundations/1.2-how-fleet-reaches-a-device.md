...
**Enrollment fails.** Orbit makes up to six attempts with a growing delay, roughly 10, 20, 40, 80 and 160 seconds between them, then gives up until the next attempt. `FLEETD_SILENCE_ENROLL_ERROR` is worth knowing about, but it is narrower than its name suggests: it suppresses one error-level line, while **every failed attempt is still logged at info level regardless**, at Orbit's ordinary verbosity. So a host that looks quiet is quiet at error level only, and the enrollment attempts are already in its log ([8.2](../08-troubleshooting/8.2-log-surfaces.md)).
