...
<!-- IMAGE-REDO: assets/1.2-five-channels.webp
     WHY: reviewed 2026-08-24. The dot colours work: five arrows now read as five
     independent things, which is the whole claim. Two regressions from the previous version.
     **The osqueryd brace is repeated three times, once per arrow**, when it should be one
     brace spanning arrows 1 to 3; the point of that brace is that those three share a single
     process, and three separate braces says the opposite. And the arrows float in the middle
     of the frame without reaching either the laptop or the server, so nothing connects.
     Extend them so they meet both.
