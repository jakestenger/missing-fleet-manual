...
| Windows hosts report management off | They are waiting for a user to sign in, from [2.11](../02-administer-and-deploy-fleet/2.11-windows-and-android-management-configuration.md) |
...
The decisions that are genuinely hard to reverse are named in [2.1](../02-administer-and-deploy-fleet/2.1-administration-model-and-deployment-choices.md), and two of them belong in the handover document because a successor will not guess them: the server URL, which [2.11](../02-administer-and-deploy-fleet/2.11-windows-and-android-management-configuration.md) shows Fleet will accept a change to while Google keeps delivering to the old one, so editing it silently breaks Android management, and the Windows key pair, which [2.11](../02-administer-and-deploy-fleet/2.11-windows-and-android-management-configuration.md) shows is also the escrow key for disk encryption.
...
![Reference](../_assets/icons/reference.svg) Verified against Fleet 4.90.1. Fleet publishes no backup or restore procedure at this release. That is a documentation gap rather than a product limitation, and it may close; the obligations it leaves with you will not.
