...
A histogram is not one series but several: you query its `_bucket`, `_sum` and `_count`, never the bare family name. The summary exposes its quantiles under the family name plus a `_sum` and a `_count`. The two counters you query by name.
...
| `api_service_request_latency_microseconds` | summary | `api_service_request_latency_microseconds`, `_sum`, `_count` | `method`, `error` |
