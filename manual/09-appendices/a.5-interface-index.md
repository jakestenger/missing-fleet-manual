...
![Reference](../_assets/icons/reference.svg) All 381 register rows, grouped as a reader would look for an action. Section rows in bold carry no cells; they mark where a family starts. Counts by value are published after the table and were recounted from it.
...
**381 rows, 1,524 cells, no blanks.**

| Value | UI | REST API | `fleetctl` | GitOps |
|---|---|---|---|---|
| **Full** | 213 | 201 | 194 | 133 |
| **Partial** | 56 | 107 | 57 | 35 |
| **Read only** | 12 | 6 | 8 | 0 |
| **Unsupported** | 31 | 55 | 121 | 213 |
| **Not established** | 69 | 12 | 1 | 0 |
| **Total** | **381** | **381** | **381** | **381** |

Four things in that shape are worth reading before you use any single row.

**The REST API reaches more actions than any other interface**, 308 at `Full` or `Partial` against 269 for the UI, 251 for `fleetctl` and 168 for GitOps. The other three are clients of it, so its reach is the ceiling theirs are measured against.
...
**GitOps is `Unsupported` on 112 rows the UI and the REST API can both perform.** That is not a defect in GitOps. It is the closed vocabulary and the missing read direction working as designed, and it is the number that bounds how much of Fleet a repository can manage.
