...
**This decision alone decides the `fleetctl` column on the 85 rows no specification file reaches and `fleetctl api` would**, and it is what lets a specification file's reach count as `fleetctl` support at all, which is why it is stated before the matrix rather than inside it.
