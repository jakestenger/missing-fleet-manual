...
- **`Allowed; empty in 4.90.1`**, the policy permits the request and the request succeeds, but `whereFilterHostsByTeams` names no GitOps role at either scope, so the clause is `"FALSE"` at and the result is empty or zero-valued. Eight matrix cells carry this, on the eight routes in the row universe §6.3. It is **not** applied to `host · list` or `host · read`, which are policy denials for GitOps and are simply `Denied`.
...
| Move a host between fleets<br>`host · transfer_host` | Allowed | Allowed | Allowed | Denied | Denied | Allowed; empty in 4.90.1 |
...
| **Group 9, Labels** | | | | | | |
| See labels<br>`label · read` | Allowed | Allowed | Allowed | Allowed | Allowed | Allowed; empty in 4.90.1 |
| Create a label<br>`label · create` | Allowed | Allowed | Allowed | Denied | Denied | Allowed; empty in 4.90.1 |
| Edit or delete a fleet's own label<br>`label · write` | Allowed | Allowed | Allowed | Denied | Denied | Allowed; empty in 4.90.1 |
| Edit or delete a global label<br>`label · write` | Allowed | Allowed | Allowed | Denied | Denied | Allowed; empty in 4.90.1 |
| **Group 10, Reports, live queries and carves** | | | | | | |
| See a saved report<br>`query · read` | Allowed | Allowed | Allowed | Allowed | Allowed | Allowed; empty in 4.90.1 |
...
| **Group 11, Policies** | | | | | | |
| See a policy<br>`policy · read` | Allowed | Allowed | Allowed | Allowed | Allowed | Allowed; empty in 4.90.1 |
...
| Run a script on a host<br>`host_script_result · write`; when `scripts_disabled` is set, execution returns 403 for every role without the role being evaluated | Allowed | Allowed | Allowed | Denied | Denied | Denied |
...
| Run a script on a host<br>`host_script_result · write`; when `scripts_disabled` is set, execution returns 403 for every role without the role being evaluated | Conditional (C15) | Conditional (C15) | Conditional (C15) | Denied | Denied | Denied |
...
**C13, `gitops` reading an ADE-enrolled iPhone or iPad.** The the narrower read the policy grants GitOps grant itself is unconditional, but the one route that uses it operatively, `HostByIdentifier`, builds its response through `hostDetailResponseForHost` (), which for `platform == "ipados" || "ios"` () calls `svc.GetHostDEPAssignment` (), and that method authorizes the host with plain `fleet.ActionRead`, an action `gitops` is denied. A permission error is not a not-found, so propagates it. **Allowed**, full host detail, for every host except an iOS/iPadOS host on an instance where `ac.MDM.EnabledAndConfigured` is true. **Denied**, the whole request fails, for an iOS/iPadOS host on such an instance.

**C15, the object must belong to a real fleet (the Unassigned-fleet exclusion).** The single most consequential structural fact in the policy: it accounts for 128 of the ledger's 201 conditional cells. Every fleet-scoped rule marked C15 is keyed on `team_role(subject, object.team_id)`, most carry an explicit `not is_null(object.team_id)` guard, and several also require `object.team_id != 0`. **Allowed** when the object belongs to a fleet on which the subject holds the role. **Denied** when the object belongs to the Unassigned fleet ("No team", a fleet identifier null or 0), the fleet-role lookup is undefined for a null id, so no fleet-scoped role of any kind reaches those objects. A global role reaches them normally. Contrast `host` **list**/**selective_list** (RULE 024), which keys on `team_role(subject, subject.teams[_].id)` and never inspects the object's fleet, holding the role on *any* fleet satisfies it. So a fleet-scoped maintainer can list an Unassigned host but cannot read it.

**Two further conditions are filed in the row universe's addendum and are not cell values here**, because they qualify an outcome rather than the decision: **C31**, transfer-by-filter reporting success having transferred nothing, which is the effect behind the `Allowed; empty in 4.90.1` cell on "Move a host between fleets"; and **C32**, `RenewABMToken` mutating under `ActionRead`, noted on "Renew the Apple Business Manager token".
...
**The first three are one row, and that is the point.** Fleet has no object type for a recovery key: revealing one takes exactly the permission that reading the host takes. So the group is five of the six roles at either scope, and at global scope that means every key in the deployment. [5.8](../05-manage-devices/5.8-enforce-disk-encryption-and-manage-recovery-credentials.md) covers what to do about that; this appendix's job is to say that it is not a separate decision, because everyone assumes it is.

## Version notes

![Reference](../_assets/icons/reference.svg) Verified against Fleet 4.90.1. The policy carries 154 rules over 49 object types and 16 actions; the 142 rows here are the administrator-facing projection of them, reconciled against the manual's shared capability register.
