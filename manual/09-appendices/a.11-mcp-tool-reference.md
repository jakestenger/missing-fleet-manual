...
**The enumerating tools fan out across fleets or software versions, and a failed component request is dropped rather than surfaced.** `get_queries` and `get_policies` list the global items first, then fetch each fleet's items in parallel; the two CVE tools fetch hosts one software version at a time; `get_vulnerability_impact` also fetches the estate total separately for its denominator. When one of those component requests fails, the server logs a warning to its own process log and returns the rest as an ordinary success, with no field marking the result partial. A count from these tools can therefore run silently low: a fleet's queries or policies missing, a CVE's host list short by whichever versions failed, or `get_vulnerability_impact`'s total-systems denominator falling back to zero when the estate count fails. Before treating any of these as a census, a compliance total, or a security decision, retry and read the server's process log for those warnings; only a warning-free run is a complete one.
...
| `label` | Restrict to one label by name | Resolved to a label id. Single label only; Fleet does not intersect multiple labels. `label` and `platform` do not compose: when both are set the label wins and `platform` is silently ignored, so never pair them (see below). |
| `platform` | `macos` / `windows` / `linux` / etc. | A standard Fleet host filter, resolved to a built-in platform label. When `label` is also set the label wins and this is silently ignored (see the `label` row); never pair them. |
...
**Never combine `label` and `platform` on a live query.** They do not intersect: the label wins and `platform` is silently dropped. SQL that was validated against the declared `platform` can then execute against labeled hosts running a different operating system, which is not what the caller asked for. Scope a live query by `label` or by `platform`, not both.
...
**In cross-host mode, `platform` filters the installable catalogue, not host inventory by operating system.** It narrows the returned titles to those that carry an installable artifact (a software installer, a VPP app, or an in-house app) targeting that platform for the fleet, which is why it requires `fleet` and answers only for a team's own installers. It does not restrict the list to software actually observed on hosts running that operating system. To inventory installed software by host OS, resolve the hosts first (by platform or label) and read each host's own inventory with the per-host mode above.
