...
![Troubleshooting](../_assets/icons/troubleshooting.svg) **Seven things a reader will search for that this index has no formal row for.** They are published rather than omitted, because an index that goes quiet sends you looking for a chapter that is not there, and a stated absence takes ten seconds to act on. Only one of the seven is a genuine coverage gap with no chapter behind it; the other six are already taught or recorded and lack only a row, so read the split below rather than the headline count. Thirteen earlier entries left the count once a chapter covered them; they close this section, with links to the coverage, rather than disappearing.
