...
| **The mutual-TLS proxy in front of Okta conditional access** | [2.5](../02-administer-and-deploy-fleet/2.5-identity-providers-sso-scim-and-role-sync.md#device-conditional-access-trusts-the-proxy-set-certificate-serial) | Counted here while the proxy was a one-sentence prerequisite no chapter owned. 2.5 now owns it: the client-certificate-serial header the proxy sets, that Fleet trusts it with no cryptographic binding to the request, and the requirement that the terminator derive the serial from the verified peer certificate and discard every inbound copy, with the origin restriction and the serial-format setting. Premium. |
