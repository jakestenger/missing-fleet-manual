...
| `run-script` | Licence check added | Only where `--fleet` names a fleet other than Unassigned. The server checks the licence before it looks the saved script up, so the Free chain stops short of `script · read` |
