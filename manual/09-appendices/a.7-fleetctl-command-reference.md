...
verified_source: "drafted against fleet-v4.90.1 (dd0200f062). The inventory is the assembled root command tree at the tag rather than the output of any installed binary, and every permission chain was traced from the command to the authorization decisions it reaches. Citation ledger at research/section-notes/a.7-notes.md"
reviewed_by:
reviewed_on:
further_reading:
...
![Reference](../_assets/icons/reference.svg) Every public command and leaf subcommand in the 4.90.1 tree, with what it asks Fleet to do, what it must be allowed to do, whether it destroys anything, whether it returns before or after Fleet acted, what its exit status proves, and the chapter that explains the workflow.

**Exact syntax is the installed client's job.** Run `fleetctl <command> --help` for the flag list of the client you actually have, which is the only listing guaranteed to match your binary. What is here instead is the part help does not carry and no other document collects: the resolution model, the per-command contract, the register of results that mislead, and the option families that widen an operation past what its name suggests.
...
**One family exists on two platforms of three.** The self-hosted update repository commands are built for macOS and Linux. On Windows the family is present as a single entry that fails on any invocation, including `fleetctl updates --help`, with a message telling you to use a Linux environment.
...
| **Named entries** | **77** | **72** |
| Behavioural rows in the index below | 69 | 64, plus the update family's terminal entry |

**A behavioural row is an invocation that does something.** Nineteen of the 27 top-level commands act in their own right; the other eight are containers whose job is to hold subcommands, and they appear below as group headings rather than as rows, because a row for a command that does nothing describes a behaviour that does not exist.
...
| The same, under `config get` or `config set` | **The context is created**, with a printed note. So `config set --context prod-typo` succeeds and writes a context nothing will ever use |
...
**Every access contract in the index below is the suffix after those two.** They are stated here once rather than in 51 rows, and a row that shows an application-configuration read is showing a second, command-specific one.

**Eighteen of the 69 rows do not carry the prefix**, and they are exactly the eighteen that ask nothing of your Fleet's authorization. Fifteen make no call to your Fleet at all: `convert`, `package`, `new`, `vulnerability-data-stream`, `prepare`, `preview` with its two subcommands, both `config` subcommands, and the five update-repository subcommands. Three reach Fleet without a credential: `setup`, `login` and `debug connection`.

### Where a value comes from when you did not pass a flag

**There are no global options.** The client accepts no flags before a command name other than `--help` and `--version`, so `fleetctl --config X get hosts` is a usage error rather than an override. Four flags are declared command by command and behave as though they were global:
...
**`--debug` dumps the client's own request bodies to standard error**, which on a `config set --token` or a `gitops` run puts a credential into the terminal scrollback and into any captured CI log.
...
**Two error paths exist and they write to different streams.** An error returned by a command's own work is written to standard error. An error that arrives without passing through that handler, which is where a flag or usage failure would land, is written to standard output after the help text. **Which class of failure takes which path is not established**: that dispatch belongs to the command-line framework rather than to Fleet. The consequence for a pipeline holds either way. **Capture both streams**, because the text you need is not reliably on one of them.
...
**The client's work completed and the outcome is in the text rather than in the status.** `run-script` run synchronously exits zero whatever the script returned, and the script's own exit code is a line of output that `--quiet` removes. `report` exits zero when it stops on a timeout.
...
**A large part of the client has no structured output at all.** Every `debug` subcommand, every `mdm` subcommand, `run-script`, `trigger`, `gitops`, `apply`, `delete`, `upgrade-packs`, `get mdm-command-results`, `package`, `new` and the update-repository subcommands print prose. Anything parsing those is parsing sentences, and sentences change between releases without a deprecation.

**Four commands accept a `--yaml` flag and ignore it**: `hosts transfer`, `goquery`, `user create` and `user delete`. The flag is declared, it is accepted, and nothing in those commands reads it.
...
![Reference](../_assets/icons/reference.svg) 69 behavioural rows, grouped by top-level family. The eight families that hold only subcommands appear as headings with no row of their own.
...
**Every chain is the suffix after the common prefix** stated above: a `version · read` that ends the command on any error, then an `app_config · read` that tolerates a permission refusal.
...
**Chapter** is the section that explains the workflow. **`None` means this manual does not explain that command**, which is a finding about the manual rather than about Fleet, and the sixteen are listed together after the index.
...
| **`apply -f <file>`** Push one spec file into Fleet, kind by kind | An ordered chain of up to thirteen steps, one per kind present in the file: `query · write` per report, `label · write` (fleet) per label, `pack · write` (global), `certificate_authority · write`, a bootstrap-package write, `script · write` (fleet), `app_config · write` (global), an Unassigned-profile batch, `enroll_secret · write` (global), `team · write` per fleet spec, the four per-fleet subchains for profiles, scripts, installers and App Store apps, `policy · write`, `user · write`. **Free:** the fleet, installer and App Store nodes evaluate no permission and return a licence error | **Destructive.** Overwrites whole objects. `--force` pushes past the server's own validation. `sync` | Every kind present in the file was accepted. With `--dry-run` it proves that three of the eight accepted kinds were checked. Z19 | [6.4](../06-automate-fleet/6.4-use-fleetctl.md) |
| **`delete -f <file>`** Delete the reports, packs and labels named in a spec file | Three kinds in a fixed order: `query · write` on each loaded report, `pack · write` (global) per pack, then `label · write` **twice** per label. Identical on both editions | **Destructive and quietly partial.** `sync` | The three handled kinds were deleted or did not exist. It says nothing about the other five kinds the same file may carry. Z20 | [6.4](../06-automate-fleet/6.4-use-fleetctl.md) |
...
| **`goquery`** Open an interactive query shell against Fleet | A chain that repeats **per shell command**. Per connect: `target · read` (global). Per query: `label · read` (global), `app_config · read` (global) twice, `query · run_new` (global) **always**, `targeted_query · run`. Identical on both editions | Interactive, and can run live queries. `interactive` | The shell exited without an error | None |
| **`preview`** (alias `sandbox`) Run a local sandbox Fleet under Docker | `local` against your Fleet. It logs in to the sandbox it created, at a fixed local address, not to your context's server | **Rewrites your client configuration file**, and enrols the machine it runs on into the sandbox. `local` | Docker came up, a sandbox was configured, and unless `--no-hosts` this machine enrolled into it. Z30 | None |
| **`vulnerability-data-stream --dir`** Download vulnerability feeds to a directory | `local`. Declares `--config` and `--context` and reads neither | Creates the directory and fills it. `local` | Every feed downloaded | None |
| **`package --type <type>`** Build a fleetd installer | `local`. Talks to the update server, never to Fleet, and declares no `--config` or `--context` at all | Writes a package. `--insecure` bakes certificate-verification-off into every host installed from it. `local` | An installer file was produced | [3.7](../03-connect-devices/3.7-manage-fleetd-orbit-and-updates.md) |
| **`prepare`** Present as a command, and does nothing | `no contract`. It fails before authentication is reached | None. `local` | **Never exits zero.** It returns an error telling you to use the `fleet` server binary | [7.2](../07-operate-fleet/7.2-upgrade-fleet-and-fleetd.md) |
...
| **`upgrade-packs -o <file>`** Write a migration file turning legacy packs into reports | The client refuses unless your account holds the global administrator role, then five decisions: `user · read` (self), `app_config · read` (global), `pack · read` (global) twice, `query · read` **(global)** | Read-only against Fleet. Writes a local file. `sync` | A file was written | None |
| **`run-script`** (alias `run_script`) Run a script on one host | Five decisions: `app_config · read` (global, a second and **less tolerant** read than the prefix), `host · selective_list` (global), `host · selective_read` (fleet), `script · read` (the fleet you named, on the `--script-name` form), `host_script_result · write` (the host's fleet). **Premium** only where `--fleet` names a fleet, and there the licence check sits near the end rather than first | **Runs arbitrary code on a device as root or SYSTEM.** No confirmation. `sync` by default; `--async` returns an execution identifier. Refused when the host is not online, or when scripts are disabled | Fleet delivered the script and a result came back. **The script's own exit code is a line of output**, which `--quiet` removes. Z18 | [5.3](../05-manage-devices/5.3-run-and-manage-scripts.md) |
| **`gitops -f <file>...`** Reconcile Fleet against declared files | A fourteen-step chain: `app_config · read` (global) twice, `label · read` (Unassigned), `team · read`, a `label · read` per non-global file, the whole of `apply`'s chain per file plus custom host vitals and organisation logos, the Apple Business token count, `app_config · write` for the token re-patch, the fleet listing and deletion under `--delete-other-fleets`, `label · write` twice per removed label, and `certificate_authority · write`. **Free:** every fleet-scoped node returns a licence error without evaluating permission | **Destructive by design.** `--delete-other-fleets` removes every fleet not named in the run. A global file with no Unassigned file applies an empty Unassigned configuration. `sync` | Reconciliation completed. With `--dry-run`, that validation passed. Z21 | [6.2](../06-automate-fleet/6.2-manage-fleet-with-gitops.md) |
...
| Command and purpose | Access contract | Effect | Result contract | Chapter |
|---|---|---|---|---|
| **`get reports [name]`** (aliases `report`, `r`; deprecated `queries`, `query`, `q`) List reports, or print one | Four decisions: `team · read` (the fleet identifier you passed), `query · read` (that fleet), `user · read` (self), and `query · read` **(global)** where the fleet holds no reports of its own and Fleet prints the inherited-reports note. **Free:** with `--fleet`, the fleet read returns a licence error without evaluating permission | Read-only. `sync` | The list Fleet returned, filtered client-side for observers. Z11 | [4.2](../04-know-your-devices/4.2-run-queries-and-reports.md) |
...
| **`get labels [name]`** (aliases `label`, `l`) List labels, or print one | `label · read` (the fleet you named). **Premium** where `--fleet` names a fleet | Read-only. `sync` | The labels Fleet returned, **minus any that failed to render**. Z24 | [1.3](../01-foundations/1.3-hosts-fleets-labels.md) |
...
| **`get user_roles`** Print every account's roles | `user · read` (global) | Read-only. `sync` | The roles Fleet returned | [2.3](../02-administer-and-deploy-fleet/2.3-user-accounts-roles-and-service-identities.md) |
...
| **`get software`** (alias `s`) List software titles, or versions | `software_inventory · read` (the fleet you named). With `--versions`, the same decision twice, because the endpoint lists and then counts. **Premium** where `--fleet` names a fleet | Read-only. `sync` | The titles or versions Fleet returned | [4.4](../04-know-your-devices/4.4-understand-software-and-vulnerabilities.md) |
| **`get mdm-apple`** (alias `mdm_apple`) Print the Apple push certificate's status | `mdm_apple · read` (global) | Read-only, key-and-value table only. `sync` | The certificate's status, or an advisory that none is configured, which also exits zero | [2.7](../02-administer-and-deploy-fleet/2.7-apple-mdm-configuration.md) |
| **`get mdm-ab`** (alias `mdm_ab`) Print the Apple Business token's status | `mdm_apple · read` (global). **Free:** a licence error without evaluating permission | Read-only, key-and-value table only. `sync` | The token's status, or an advisory that none is configured | [2.7](../02-administer-and-deploy-fleet/2.7-apple-mdm-configuration.md) |
| **`get mdm-apple-bm`** (alias `mdm_apple_bm`) **Deprecated.** The former name of `get mdm-ab`, and a separately registered command with its own deprecation notice | As `get mdm-ab`, including the Free behaviour | Read-only. `sync` | As `get mdm-ab` | [2.7](../02-administer-and-deploy-fleet/2.7-apple-mdm-configuration.md) |
| **`get mdm-command-results --id`** Print the per-host results of one MDM command | Three decisions: a tolerated `app_config · read` (global), `host · list` (global), then `mdm_command · read` (fleet) **once per fleet the results touch**. **A refusal here ends the command**, which is the difference from the row below | Read-only, text blocks only. `sync` | The results Fleet returned, or an advisory that none have arrived yet, which also exits zero | [8.8](../08-troubleshooting/8.8-apple-mdm-diagnostics.md) |
| **`get mdm-commands --host`** List recent MDM commands for a host | The same three decisions, but **a refusal is not fatal**: the commands you may not read are filtered out and the call still succeeds | Read-only, table output only, and a fixed page of the 20 most recent. `sync` | The commands Fleet returned **after removing any you were refused**, with the count shown taken before the removal. Z33 | [8.8](../08-troubleshooting/8.8-apple-mdm-diagnostics.md) |
...
| Command and purpose | Access contract | Effect | Result contract | Chapter |
|---|---|---|---|---|
| **`user create`** Create an account | `user · write`, scoped by the fleets named in the request. **Permission is charged before the licence check**, so a Premium role on Free is refused for the licence after passing the permission decision | Writes an account. `--api-only` prints a permanent API token to standard output. The default role is global observer. `sync` | The account exists | [2.3](../02-administer-and-deploy-fleet/2.3-user-accounts-roles-and-service-identities.md) |
| **`user delete --email`** Delete one account | Two decisions: `user · read` (global) to translate the address into an identifier, then `user · write` on the loaded account, **carrying that account's fleets** | **Irreversible. No confirmation.** `sync` | The account is gone | [2.3](../02-administer-and-deploy-fleet/2.3-user-accounts-roles-and-service-identities.md) |
| **`user create-users --csv`** Create accounts in bulk | The `user create` chain, once per row of the file | Writes accounts, and **prints every generated password to standard output**. `sync` | Every row was created | [2.3](../02-administer-and-deploy-fleet/2.3-user-accounts-roles-and-service-identities.md) |
| **`user delete-users --csv`** Delete accounts in bulk | The `user delete` two-decision chain, once per row | **Irreversible, bulk, no confirmation and no dry run.** It stops at the first failure part-way through, leaving a partial deletion. `sync` | The rows processed before any failure are gone | [2.3](../02-administer-and-deploy-fleet/2.3-user-accounts-roles-and-service-identities.md) |

### `fleetctl debug`, twelve server diagnostics

**Eleven need a global administrator token**, and that is decided by the diagnostic routes' own middleware rather than by the authorization policy, so [a.4](a.4-roles-and-permissions-matrix.md)'s matrix does not reach them. `debug connection` is the exception and needs no credential at all. All twelve write files with owner-only permissions where they write files. Every one is owned by [8.5](../08-troubleshooting/8.5-fleetctl-debug.md).
...
| **`debug cmdline`** Print the server's own command line | `global administrator` | Read-only, and **the highest-risk single item in the archive below**. `sync` | The server's arguments |
...
| **`updates add`** Publish an artifact for a platform and channel | `local` | **Publishes something your hosts will download and install.** The copy it makes is written without truncating an existing file, and the artifact is hashed after the copy, so a stale tail is signed rather than caught. `local` | An artifact was added to the repository. Z31 | None |
| **`updates timestamp`** Re-sign the repository's timestamp | `local` | Re-signs. `local` | The timestamp was refreshed | None |
| **`updates rotate <role>`** Replace the signing key for a role | `local` | **Retires a signing key.** `local` | The repository was committed. **It does not prove the old private keys were removed**: a cleanup failure leaves them in a backup directory and reports one warning line. Z27 | None |

### `fleetctl hosts`, one subcommand

| Command and purpose | Access contract | Effect | Result contract | Chapter |
|---|---|---|---|---|
| **`hosts transfer --fleet <name>`** Move hosts into a fleet | **Variant-dependent, four decisions either way.** With `--hosts`: `host · read` (global) once per host, `team · read` (global) for the destination, `host · transfer_host` on the destination fleet, then `host · transfer_host` on **each distinct source fleet**. With `--label`: `label · read` (global) in place of the per-host reads, and the same three that follow. `--status` or `--search_query` without `--label` skips the first decision entirely. Identical on both editions | **Destructive in the sense that matters: it moves configuration scope.** The filter form moves whatever the filter selects, server-side, and **the client never counts or prints how many hosts it is about to move**. `--fleet ''` means Unassigned. `sync` | Fleet accepted the transfer. **No count is available in the protocol**, so the number moved is not knowable from the command | [1.3](../01-foundations/1.3-hosts-fleets-labels.md) |

### `fleetctl generate`, three credential-request subcommands

**Each is a global write followed by a read**, which is the clearest case in the client for reading a chain in order rather than naming its last decision: a contract naming only the last would report these as a settings read when they require a global Apple MDM write.

| Command and purpose | Access contract | Effect | Result contract | Chapter |
|---|---|---|---|---|
| **`generate mdm-apple`** (alias `mdm_apple`) Request the signing request for an Apple push certificate | `mdm_apple · write` (global) then `app_config · read` (global) | Writes a request file with owner-only permissions. `sync` | Apple's service accepted the request and a file was written | [2.7](../02-administer-and-deploy-fleet/2.7-apple-mdm-configuration.md) |
| **`generate mdm-ab`** (alias `mdm_ab`) Request the public key for an Apple Business token | `mdm_apple · write` (global) then `app_config · read` (global) | Writes a key file. `sync` | A key file was written | [2.7](../02-administer-and-deploy-fleet/2.7-apple-mdm-configuration.md) |
| **`generate mdm-apple-bm`** (alias `mdm_apple_bm`) **Deprecated.** The former name of `generate mdm-ab` | Identical to `generate mdm-ab` | As above. `sync` | As above | [2.7](../02-administer-and-deploy-fleet/2.7-apple-mdm-configuration.md) |

### `fleetctl mdm`, five device-action subcommands

**This is the most destructive family in the client, and the family where the edition split bites hardest.** Four of the five share a client preflight that resolves the host and turns a permission refusal into a readable message; that preflight contributes the first two decisions of each chain.
...
| **`mdm lock --host`** Lock a device | `host · selective_list` (global), `host · selective_read` (fleet), `host · list` (global, broad), `mdm_command · write` (the host's fleet). **Free: no permission is evaluated at all and the command returns a licence error**, so a refusal there tells you nothing about your permissions | **Destructive.** Recovery needs the PIN Fleet issues, or the end user's own PIN on Android. `accepted` | Fleet accepted the lock. The device acts when it next checks in | [5.7](../05-manage-devices/5.7-control-devices-and-send-mdm-commands.md) |
| **`mdm unlock --host`** Unlock a device | The same four decisions. **Free: no permission is evaluated; licence error** | Reverses a lock, and prints the six-digit PIN for a Mac. `accepted` | Fleet accepted the unlock | [5.7](../05-manage-devices/5.7-control-devices-and-send-mdm-commands.md) |
...
**The community build evaluates no permission at all and returns a licence error.** Ten rows. **The Free chain has no decisions at that point, not fewer decisions**, which is a different kind of answer: a refusal on Free tells the operator nothing about what they are permitted to do, and moving to Premium can then refuse them again on grounds they have not yet seen.

**The decisions are the same on both editions and a licence check is added.** Seven rows. **Where that check sits is the useful part**, and it differs: `mdm wipe` takes every permission decision and then checks the licence, while `run-script` against a named fleet checks the licence near the end of the server's work but behind several authenticated client calls that can refuse you first. `user create` charges the permission before the licence refusal, so a Premium-only role requested on Free is refused for the licence rather than for permission.

| Row | Kind | What differs |
|---|---|---|
| `mdm lock` | No decisions on Free | Premium adds a broad global `host · list` and `mdm_command · write` |
| `mdm unlock` | No decisions on Free | As above |
| `mdm clear-passcode` | No decisions on Free | Premium adds a broad global `host · read` and `mdm_command · write` |
| `get fleets` | No decisions on Free | The whole structured continuation is unreachable on Free |
| `get mdm-ab` | No decisions on Free | |
| `get mdm-apple-bm` | No decisions on Free | A separately registered command, so a separate row |
| `get reports --fleet <id>` | No decisions on Free | The fleet read is the split; the report reads that follow are not |
| `gitops` | No decisions on Free, at many nodes | Fleet listing, fleet specs, fleet deletion, Apple Business tokens, installers, setup experience |
| `generate-gitops` | No decisions on Free, at many nodes | Fleet reads, certificate authorities, the licence agreement, App Store tokens, maintained apps, setup experience, icons |
| `apply -f` and `gitops` fleet software and App Store apps | No decisions on Free | Premium takes a fleet read then an `installable_entity · write` for each |
| `apply -f` | **Both kinds** | The fleet kind takes the first shape; label-scoped reports, policies and label specs take the second |
...
| `run-script` | Licence check added | Only where `--fleet` names a fleet, and the check is late rather than first |
| `get labels --fleet <id>` | Licence check added | |
| `get software --fleet <id>` | Licence check added | |
...
**Five rows were checked for a split and do not have one**, recorded so the question is not reopened: `hosts transfer`, `trigger`, `generate mdm-ab`, `delete -f`, and both `report` and `goquery`. On Free, `hosts transfer` can still fail, and it fails because the destination fleet does not exist rather than because a permission decision refused it.

### Sixteen commands this manual does not explain

**Sixteen of the 69 rows have no owning chapter, and this appendix names them rather than inventing a link.** A chapter owns a row when it explains the workflow the command serves, not when it mentions the command in passing. Six groups, and each is a gap in the book rather than in Fleet:
...
| **Running your own update repository** | `updates init`, `updates roots`, `updates add`, `updates timestamp`, `updates rotate` | [3.7](../03-connect-devices/3.7-manage-fleetd-orbit-and-updates.md) teaches consuming Fleet's update repository. Creating, signing, key-rotating and re-timestamping your own is not covered anywhere, and [6.4](../06-automate-fleet/6.4-use-fleetctl.md) says so plainly |
...
| **First-run setup and the interactive shell** | `setup`, `goquery` | Neither appears anywhere in the manual, and no chapter covers standing a server up or creating its first administrator |
...
**Read those rows as complete contracts and incomplete guidance.** The access, effect and result columns are verified for all sixteen exactly as they are for the other 53; what is missing is a chapter explaining when you would want the command and what to do around it.

## The exit-zero register

![Troubleshooting](../_assets/icons/troubleshooting.svg) **Thirty-three invocations where Fleet or the client detected an adverse, incomplete or refused outcome and the command still exited zero.** This is the section to read before you run `fleetctl` unattended, because every row is a case where a pipeline that branches on the exit code branches the wrong way.
...
**Every row below exits zero**, so there is no status column. Two rows are worth reading first even if you read nothing else: **Z8, the only row in the register where a command prints its success line for work that did not happen**, and **Z20, the only one that prints nothing at all**.
...
| **Z20** | `delete -f` on a file holding any kind other than report, pack or label | **Nothing was deleted for those kinds** | **Nothing.** No message, no warning | Whether the objects still exist. This is the only row in the register that is wholly silent |
...
| **Z22** | `debug archive` when writing the archive fails as it closes | **A truncated, unreadable archive**, after the path has already been printed | The archive path | That the archive opens |
| **Z23** | `get carve --outfile` | **A short or corrupt carve file.** The failure to close is discarded, and the file is opened without being truncated first, so a shorter carve leaves the previous file's tail attached | **Nothing** | The file's size against the carve's recorded size, and write to a fresh path each time |
...
| **Z25** | `api` with a field read from a file that cannot be opened or read | **The field is sent as the literal text you typed**, not the file's contents | A warning saying so | The request Fleet received. A 2xx response to the wrong body still exits zero |
...
| **Z27** | `updates rotate` where the cleanup after a successful rotation fails | **The pre-rotation private keys are still on disk.** The repository itself is correctly committed | "Warning: failure during commit" | The backup directory under the repository's key path, and remove it by hand. The point of a rotation is that the old key is gone |
| **Z28** | `convert -o <file>` when writing the output fails as it closes | **A truncated YAML file** | **Nothing** | That the output parses |
...
| **Z31** | `updates add` when the copy of the artifact fails as it closes | **A truncated artifact is hashed and signed as it stands**, then published to hosts | **Nothing** | The published artifact's size, before hosts install it |
...
| **Z33** | `get mdm-commands --host` where you may not read the commands of one of the fleets involved | **The commands you were refused are removed from the result and the call still succeeds.** If they were all refused, you are told none have been run | A short table with no marker, and **a count taken before the removal**, so the count and the rows disagree | The same question as a global administrator, or the command results by identifier, which refuses out loud instead |

### The shape of the register, which is the useful part

**Nine of the thirty-three are one command.** `generate-gitops` refuses in nine distinct ways and exits zero every time, including on an authorization refusal. It is hidden, which softens the consequence, and it is the command whose entire output is a repository you are about to commit.
...
**Three are a file written correctly and then closed badly**, in `debug archive`, `get carve` and `convert`, plus a fourth of the same shape in `updates add`. The failure is discarded in each case, so the only evidence is the file itself.
...
> ### One path has no exit status at all
>
...
>
> **Always pass `--timeout` when the command runs unattended.** It is the one invocation in the client where the answer to "what does the exit status prove" is that there is no exit status.

### Why the register is a floor

**Three places a thirty-fourth case could hide**, each stated so a later pass knows where to look. A failure detected inside a helper that returns success to a caller which then prints nothing leaves no trace at either end. The two most consequential rows here, `trigger` and `mdm run-command`, were found by reading Fleet's responses against what the client does with them, which is work no search performs. And the client library paths were audited where the commands reach them rather than in full.
...
| **`hosts transfer --status online --fleet X`** | The largest selection in the client that nothing bounds. `--status online` moves every online host in the deployment. **The client prints no count, asks nothing, and the protocol carries no count to print**, so nothing records the prior assignment either |
...
| **`gitops --delete-other-fleets`** | Deletes every fleet not named in the run. A dry run exists and covers it, **and the flag defaults to on in the pipeline scaffold `fleetctl new` generates** ([6.2](../06-automate-fleet/6.2-manage-fleet-with-gitops.md)) |
...
|---|---|---|
| `--delete-other-fleets` (alias `--delete-other-teams`) | `gitops` | Deletes **every fleet not named in this run**. It refuses only for fleets bound to an Apple Business or App Store token. Premium only, and **on by default in the generated pipeline scaffold** |
...
|---|---|---|
| `--status`, `--label`, `--search_query` | `hosts transfer` | A server-side filter. `--status online` moves every online host in the deployment into the named fleet, and no count is printed at any point |
...
|---|---|
| `--tls-skip-verify`, or `INSECURE` in the environment, on `config set` | Certificate verification is off **for every later command using that context**, because the setting is persisted |
| `--insecure` on `package` | Verification is off on **every host installed from that package**, and changing it means rebuilding and reinstalling |
...
**The client reads from the terminal in eleven places, and every one of them is a credential or a name**: the password on `login`, the password and its confirmation on `setup`, the password and confirmation and the token reveal in `user create`, the organisation name in `new`, and the passphrase and confirmation in the update-repository commands.

**None of those is a confirmation before a destructive act.** `mdm wipe`, `hosts transfer`, `user delete-users`, `gitops --delete-other-fleets` and `api` with a delete verb take no prompt, offer no `--yes`, and in three of the five have no dry run either.

**The bound on that claim, stated rather than hidden.** The eleven were enumerated from the mechanisms the client can prompt with. A prompt built on something outside that set would not have been found, so **whether any destructive command confirms by some other means is not established**. What is established is that none of the files holding those five invocations reads from the terminal at all.

## Choosing packaging options

![How-to](../_assets/icons/howto.svg) `fleetctl package` is the one command in this appendix that never contacts Fleet, and the one whose mistakes are hardest to undo, because the result is installed on hosts. [3.1](../03-connect-devices/3.1-enrollment-design-and-host-lifecycle.md), [3.4](../03-connect-devices/3.4-enroll-linux-devices.md) and [3.7](../03-connect-devices/3.7-manage-fleetd-orbit-and-updates.md) defer the option choice here rather than each carrying a flag list; run `fleetctl package --help` for the list itself, which is current for your client.
...
**Whether certificate verification is on.** `--insecure` bakes verification-off into the package, so **every host installed from it skips verification for as long as it is installed**. There is no server-side setting that turns it back on, and no agent upgrade that fixes it. Rebuild and reinstall is the whole remedy.
...
**And `delete -f` prints none of the spec-file notices that `apply -f` prints for the same file.** The same YAML fed to the two commands that both accept it warns in one and is silent in the other, so a `delete -f` run against a legacy file looks clean whatever it contains.
