...
| **`setup`** Create the first administrator on a server that has none | `unauthenticated` | Writes the local configuration file. `sync` | The server was un-set-up, is now set up, and your context file holds the new credential | [a.9](a.9-hands-on-labs.md#lab-2-production-bootstrap-at-reading-level) |
...
| Command and purpose | Access contract | Effect | Result contract | Chapter |
|---|---|---|---|---|
| **`preview stop`** Stop the sandbox | `local` | Stops the containers and the sandbox agent. `local` | The stop was issued | [a.9](a.9-hands-on-labs.md#lab-1-preview-as-a-sandbox) |
| **`preview reset`** Delete the sandbox | `local` | **Destroys the sandbox's data**, its containers and its agent directory. `local` | The sandbox was removed | [a.9](a.9-hands-on-labs.md#lab-1-preview-as-a-sandbox) |
...
### Every command in the index now has an owning chapter

**This register was the macOS and Linux rows with no owning chapter, and this revision emptied it.** A chapter owns a row when it explains the workflow the command serves, not when it mentions the command in passing, and equally not when it explains the workflow without naming the command. The four rows that once had no owner now have one:

| Group | Commands | Where it is now taught |
|---|---|---|
| **The local evaluation sandbox** | `preview`, `preview stop`, `preview reset` | [a.9](a.9-hands-on-labs.md#lab-1-preview-as-a-sandbox) Lab 1 starts the sandbox, runs reports against it, then stops and resets it |
| **First-run setup** | `setup` | [a.9](a.9-hands-on-labs.md#lab-2-production-bootstrap-at-reading-level) Lab 2 walks standing a server up and creating its first administrator |

**Read every row as a complete contract with an owning chapter.** The access, effect and result columns are verified for all 70 rows exactly as before; each now also names a section explaining when you would want the command and what to do around it. **The Windows-only `updates` row is owned too**, by [3.8](../03-connect-devices/3.8-manage-fleetd-orbit-and-updates.md#running-your-own-update-repository), which teaches the update-repository workflow and records that the family refuses on Windows and sends you to a Linux environment.

**The outcome-level sibling of this register is [a.1](a.1-capability-index.md#where-this-index-ends)'s closing section, "Where this index ends".** The local evaluation sandbox that both registers once carried is now taught in [a.9](a.9-hands-on-labs.md#lab-1-preview-as-a-sandbox) Lab 1, so it is a gap in neither; first-run `setup` was this register's alone and Lab 2 now owns it. The two registers still describe overlapping surfaces at different grain, so quote either on its own and never their sum.
