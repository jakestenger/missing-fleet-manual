...
A third pass swept every `Unsupported` cell on the two platforms where that error concentrates, and moved three more where the only finding was that Fleet accepts the request and the device never asks for it. A fourth pass found that sweep had kept a group of Chromebook report cells on the strength of a refusal belonging to a different operation: it governs which platform you may filter by when listing saved reports, and says nothing about creating a report or delivering one, while Fleet's write path accepts ChromeOS as a report platform outright. Those cells moved as well.
