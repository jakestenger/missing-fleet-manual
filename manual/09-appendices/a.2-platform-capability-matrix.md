...
| **CAP-349** | Let an end user see their own device's details and software | Supported | Supported | Supported | Supported | Supported | Supported | Free | An address for the device. The end-user surface issues one where it runs, an administrator can mint one on every platform except iPhone and iPad, and those two authenticate by certificate or by their own issued address instead |
...
| **CAP-095** | Collect a question's answer on a schedule | Supported | Not applicable | Supported | Supported | Not applicable | Not established (E24) | Free | An interval above zero, and results not discarded |
| **CAP-096** | Keep the newest result per host in Fleet | Supported | Not applicable | Supported | Supported | Not applicable | Not established (E25) | Free | Results not discarded, and snapshot logging |
| **CAP-097** | Send a report's results to a log destination | Supported | Not applicable | Supported | Supported | Not applicable | Not established (E26) | Free | A destination chosen at server start |
| **CAP-098** | Read a report's results across the estate | Supported | Not applicable | Supported | Supported | Not applicable | Not established (E27) | Free | Results not discarded |
| **CAP-099** | Read one host's result, including a successful empty one | Supported | Not applicable | Supported | Supported | Not applicable | Not established (E28) | Free | None |
| **CAP-100** | Retrieve stored report rows for export | Supported | Not applicable | Supported | Supported | Not applicable | Not established (E29) | Free | Results not discarded |
| **CAP-101** | Keep a report away from platforms whose tables do not exist | Supported | Not applicable | Supported | Supported | Not applicable | Not established (E30) | Free | None |
...
| **CAP-353** | Let an end user uninstall their own software | Conditional (C041) | Unsupported | Supported | Supported | Unsupported | Unsupported | Premium | The title offered as self-service, and the end-user surface installed. Authenticated by the device's own address, not by a Fleet account |
...
| **CAP-180** | Hold a catalogue app at a version | Supported | Not established (E36) | Supported | Not established (E37) | Not established (E38) | Not established (E39) | Premium | A cached version must exist |
...
| **CAP-359** | Choose whether a Play application is offered as self-service | Not applicable | Not applicable | Not applicable | Not applicable | Unsupported | Not applicable | Premium | Android Enterprise bound, and the application already in the library |
...
| **CAP-354** | Lock a Chromebook | Not applicable | Not applicable | Not applicable | Not applicable | Not applicable | Unsupported | Not applicable | Not applicable |
...
| **CAP-355** | Release a locked Chromebook | Not applicable | Not applicable | Not applicable | Not applicable | Not applicable | Unsupported | Not applicable | Not applicable |
...
| **CAP-356** | Erase a Chromebook | Not applicable | Not applicable | Not applicable | Not applicable | Not applicable | Unsupported | Not applicable | Not applicable |
| **CAP-357** | Turn Fleet's device management off for one host | Supported | Supported | Unsupported | Unsupported | Supported | Unsupported | Free | The host must currently be enrolled in Fleet's management for its platform |
...
| **CAP-358** | Rotate the FileVault recovery key Fleet holds for a Mac | Conditional (C106) | Not applicable | Not applicable | Not applicable | Not applicable | Not applicable | No direct gate, Premium-only prerequisite | Apple MDM configured, the host connected to it and agent-enrolled, and disk encryption enforced for its scope |
...
| **CAP-257** | Send osquery status and result logs to a destination | Supported | Not applicable | Supported | Supported | Not applicable | Not established (E22) | Free | Agent installed |
...
| **CAP-295** | Send scheduled-report results somewhere | Supported | Not applicable | Supported | Supported | Not applicable | Not established (E31) | Free | A destination chosen at server start |
| **CAP-296** | Send osquery's own status messages somewhere | Supported | Not applicable | Supported | Supported | Not applicable | Not established (E23) | Free | A destination chosen at server start |
...
40 cells where no boundary was found in either direction. Each record says what was searched, which is the part a later pass needs.
...
**E22** CAP-257, ChromeOS. Whether osquery status and result logs from a Chromebook reach a log destination. The destinations are built when the server starts, with no platform predicate anywhere on the path, and the write endpoint would accept a submission from any enrolled host. The extension never calls it: it enrolls, it reads live-query work and it writes live-query results, and those three are the whole of its traffic. That is acceptance on one side and absence on the other, which is not a boundary. Searched: the construction of the status and result destinations and their licence and platform handling; the log-write endpoint and its authorisation; the extension's whole source for every request it makes; and ChromeOS crossed with logging and with each destination's own name across the server and the extension. No rejection, no error arm and no allow-list was found in either direction.

**E23** CAP-296, ChromeOS. Whether osquery's own status messages from a Chromebook reach a destination. The same question as the row above and the same evidence: a destination chosen at server start with no platform predicate, an endpoint that would accept a Chromebook's submission, and an extension that never calls it. Searched as for the row above. No boundary was found in either direction.

**E24** CAP-095, ChromeOS. Whether a question's answer can be collected from a Chromebook on a schedule. Fleet's write path accepts ChromeOS: the check that validates the platform list on a saved report names ChromeOS alongside macOS, Windows and Linux and lets it through. The refusal previously read as a boundary here belongs to a different operation, the one that lists saved reports for an administrator, where a platform supplied as a filter in the address must be one of the three targetable platforms, and the message immediately above it refuses more than one platform at a time. That operation neither creates a report nor delivers one. The path that assembles a host's configuration packages its scheduled reports with no ChromeOS rejection anywhere on it. The extension never asks: it enrolls, it reads live-query work and it writes live-query results, and those three are the whole of its traffic. That is acceptance on one side and absence on the other, which is not a boundary. Searched: the report write validator and its platform list; the list operation and its filter validation, which is where the refusal actually lives; the configuration path that assembles a host's scheduled reports; the extension's whole source for every request it makes; and ChromeOS crossed with schedule, report and interval across the server and the extension. No rejection, no error arm and no allow-list was found in either direction.

**E25** CAP-096, ChromeOS. Whether the newest result per host is kept for a Chromebook. Follows the scheduled-report question above, on the same evidence and for the same reason: the write path accepts ChromeOS as a report platform, the refusal that was read as a boundary guards the operation that lists saved reports rather than this one, and the extension never fetches a schedule or submits a scheduled result. Searched as for the row above, and again for result storage and snapshot logging crossed with ChromeOS across the server and the extension, with no hit in either direction.

**E26** CAP-097, ChromeOS. Whether a report's results from a Chromebook reach a log destination. Follows the scheduled-report question above, on the same evidence and for the same reason. Searched as for the rows above, and again for the result destinations and their construction crossed with ChromeOS across the server and the extension, with no hit in either direction.

**E27** CAP-098, ChromeOS. Whether a report's results can be read across the estate for Chromebooks. Follows the scheduled-report question above, on the same evidence and for the same reason: nothing can be read that was never collected, and whether it is collected is unsettled. Searched as for the rows above, and again for the estate-wide result read and its filters crossed with ChromeOS, with no hit in either direction.

**E28** CAP-099, ChromeOS. Whether one Chromebook's result, including a successful empty one, can be read. Follows the scheduled-report question above, on the same evidence and for the same reason. Searched as for the rows above, and again for the per-host result read and its handling of an empty result crossed with ChromeOS, with no hit in either direction.

**E29** CAP-100, ChromeOS. Whether stored report rows for a Chromebook can be retrieved for export. Follows the scheduled-report question above, on the same evidence and for the same reason. Searched as for the rows above, and again for the row retrieval and export paths crossed with ChromeOS, with no hit in either direction.

**E30** CAP-101, ChromeOS. Whether scoping a report to ChromeOS has any effect on a Chromebook. A ChromeOS scope is accepted, stored and shown, and the check that accepts it is the same one that accepts ChromeOS as a report platform at all. Whether the scope then does anything turns on whether a Chromebook ever runs a scheduled report, which is the question the rows above leave open, so this cannot be settled either. The condition record this cell used to carry said the scope never runs; that rested on the extension not fetching a schedule, which is absence rather than a boundary. Searched as for the rows above, and again for the stored scope and every use of it when a host's reports are assembled. No rejection was found in either direction.

**E31** CAP-295, ChromeOS. Whether scheduled-report results from a Chromebook reach the destination chosen at server start. Follows the scheduled-report question above, on the same evidence and for the same reason: a destination built at start with no platform predicate, an endpoint that would accept a Chromebook's submission, a write validator that accepts ChromeOS as a report platform, and an extension that never sends one. Searched as for the rows above, and again for each destination's own name crossed with ChromeOS across the server and the extension.
...
**E36** CAP-180, iOS/iPadOS. Whether a catalogue application can be held at a version for iPhone and iPad. The version to hold comes from the same catalogue as the rows above, so the same missing source boundary governs it: nothing restricts which platform an entry may name, nothing validates the platform on the way in, and nothing refuses a request for one platform rather than another. Searched as for the catalogue rows above, and again for the cached version and the pin crossed with each platform name across the server. No rejection, no error arm and no allow-list was found in either direction.

**E37** CAP-180, Linux. Follows the catalogue version question above, on the same evidence and for the same reason. Searched as for the rows above, with no hit in either direction.

**E38** CAP-180, Android. Follows the catalogue version question above, on the same evidence and for the same reason. Searched as for the rows above, with no hit in either direction.

**E39** CAP-180, ChromeOS. Follows the catalogue version question above, on the same evidence and for the same reason. Searched as for the rows above, with no hit in either direction.
...
**The count of `Unsupported` cells fell substantially between the two research passes**, and that is worth knowing when reading them. The first pass wrote `Unsupported` in places where it meant that it had not found a mechanism; the second reclassified those to `Not established` unless a positive boundary existed, and reclassified another group to `Not applicable` where the row's subject does not exist on the platform at all. A third pass swept every `Unsupported` cell on the two platforms where that error concentrates, and moved three more where the only finding was that Fleet accepts the request and the device never asks for it. A fourth pass found that sweep had kept a group of Chromebook report cells on the strength of a refusal belonging to a different operation: it governs which platform you may filter by when listing saved reports, and says nothing about creating a report or delivering one, while Fleet's write path accepts ChromeOS as a report platform outright. Those cells moved as well. **Checking which operation a refusal guards is part of the work rather than a refinement of it**, and it is the most expensive check to skip, because a sweep that skips it still reports itself as coverage.

**The catalogue rows no longer hedge.** They had been left `Unsupported` with a note admitting that the answer rested on Fleet describing its catalogue as covering two platforms rather than on anything enforcing it. A description is not a boundary, and the search that was owed has now been run: Fleet takes the catalogue from a list held outside the deployment, accepts whatever platform that list names, and applies no platform limit when it shows you what is in it. Those cells are `Not established` now, each with a record of what was searched.
