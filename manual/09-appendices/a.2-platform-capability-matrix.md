...
![Reference](../_assets/icons/reference.svg) Grouped as a reader would look for a capability, 271 rows. Section rows in bold carry no cells; they mark where a family starts.
...
| **CAP-091** | Ask a host to report again now | Supported | Supported | Supported | Supported | Not established (E21) | Supported | Free | Apple MDM configured and connected |
...
| **CAP-180** | Hold a catalogue app at a version | Supported | Unsupported | Supported | Unsupported | Unsupported | Unsupported | Premium | A cached version must exist |
...
**C105** CAP-189, Android. Supported only in the sense that the software Fleet is to install is supplied in the device's enrollment policy, so it arrives as the device enrolls and Fleet records how each item finished. Not supported as a hold before the person starts using the device, and not as the ordered step-by-step experience the row is named for: Android has no on-device setup screen and no ordering of installs, so nothing blocks the user and nothing reports progress on the device itself.
...
23 cells where no boundary was found in either direction. Each record says what was searched, which is the part a later pass needs.
