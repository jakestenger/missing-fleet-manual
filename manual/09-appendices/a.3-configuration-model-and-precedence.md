...
**It does not explain what every setting is for.** Fleet's own configuration reference does that, and copying that hand-maintained prose would be a liability rather than a service: this project has confirmed the reference and the server disagreeing in two different ways at this release, about what a default is and about whether a documented key is bound at all. What it does carry, generated from the server binary rather than copied, is a complete catalog of the keys the server binds and the defaults it registers. The disagreements that change a decision are called out first; the full catalog closes the appendix.
...
Read the columns as the key, the environment variable that sets it, the type, and the default the server registers. A key marked *(hidden)* is bound and functional but kept out of `fleet serve --help`. A default marked *(computed)* is derived when the server starts rather than being a fixed literal, so the cell shows the value it resolves to, which for a few paths is relative to a directory the operating system chooses at runtime. Every one of the 320 registered keys is listed here; none is omitted. The source location of each registration sits in an HTML comment on its row, for an editor checking the work, and not in the reader's way.

<!-- To regenerate: python3 build/gen-config-catalog.py --out FILE  (FLEET_SRC overrides the source checkout). Pinned to fleet-v4.90.1 (dd0200f062). -->
<!-- GENERATED by build/gen-config-catalog.py; do not edit by hand.
     source: server/config/config.go @ dd0200f062c5982c46dd3bf8de81a6b5c0c5ce6d
     registration calls found: 302; keys parsed: 320 (of which 5 computed defaults); unparsed: 0; duplicate keys: 0 -->

| Key | Environment variable | Type | Registered default |
|---|---|---|---|
| `mysql.protocol` | `FLEET_MYSQL_PROTOCOL` | string | `"tcp"` <!-- server/config/config.go:1301; via addMysqlConfig("mysql", "localhost:3306", ".") -->|
| `mysql.address` | `FLEET_MYSQL_ADDRESS` | string | `"localhost:3306"` <!-- server/config/config.go:1303; via addMysqlConfig("mysql", "localhost:3306", ".") -->|
| `mysql.username` | `FLEET_MYSQL_USERNAME` | string | `"fleet"` <!-- server/config/config.go:1305; via addMysqlConfig("mysql", "localhost:3306", ".") -->|
| `mysql.password` | `FLEET_MYSQL_PASSWORD` | string | `""` <!-- server/config/config.go:1307; via addMysqlConfig("mysql", "localhost:3306", ".") -->|
| `mysql.password_path` | `FLEET_MYSQL_PASSWORD_PATH` | string | `""` <!-- server/config/config.go:1309; via addMysqlConfig("mysql", "localhost:3306", ".") -->|
| `mysql.database` | `FLEET_MYSQL_DATABASE` | string | `"fleet"` <!-- server/config/config.go:1311; via addMysqlConfig("mysql", "localhost:3306", ".") -->|
| `mysql.tls_cert` | `FLEET_MYSQL_TLS_CERT` | string | `""` <!-- server/config/config.go:1313; via addMysqlConfig("mysql", "localhost:3306", ".") -->|
| `mysql.tls_key` | `FLEET_MYSQL_TLS_KEY` | string | `""` <!-- server/config/config.go:1315; via addMysqlConfig("mysql", "localhost:3306", ".") -->|
| `mysql.tls_ca` | `FLEET_MYSQL_TLS_CA` | string | `""` <!-- server/config/config.go:1317; via addMysqlConfig("mysql", "localhost:3306", ".") -->|
| `mysql.tls_server_name` | `FLEET_MYSQL_TLS_SERVER_NAME` | string | `""` <!-- server/config/config.go:1319; via addMysqlConfig("mysql", "localhost:3306", ".") -->|
| `mysql.tls_config` | `FLEET_MYSQL_TLS_CONFIG` | string | `""` <!-- server/config/config.go:1321; via addMysqlConfig("mysql", "localhost:3306", ".") -->|
| `mysql.max_open_conns` | `FLEET_MYSQL_MAX_OPEN_CONNS` | int | `50` <!-- server/config/config.go:1323; via addMysqlConfig("mysql", "localhost:3306", ".") -->|
| `mysql.max_idle_conns` | `FLEET_MYSQL_MAX_IDLE_CONNS` | int | `50` <!-- server/config/config.go:1324; via addMysqlConfig("mysql", "localhost:3306", ".") -->|
| `mysql.conn_max_lifetime` | `FLEET_MYSQL_CONN_MAX_LIFETIME` | int | `0` <!-- server/config/config.go:1325; via addMysqlConfig("mysql", "localhost:3306", ".") -->|
| `mysql.sql_mode` | `FLEET_MYSQL_SQL_MODE` | string | `""` <!-- server/config/config.go:1326; via addMysqlConfig("mysql", "localhost:3306", ".") -->|
| `mysql.region` | `FLEET_MYSQL_REGION` | string | `""` <!-- server/config/config.go:1327; via addMysqlConfig("mysql", "localhost:3306", ".") -->|
| `mysql.sts_assume_role_arn` | `FLEET_MYSQL_STS_ASSUME_ROLE_ARN` | string | `""` <!-- server/config/config.go:1328; via addMysqlConfig("mysql", "localhost:3306", ".") -->|
| `mysql.sts_external_id` | `FLEET_MYSQL_STS_EXTERNAL_ID` | string | `""` <!-- server/config/config.go:1329; via addMysqlConfig("mysql", "localhost:3306", ".") -->|
| `mysql_read_replica.protocol` | `FLEET_MYSQL_READ_REPLICA_PROTOCOL` | string | `"tcp"` <!-- server/config/config.go:1301; via addMysqlConfig("mysql_read_replica", "", " for the read replica.") -->|
| `mysql_read_replica.address` | `FLEET_MYSQL_READ_REPLICA_ADDRESS` | string | `""` <!-- server/config/config.go:1303; via addMysqlConfig("mysql_read_replica", "", " for the read replica.") -->|
| `mysql_read_replica.username` | `FLEET_MYSQL_READ_REPLICA_USERNAME` | string | `"fleet"` <!-- server/config/config.go:1305; via addMysqlConfig("mysql_read_replica", "", " for the read replica.") -->|
| `mysql_read_replica.password` | `FLEET_MYSQL_READ_REPLICA_PASSWORD` | string | `""` <!-- server/config/config.go:1307; via addMysqlConfig("mysql_read_replica", "", " for the read replica.") -->|
| `mysql_read_replica.password_path` | `FLEET_MYSQL_READ_REPLICA_PASSWORD_PATH` | string | `""` <!-- server/config/config.go:1309; via addMysqlConfig("mysql_read_replica", "", " for the read replica.") -->|
| `mysql_read_replica.database` | `FLEET_MYSQL_READ_REPLICA_DATABASE` | string | `"fleet"` <!-- server/config/config.go:1311; via addMysqlConfig("mysql_read_replica", "", " for the read replica.") -->|
| `mysql_read_replica.tls_cert` | `FLEET_MYSQL_READ_REPLICA_TLS_CERT` | string | `""` <!-- server/config/config.go:1313; via addMysqlConfig("mysql_read_replica", "", " for the read replica.") -->|
| `mysql_read_replica.tls_key` | `FLEET_MYSQL_READ_REPLICA_TLS_KEY` | string | `""` <!-- server/config/config.go:1315; via addMysqlConfig("mysql_read_replica", "", " for the read replica.") -->|
| `mysql_read_replica.tls_ca` | `FLEET_MYSQL_READ_REPLICA_TLS_CA` | string | `""` <!-- server/config/config.go:1317; via addMysqlConfig("mysql_read_replica", "", " for the read replica.") -->|
| `mysql_read_replica.tls_server_name` | `FLEET_MYSQL_READ_REPLICA_TLS_SERVER_NAME` | string | `""` <!-- server/config/config.go:1319; via addMysqlConfig("mysql_read_replica", "", " for the read replica.") -->|
| `mysql_read_replica.tls_config` | `FLEET_MYSQL_READ_REPLICA_TLS_CONFIG` | string | `""` <!-- server/config/config.go:1321; via addMysqlConfig("mysql_read_replica", "", " for the read replica.") -->|
| `mysql_read_replica.max_open_conns` | `FLEET_MYSQL_READ_REPLICA_MAX_OPEN_CONNS` | int | `50` <!-- server/config/config.go:1323; via addMysqlConfig("mysql_read_replica", "", " for the read replica.") -->|
| `mysql_read_replica.max_idle_conns` | `FLEET_MYSQL_READ_REPLICA_MAX_IDLE_CONNS` | int | `50` <!-- server/config/config.go:1324; via addMysqlConfig("mysql_read_replica", "", " for the read replica.") -->|
| `mysql_read_replica.conn_max_lifetime` | `FLEET_MYSQL_READ_REPLICA_CONN_MAX_LIFETIME` | int | `0` <!-- server/config/config.go:1325; via addMysqlConfig("mysql_read_replica", "", " for the read replica.") -->|
| `mysql_read_replica.sql_mode` | `FLEET_MYSQL_READ_REPLICA_SQL_MODE` | string | `""` <!-- server/config/config.go:1326; via addMysqlConfig("mysql_read_replica", "", " for the read replica.") -->|
| `mysql_read_replica.region` | `FLEET_MYSQL_READ_REPLICA_REGION` | string | `""` <!-- server/config/config.go:1327; via addMysqlConfig("mysql_read_replica", "", " for the read replica.") -->|
| `mysql_read_replica.sts_assume_role_arn` | `FLEET_MYSQL_READ_REPLICA_STS_ASSUME_ROLE_ARN` | string | `""` <!-- server/config/config.go:1328; via addMysqlConfig("mysql_read_replica", "", " for the read replica.") -->|
| `mysql_read_replica.sts_external_id` | `FLEET_MYSQL_READ_REPLICA_STS_EXTERNAL_ID` | string | `""` <!-- server/config/config.go:1329; via addMysqlConfig("mysql_read_replica", "", " for the read replica.") -->|
| `redis.address` | `FLEET_REDIS_ADDRESS` | string | `"localhost:6379"` <!-- server/config/config.go:1336 -->|
| `redis.username` | `FLEET_REDIS_USERNAME` | string | `""` <!-- server/config/config.go:1338 -->|
| `redis.password` | `FLEET_REDIS_PASSWORD` | string | `""` <!-- server/config/config.go:1340 -->|
| `redis.cache_name` | `FLEET_REDIS_CACHE_NAME` | string | `""` <!-- server/config/config.go:1342 -->|
| `redis.region` | `FLEET_REDIS_REGION` | string | `""` <!-- server/config/config.go:1344 -->|
| `redis.database` | `FLEET_REDIS_DATABASE` | int | `0` <!-- server/config/config.go:1346 -->|
| `redis.use_tls` | `FLEET_REDIS_USE_TLS` | bool | `false` <!-- server/config/config.go:1348 -->|
| `redis.duplicate_results` | `FLEET_REDIS_DUPLICATE_RESULTS` | bool | `false` <!-- server/config/config.go:1349 -->|
| `redis.connect_timeout` | `FLEET_REDIS_CONNECT_TIMEOUT` | duration | `5s` <!-- server/config/config.go:1350 -->|
| `redis.keep_alive` | `FLEET_REDIS_KEEP_ALIVE` | duration | `10s` <!-- server/config/config.go:1351 -->|
| `redis.connect_retry_attempts` | `FLEET_REDIS_CONNECT_RETRY_ATTEMPTS` | int | `0` <!-- server/config/config.go:1352 -->|
| `redis.cluster_follow_redirections` | `FLEET_REDIS_CLUSTER_FOLLOW_REDIRECTIONS` | bool | `true` <!-- server/config/config.go:1353 -->|
| `redis.cluster_read_from_replica` | `FLEET_REDIS_CLUSTER_READ_FROM_REPLICA` | bool | `false` <!-- server/config/config.go:1354 -->|
| `redis.tls_cert` | `FLEET_REDIS_TLS_CERT` | string | `""` <!-- server/config/config.go:1355 -->|
| `redis.tls_key` | `FLEET_REDIS_TLS_KEY` | string | `""` <!-- server/config/config.go:1356 -->|
| `redis.tls_ca` | `FLEET_REDIS_TLS_CA` | string | `""` <!-- server/config/config.go:1357 -->|
| `redis.tls_server_name` | `FLEET_REDIS_TLS_SERVER_NAME` | string | `""` <!-- server/config/config.go:1358 -->|
| `redis.tls_handshake_timeout` | `FLEET_REDIS_TLS_HANDSHAKE_TIMEOUT` | duration | `10s` <!-- server/config/config.go:1359 -->|
| `redis.max_idle_conns` | `FLEET_REDIS_MAX_IDLE_CONNS` | int | `3` <!-- server/config/config.go:1360 -->|
| `redis.max_open_conns` | `FLEET_REDIS_MAX_OPEN_CONNS` | int | `0` <!-- server/config/config.go:1361 -->|
| `redis.conn_max_lifetime` | `FLEET_REDIS_CONN_MAX_LIFETIME` | duration | `0s` <!-- server/config/config.go:1362 -->|
| `redis.idle_timeout` | `FLEET_REDIS_IDLE_TIMEOUT` | duration | `240s` <!-- server/config/config.go:1363 -->|
| `redis.conn_wait_timeout` | `FLEET_REDIS_CONN_WAIT_TIMEOUT` | duration | `0s` <!-- server/config/config.go:1364 -->|
| `redis.write_timeout` | `FLEET_REDIS_WRITE_TIMEOUT` | duration | `10s` <!-- server/config/config.go:1365 -->|
| `redis.read_timeout` | `FLEET_REDIS_READ_TIMEOUT` | duration | `10s` <!-- server/config/config.go:1366 -->|
| `redis.sts_assume_role_arn` | `FLEET_REDIS_STS_ASSUME_ROLE_ARN` | string | `""` <!-- server/config/config.go:1367 -->|
| `redis.sts_external_id` | `FLEET_REDIS_STS_EXTERNAL_ID` | string | `""` <!-- server/config/config.go:1368 -->|
| `redis.host_cache_enabled` | `FLEET_REDIS_HOST_CACHE_ENABLED` | bool | `true` <!-- server/config/config.go:1369 -->|
| `redis.host_cache_ttl` | `FLEET_REDIS_HOST_CACHE_TTL` | duration | `180s` <!-- server/config/config.go:1372 -->|
| `redis.live_query_small_target_threshold` | `FLEET_REDIS_LIVE_QUERY_SMALL_TARGET_THRESHOLD` | int | `1000` <!-- server/config/config.go:1376 -->|
| `server.address` | `FLEET_SERVER_ADDRESS` | string | `"0.0.0.0:8080"` <!-- server/config/config.go:1382 -->|
| `server.cert` | `FLEET_SERVER_CERT` | string | `"./tools/osquery/fleet.crt"` <!-- server/config/config.go:1384 -->|
| `server.key` | `FLEET_SERVER_KEY` | string | `"./tools/osquery/fleet.key"` <!-- server/config/config.go:1386 -->|
| `server.tls` | `FLEET_SERVER_TLS` | bool | `true` <!-- server/config/config.go:1388 -->|
| `server.tls_compatibility` | `FLEET_SERVER_TLS_COMPATIBILITY` | string | `"intermediate"` <!-- server/config/config.go:1390 -->|
| `server.url_prefix` | `FLEET_SERVER_URL_PREFIX` | string | `""` <!-- server/config/config.go:1393 -->|
| `server.keepalive` | `FLEET_SERVER_KEEPALIVE` | bool | `true` <!-- server/config/config.go:1395 -->|
| `server.sandbox_enabled` *(hidden)* | `FLEET_SERVER_SANDBOX_ENABLED` | bool | `false` <!-- server/config/config.go:1397 -->|
| `server.websockets_allow_unsafe_origin` | `FLEET_SERVER_WEBSOCKETS_ALLOW_UNSAFE_ORIGIN` | bool | `false` <!-- server/config/config.go:1399 -->|
| `server.frequent_cleanups_enabled` | `FLEET_SERVER_FREQUENT_CLEANUPS_ENABLED` | bool | `false` <!-- server/config/config.go:1400 -->|
| `server.force_h2c` | `FLEET_SERVER_FORCE_H2C` | bool | `false` <!-- server/config/config.go:1401 -->|
| `server.private_key` | `FLEET_SERVER_PRIVATE_KEY` | string | `""` <!-- server/config/config.go:1402 -->|
| `server.private_key_region` | `FLEET_SERVER_PRIVATE_KEY_REGION` | string | `""` <!-- server/config/config.go:1403 -->|
| `server.private_key_arn` | `FLEET_SERVER_PRIVATE_KEY_ARN` | string | `""` <!-- server/config/config.go:1404 -->|
| `server.private_key_sts_assume_role_arn` | `FLEET_SERVER_PRIVATE_KEY_STS_ASSUME_ROLE_ARN` | string | `""` <!-- server/config/config.go:1405 -->|
| `server.private_key_sts_external_id` | `FLEET_SERVER_PRIVATE_KEY_STS_EXTERNAL_ID` | string | `""` <!-- server/config/config.go:1406 -->|
| `server.vpp_verify_timeout` | `FLEET_SERVER_VPP_VERIFY_TIMEOUT` | duration | `10m` <!-- server/config/config.go:1407 -->|
| `server.vpp_verify_request_delay` | `FLEET_SERVER_VPP_VERIFY_REQUEST_DELAY` | duration | `5s` <!-- server/config/config.go:1408 -->|
| `server.cleanup_dist_targets_age` | `FLEET_SERVER_CLEANUP_DIST_TARGETS_AGE` | duration | `24h` <!-- server/config/config.go:1409 -->|
| `server.max_installer_size` | `FLEET_SERVER_MAX_INSTALLER_SIZE` | bytes | `10 GiB` *(computed)* <!-- server/config/config.go:1410; expr installersize.Human(installersize.MaxSoftwareInstallerSize) -->|
| `server.trusted_proxies` | `FLEET_SERVER_TRUSTED_PROXIES` | string | `""` <!-- server/config/config.go:1411 -->|
| `server.gzip_responses` | `FLEET_SERVER_GZIP_RESPONSES` | bool | `false` <!-- server/config/config.go:1413 -->|
| `server.allow_private_network_integrations` | `FLEET_SERVER_ALLOW_PRIVATE_NETWORK_INTEGRATIONS` | bool | `false` <!-- server/config/config.go:1414 -->|
| `server.bypass_network_blocking` | `FLEET_SERVER_BYPASS_NETWORK_BLOCKING` | bool | `false` <!-- server/config/config.go:1415 -->|
| `server.default_max_request_body_size` | `FLEET_SERVER_DEFAULT_MAX_REQUEST_BODY_SIZE` | bytes | `1 MiB` *(computed)* <!-- server/config/config.go:1416; expr installersize.Human(platform_http.MaxRequestBodySize) -->|
| `auth.bcrypt_cost` | `FLEET_AUTH_BCRYPT_COST` | int | `12` <!-- server/config/config.go:1422 -->|
| `auth.salt_key_size` | `FLEET_AUTH_SALT_KEY_SIZE` | int | `24` <!-- server/config/config.go:1424 -->|
| `auth.sso_session_validity_period` | `FLEET_AUTH_SSO_SESSION_VALIDITY_PERIOD` | duration | `15m` <!-- server/config/config.go:1426 -->|
| `auth.require_http_message_signature` | `FLEET_AUTH_REQUIRE_HTTP_MESSAGE_SIGNATURE` | bool | `false` <!-- server/config/config.go:1428 -->|
| `auth.sso_rate_limit_per_minute` | `FLEET_AUTH_SSO_RATE_LIMIT_PER_MINUTE` | int | `0` <!-- server/config/config.go:1430 -->|
| `app.token_key` | `FLEET_APP_TOKEN_KEY` | string | `"CHANGEME"` <!-- server/config/config.go:1434 -->|
| `app.invite_token_validity_period` | `FLEET_APP_INVITE_TOKEN_VALIDITY_PERIOD` | duration | `120h` <!-- server/config/config.go:1436 -->|
| `app.token_key_size` | `FLEET_APP_TOKEN_KEY_SIZE` | int | `24` <!-- server/config/config.go:1438 -->|
| `app.enable_scheduled_query_stats` | `FLEET_APP_ENABLE_SCHEDULED_QUERY_STATS` | bool | `true` <!-- server/config/config.go:1440 -->|
| `session.key_size` | `FLEET_SESSION_KEY_SIZE` | int | `64` <!-- server/config/config.go:1444 -->|
| `session.duration` | `FLEET_SESSION_DURATION` | duration | `120h` <!-- server/config/config.go:1446 -->|
| `osquery.node_key_size` | `FLEET_OSQUERY_NODE_KEY_SIZE` | int | `24` <!-- server/config/config.go:1450 -->|
| `osquery.host_identifier` | `FLEET_OSQUERY_HOST_IDENTIFIER` | string | `"provided"` <!-- server/config/config.go:1452 -->|
| `osquery.enroll_cooldown` | `FLEET_OSQUERY_ENROLL_COOLDOWN` | duration | `0s` <!-- server/config/config.go:1454 -->|
| `osquery.status_log_plugin` | `FLEET_OSQUERY_STATUS_LOG_PLUGIN` | string | `"filesystem"` <!-- server/config/config.go:1456 -->|
| `osquery.result_log_plugin` | `FLEET_OSQUERY_RESULT_LOG_PLUGIN` | string | `"filesystem"` <!-- server/config/config.go:1458 -->|
| `osquery.label_update_interval` | `FLEET_OSQUERY_LABEL_UPDATE_INTERVAL` | duration | `1h` <!-- server/config/config.go:1460 -->|
| `osquery.policy_update_interval` | `FLEET_OSQUERY_POLICY_UPDATE_INTERVAL` | duration | `1h` <!-- server/config/config.go:1462 -->|
| `osquery.detail_update_interval` | `FLEET_OSQUERY_DETAIL_UPDATE_INTERVAL` | duration | `1h` <!-- server/config/config.go:1464 -->|
| `osquery.status_log_file` | `FLEET_OSQUERY_STATUS_LOG_FILE` | string | `""` <!-- server/config/config.go:1466 -->|
| `osquery.result_log_file` | `FLEET_OSQUERY_RESULT_LOG_FILE` | string | `""` <!-- server/config/config.go:1468 -->|
| `osquery.enable_log_rotation` | `FLEET_OSQUERY_ENABLE_LOG_ROTATION` | bool | `false` <!-- server/config/config.go:1470 -->|
| `osquery.max_jitter_percent` | `FLEET_OSQUERY_MAX_JITTER_PERCENT` | int | `10` <!-- server/config/config.go:1472 -->|
| `osquery.enable_async_host_processing` | `FLEET_OSQUERY_ENABLE_ASYNC_HOST_PROCESSING` | string | `"false"` <!-- server/config/config.go:1474 -->|
| `osquery.async_host_collect_interval` | `FLEET_OSQUERY_ASYNC_HOST_COLLECT_INTERVAL` | string | `"30s"` <!-- server/config/config.go:1476 -->|
| `osquery.async_host_collect_max_jitter_percent` | `FLEET_OSQUERY_ASYNC_HOST_COLLECT_MAX_JITTER_PERCENT` | int | `10` <!-- server/config/config.go:1478 -->|
| `osquery.async_host_collect_lock_timeout` | `FLEET_OSQUERY_ASYNC_HOST_COLLECT_LOCK_TIMEOUT` | string | `"1m0s"` <!-- server/config/config.go:1480 -->|
| `osquery.async_host_collect_log_stats_interval` | `FLEET_OSQUERY_ASYNC_HOST_COLLECT_LOG_STATS_INTERVAL` | duration | `1m` <!-- server/config/config.go:1482 -->|
| `osquery.async_host_insert_batch` | `FLEET_OSQUERY_ASYNC_HOST_INSERT_BATCH` | int | `2000` <!-- server/config/config.go:1484 -->|
| `osquery.async_host_delete_batch` | `FLEET_OSQUERY_ASYNC_HOST_DELETE_BATCH` | int | `2000` <!-- server/config/config.go:1486 -->|
| `osquery.async_host_update_batch` | `FLEET_OSQUERY_ASYNC_HOST_UPDATE_BATCH` | int | `1000` <!-- server/config/config.go:1488 -->|
| `osquery.async_host_redis_pop_count` | `FLEET_OSQUERY_ASYNC_HOST_REDIS_POP_COUNT` | int | `1000` <!-- server/config/config.go:1490 -->|
| `osquery.async_host_redis_scan_keys_count` | `FLEET_OSQUERY_ASYNC_HOST_REDIS_SCAN_KEYS_COUNT` | int | `1000` <!-- server/config/config.go:1492 -->|
| `osquery.min_software_last_opened_at_diff` | `FLEET_OSQUERY_MIN_SOFTWARE_LAST_OPENED_AT_DIFF` | duration | `2m` <!-- server/config/config.go:1494 -->|
| `osquery.max_log_write_body_size` | `FLEET_OSQUERY_MAX_LOG_WRITE_BODY_SIZE` | bytes | `"0"` <!-- server/config/config.go:1496 -->|
| `osquery.max_distributed_write_body_size` | `FLEET_OSQUERY_MAX_DISTRIBUTED_WRITE_BODY_SIZE` | bytes | `"0"` <!-- server/config/config.go:1498 -->|
| `osquery.allow_body_auth_fallback` | `FLEET_OSQUERY_ALLOW_BODY_AUTH_FALLBACK` | bool | `true` <!-- server/config/config.go:1500 -->|
| `activity.enable_audit_log` | `FLEET_ACTIVITY_ENABLE_AUDIT_LOG` | bool | `false` <!-- server/config/config.go:1504 -->|
| `activity.audit_log_plugin` | `FLEET_ACTIVITY_AUDIT_LOG_PLUGIN` | string | `"filesystem"` <!-- server/config/config.go:1506 -->|
| `logging.debug` | `FLEET_LOGGING_DEBUG` | bool | `false` <!-- server/config/config.go:1510 -->|
| `logging.json` | `FLEET_LOGGING_JSON` | bool | `false` <!-- server/config/config.go:1512 -->|
| `logging.disable_banner` | `FLEET_LOGGING_DISABLE_BANNER` | bool | `false` <!-- server/config/config.go:1514 -->|
| `logging.error_retention_period` | `FLEET_LOGGING_ERROR_RETENTION_PERIOD` | duration | `24h` <!-- server/config/config.go:1516 -->|
| `logging.tracing_enabled` | `FLEET_LOGGING_TRACING_ENABLED` | bool | `false` <!-- server/config/config.go:1518 -->|
| `logging.tracing_type` | `FLEET_LOGGING_TRACING_TYPE` | string | `""` <!-- server/config/config.go:1520 -->|
| `logging.otel_logs_enabled` | `FLEET_LOGGING_OTEL_LOGS_ENABLED` | bool | `false` <!-- server/config/config.go:1522 -->|
| `logging.enable_topics` | `FLEET_LOGGING_ENABLE_TOPICS` | string | `""` <!-- server/config/config.go:1524 -->|
| `logging.disable_topics` | `FLEET_LOGGING_DISABLE_TOPICS` | string | `""` <!-- server/config/config.go:1526 -->|
| `email.backend` | `FLEET_EMAIL_BACKEND` | string | `""` <!-- server/config/config.go:1530 -->|
| `ses.region` | `FLEET_SES_REGION` | string | `""` <!-- server/config/config.go:1533 -->|
| `ses.endpoint_url` | `FLEET_SES_ENDPOINT_URL` | string | `""` <!-- server/config/config.go:1534 -->|
| `ses.access_key_id` | `FLEET_SES_ACCESS_KEY_ID` | string | `""` <!-- server/config/config.go:1535 -->|
| `ses.secret_access_key` | `FLEET_SES_SECRET_ACCESS_KEY` | string | `""` <!-- server/config/config.go:1536 -->|
| `ses.sts_assume_role_arn` | `FLEET_SES_STS_ASSUME_ROLE_ARN` | string | `""` <!-- server/config/config.go:1537 -->|
| `ses.sts_external_id` | `FLEET_SES_STS_EXTERNAL_ID` | string | `""` <!-- server/config/config.go:1538 -->|
| `ses.source_arn` | `FLEET_SES_SOURCE_ARN` | string | `""` <!-- server/config/config.go:1539 -->|
| `ses.sender_domain` | `FLEET_SES_SENDER_DOMAIN` | string | `""` <!-- server/config/config.go:1540 -->|
| `firehose.region` | `FLEET_FIREHOSE_REGION` | string | `""` <!-- server/config/config.go:1543 -->|
| `firehose.endpoint_url` | `FLEET_FIREHOSE_ENDPOINT_URL` | string | `""` <!-- server/config/config.go:1544 -->|
| `firehose.access_key_id` | `FLEET_FIREHOSE_ACCESS_KEY_ID` | string | `""` <!-- server/config/config.go:1546 -->|
| `firehose.secret_access_key` | `FLEET_FIREHOSE_SECRET_ACCESS_KEY` | string | `""` <!-- server/config/config.go:1547 -->|
| `firehose.sts_assume_role_arn` | `FLEET_FIREHOSE_STS_ASSUME_ROLE_ARN` | string | `""` <!-- server/config/config.go:1548 -->|
| `firehose.sts_external_id` | `FLEET_FIREHOSE_STS_EXTERNAL_ID` | string | `""` <!-- server/config/config.go:1550 -->|
| `firehose.status_stream` | `FLEET_FIREHOSE_STATUS_STREAM` | string | `""` <!-- server/config/config.go:1552 -->|
| `firehose.result_stream` | `FLEET_FIREHOSE_RESULT_STREAM` | string | `""` <!-- server/config/config.go:1554 -->|
| `firehose.audit_stream` | `FLEET_FIREHOSE_AUDIT_STREAM` | string | `""` <!-- server/config/config.go:1556 -->|
| `kinesis.region` | `FLEET_KINESIS_REGION` | string | `""` <!-- server/config/config.go:1560 -->|
| `kinesis.endpoint_url` | `FLEET_KINESIS_ENDPOINT_URL` | string | `""` <!-- server/config/config.go:1561 -->|
| `kinesis.access_key_id` | `FLEET_KINESIS_ACCESS_KEY_ID` | string | `""` <!-- server/config/config.go:1563 -->|
| `kinesis.secret_access_key` | `FLEET_KINESIS_SECRET_ACCESS_KEY` | string | `""` <!-- server/config/config.go:1564 -->|
| `kinesis.sts_assume_role_arn` | `FLEET_KINESIS_STS_ASSUME_ROLE_ARN` | string | `""` <!-- server/config/config.go:1565 -->|
| `kinesis.sts_external_id` | `FLEET_KINESIS_STS_EXTERNAL_ID` | string | `""` <!-- server/config/config.go:1567 -->|
| `kinesis.status_stream` | `FLEET_KINESIS_STATUS_STREAM` | string | `""` <!-- server/config/config.go:1569 -->|
| `kinesis.result_stream` | `FLEET_KINESIS_RESULT_STREAM` | string | `""` <!-- server/config/config.go:1571 -->|
| `kinesis.audit_stream` | `FLEET_KINESIS_AUDIT_STREAM` | string | `""` <!-- server/config/config.go:1573 -->|
| `lambda.region` | `FLEET_LAMBDA_REGION` | string | `""` <!-- server/config/config.go:1577 -->|
| `lambda.access_key_id` | `FLEET_LAMBDA_ACCESS_KEY_ID` | string | `""` <!-- server/config/config.go:1578 -->|
| `lambda.secret_access_key` | `FLEET_LAMBDA_SECRET_ACCESS_KEY` | string | `""` <!-- server/config/config.go:1579 -->|
| `lambda.sts_assume_role_arn` | `FLEET_LAMBDA_STS_ASSUME_ROLE_ARN` | string | `""` <!-- server/config/config.go:1580 -->|
| `lambda.sts_external_id` | `FLEET_LAMBDA_STS_EXTERNAL_ID` | string | `""` <!-- server/config/config.go:1582 -->|
| `lambda.status_function` | `FLEET_LAMBDA_STATUS_FUNCTION` | string | `""` <!-- server/config/config.go:1584 -->|
| `lambda.result_function` | `FLEET_LAMBDA_RESULT_FUNCTION` | string | `""` <!-- server/config/config.go:1586 -->|
| `lambda.audit_function` | `FLEET_LAMBDA_AUDIT_FUNCTION` | string | `""` <!-- server/config/config.go:1588 -->|
| `s3.bucket` *(hidden)* | `FLEET_S3_BUCKET` | string | `""` <!-- server/config/config.go:1592 -->|
| `s3.prefix` *(hidden)* | `FLEET_S3_PREFIX` | string | `""` <!-- server/config/config.go:1593 -->|
| `s3.region` *(hidden)* | `FLEET_S3_REGION` | string | `""` <!-- server/config/config.go:1594 -->|
| `s3.endpoint_url` *(hidden)* | `FLEET_S3_ENDPOINT_URL` | string | `""` <!-- server/config/config.go:1595 -->|
| `s3.access_key_id` *(hidden)* | `FLEET_S3_ACCESS_KEY_ID` | string | `""` <!-- server/config/config.go:1596 -->|
| `s3.secret_access_key` *(hidden)* | `FLEET_S3_SECRET_ACCESS_KEY` | string | `""` <!-- server/config/config.go:1597 -->|
| `s3.sts_assume_role_arn` *(hidden)* | `FLEET_S3_STS_ASSUME_ROLE_ARN` | string | `""` <!-- server/config/config.go:1598 -->|
| `s3.sts_external_id` *(hidden)* | `FLEET_S3_STS_EXTERNAL_ID` | string | `""` <!-- server/config/config.go:1599 -->|
| `s3.disable_ssl` *(hidden)* | `FLEET_S3_DISABLE_SSL` | bool | `false` <!-- server/config/config.go:1600 -->|
| `s3.force_s3_path_style` *(hidden)* | `FLEET_S3_FORCE_S3_PATH_STYLE` | bool | `false` <!-- server/config/config.go:1601 -->|
| `s3.carves_bucket` | `FLEET_S3_CARVES_BUCKET` | string | `""` <!-- server/config/config.go:1620 -->|
| `s3.carves_prefix` | `FLEET_S3_CARVES_PREFIX` | string | `""` <!-- server/config/config.go:1621 -->|
| `s3.carves_region` | `FLEET_S3_CARVES_REGION` | string | `""` <!-- server/config/config.go:1622 -->|
| `s3.carves_endpoint_url` | `FLEET_S3_CARVES_ENDPOINT_URL` | string | `""` <!-- server/config/config.go:1623 -->|
| `s3.carves_access_key_id` | `FLEET_S3_CARVES_ACCESS_KEY_ID` | string | `""` <!-- server/config/config.go:1624 -->|
| `s3.carves_secret_access_key` | `FLEET_S3_CARVES_SECRET_ACCESS_KEY` | string | `""` <!-- server/config/config.go:1625 -->|
| `s3.carves_sts_assume_role_arn` | `FLEET_S3_CARVES_STS_ASSUME_ROLE_ARN` | string | `""` <!-- server/config/config.go:1626 -->|
| `s3.carves_sts_external_id` | `FLEET_S3_CARVES_STS_EXTERNAL_ID` | string | `""` <!-- server/config/config.go:1627 -->|
| `s3.carves_disable_ssl` | `FLEET_S3_CARVES_DISABLE_SSL` | bool | `false` <!-- server/config/config.go:1628 -->|
| `s3.carves_force_s3_path_style` | `FLEET_S3_CARVES_FORCE_S3_PATH_STYLE` | bool | `false` <!-- server/config/config.go:1629 -->|
| `s3.carves_gcs_iam_auth` | `FLEET_S3_CARVES_GCS_IAM_AUTH` | bool | `false` <!-- server/config/config.go:1630 -->|
| `s3.carves_cleanup_disabled` | `FLEET_S3_CARVES_CLEANUP_DISABLED` | bool | `false` <!-- server/config/config.go:1631 -->|
| `s3.carves_cleanup_max_per_run` | `FLEET_S3_CARVES_CLEANUP_MAX_PER_RUN` | int | `1000` <!-- server/config/config.go:1632 -->|
| `s3.carves_cleanup_concurrency` | `FLEET_S3_CARVES_CLEANUP_CONCURRENCY` | int | `32` <!-- server/config/config.go:1633 -->|
| `s3.software_installers_bucket` | `FLEET_S3_SOFTWARE_INSTALLERS_BUCKET` | string | `""` <!-- server/config/config.go:1636 -->|
| `s3.software_installers_prefix` | `FLEET_S3_SOFTWARE_INSTALLERS_PREFIX` | string | `""` <!-- server/config/config.go:1637 -->|
| `s3.software_installers_region` | `FLEET_S3_SOFTWARE_INSTALLERS_REGION` | string | `""` <!-- server/config/config.go:1638 -->|
| `s3.software_installers_endpoint_url` | `FLEET_S3_SOFTWARE_INSTALLERS_ENDPOINT_URL` | string | `""` <!-- server/config/config.go:1639 -->|
| `s3.software_installers_access_key_id` | `FLEET_S3_SOFTWARE_INSTALLERS_ACCESS_KEY_ID` | string | `""` <!-- server/config/config.go:1640 -->|
| `s3.software_installers_secret_access_key` | `FLEET_S3_SOFTWARE_INSTALLERS_SECRET_ACCESS_KEY` | string | `""` <!-- server/config/config.go:1641 -->|
| `s3.software_installers_sts_assume_role_arn` | `FLEET_S3_SOFTWARE_INSTALLERS_STS_ASSUME_ROLE_ARN` | string | `""` <!-- server/config/config.go:1642 -->|
| `s3.software_installers_sts_external_id` | `FLEET_S3_SOFTWARE_INSTALLERS_STS_EXTERNAL_ID` | string | `""` <!-- server/config/config.go:1643 -->|
| `s3.software_installers_disable_ssl` | `FLEET_S3_SOFTWARE_INSTALLERS_DISABLE_SSL` | bool | `false` <!-- server/config/config.go:1644 -->|
| `s3.software_installers_force_s3_path_style` | `FLEET_S3_SOFTWARE_INSTALLERS_FORCE_S3_PATH_STYLE` | bool | `false` <!-- server/config/config.go:1645 -->|
| `s3.software_installers_gcs_iam_auth` | `FLEET_S3_SOFTWARE_INSTALLERS_GCS_IAM_AUTH` | bool | `false` <!-- server/config/config.go:1646 -->|
| `s3.software_installers_cloudfront_url` | `FLEET_S3_SOFTWARE_INSTALLERS_CLOUDFRONT_URL` | string | `""` <!-- server/config/config.go:1647 -->|
| `s3.software_installers_cloudfront_url_signing_public_key_id` | `FLEET_S3_SOFTWARE_INSTALLERS_CLOUDFRONT_URL_SIGNING_PUBLIC_KEY_ID` | string | `""` <!-- server/config/config.go:1648 -->|
| `s3.software_installers_cloudfront_url_signing_private_key` | `FLEET_S3_SOFTWARE_INSTALLERS_CLOUDFRONT_URL_SIGNING_PRIVATE_KEY` | string | `""` <!-- server/config/config.go:1649 -->|
| `pubsub.project` | `FLEET_PUBSUB_PROJECT` | string | `""` <!-- server/config/config.go:1652 -->|
| `pubsub.status_topic` | `FLEET_PUBSUB_STATUS_TOPIC` | string | `""` <!-- server/config/config.go:1653 -->|
| `pubsub.result_topic` | `FLEET_PUBSUB_RESULT_TOPIC` | string | `""` <!-- server/config/config.go:1654 -->|
| `pubsub.audit_topic` | `FLEET_PUBSUB_AUDIT_TOPIC` | string | `""` <!-- server/config/config.go:1655 -->|
| `pubsub.add_attributes` | `FLEET_PUBSUB_ADD_ATTRIBUTES` | bool | `false` <!-- server/config/config.go:1656 -->|
| `filesystem.status_log_file` | `FLEET_FILESYSTEM_STATUS_LOG_FILE` | string | `<system temp dir>/osquery_status` *(computed)* <!-- server/config/config.go:1659; expr filepath.Join(os.TempDir(), "osquery_status") -->|
| `filesystem.result_log_file` | `FLEET_FILESYSTEM_RESULT_LOG_FILE` | string | `<system temp dir>/osquery_result` *(computed)* <!-- server/config/config.go:1661; expr filepath.Join(os.TempDir(), "osquery_result") -->|
| `filesystem.audit_log_file` | `FLEET_FILESYSTEM_AUDIT_LOG_FILE` | string | `<system temp dir>/audit` *(computed)* <!-- server/config/config.go:1663; expr filepath.Join(os.TempDir(), "audit") -->|
| `filesystem.enable_log_rotation` | `FLEET_FILESYSTEM_ENABLE_LOG_ROTATION` | bool | `false` <!-- server/config/config.go:1665 -->|
| `filesystem.enable_log_compression` | `FLEET_FILESYSTEM_ENABLE_LOG_COMPRESSION` | bool | `false` <!-- server/config/config.go:1667 -->|
| `filesystem.max_size` | `FLEET_FILESYSTEM_MAX_SIZE` | int | `500` <!-- server/config/config.go:1669 -->|
| `filesystem.max_age` | `FLEET_FILESYSTEM_MAX_AGE` | int | `28` <!-- server/config/config.go:1670 -->|
| `filesystem.max_backups` | `FLEET_FILESYSTEM_MAX_BACKUPS` | int | `3` <!-- server/config/config.go:1671 -->|
| `webhook.status_url` | `FLEET_WEBHOOK_STATUS_URL` | string | `""` <!-- server/config/config.go:1674 -->|
| `webhook.result_url` | `FLEET_WEBHOOK_RESULT_URL` | string | `""` <!-- server/config/config.go:1675 -->|
| `kafkarest.status_topic` | `FLEET_KAFKAREST_STATUS_TOPIC` | string | `""` <!-- server/config/config.go:1678 -->|
| `kafkarest.result_topic` | `FLEET_KAFKAREST_RESULT_TOPIC` | string | `""` <!-- server/config/config.go:1679 -->|
| `kafkarest.audit_topic` | `FLEET_KAFKAREST_AUDIT_TOPIC` | string | `""` <!-- server/config/config.go:1680 -->|
| `kafkarest.proxyhost` | `FLEET_KAFKAREST_PROXYHOST` | string | `""` <!-- server/config/config.go:1681 -->|
| `kafkarest.content_type_value` | `FLEET_KAFKAREST_CONTENT_TYPE_VALUE` | string | `"application/vnd.kafka.json.v1+json"` <!-- server/config/config.go:1682 -->|
| `kafkarest.timeout` | `FLEET_KAFKAREST_TIMEOUT` | int | `5` <!-- server/config/config.go:1684 -->|
| `nats.status_subject` | `FLEET_NATS_STATUS_SUBJECT` | string | `""` <!-- server/config/config.go:1687 -->|
| `nats.result_subject` | `FLEET_NATS_RESULT_SUBJECT` | string | `""` <!-- server/config/config.go:1688 -->|
| `nats.audit_subject` | `FLEET_NATS_AUDIT_SUBJECT` | string | `""` <!-- server/config/config.go:1689 -->|
| `nats.server` | `FLEET_NATS_SERVER` | string | `""` <!-- server/config/config.go:1690 -->|
| `nats.cred_file` | `FLEET_NATS_CRED_FILE` | string | `""` <!-- server/config/config.go:1691 -->|
| `nats.nkey_file` | `FLEET_NATS_NKEY_FILE` | string | `""` <!-- server/config/config.go:1692 -->|
| `nats.tls_client_crt_file` | `FLEET_NATS_TLS_CLIENT_CRT_FILE` | string | `""` <!-- server/config/config.go:1693 -->|
| `nats.tls_client_key_file` | `FLEET_NATS_TLS_CLIENT_KEY_FILE` | string | `""` <!-- server/config/config.go:1694 -->|
| `nats.ca_crt_file` | `FLEET_NATS_CA_CRT_FILE` | string | `""` <!-- server/config/config.go:1695 -->|
| `nats.compression` | `FLEET_NATS_COMPRESSION` | string | `""` <!-- server/config/config.go:1696 -->|
| `nats.jetstream` | `FLEET_NATS_JETSTREAM` | bool | `false` <!-- server/config/config.go:1697 -->|
| `nats.timeout` | `FLEET_NATS_TIMEOUT` | duration | `30s` <!-- server/config/config.go:1698 -->|
| `splunk.url` | `FLEET_SPLUNK_URL` | string | `""` <!-- server/config/config.go:1701 -->|
| `splunk.token` | `FLEET_SPLUNK_TOKEN` | string | `""` <!-- server/config/config.go:1702 -->|
| `splunk.index` | `FLEET_SPLUNK_INDEX` | string | `""` <!-- server/config/config.go:1703 -->|
| `splunk.source` | `FLEET_SPLUNK_SOURCE` | string | `""` <!-- server/config/config.go:1704 -->|
| `splunk.source_type` | `FLEET_SPLUNK_SOURCE_TYPE` | string | `""` <!-- server/config/config.go:1705 -->|
| `splunk.insecure_skip_verify` | `FLEET_SPLUNK_INSECURE_SKIP_VERIFY` | bool | `false` <!-- server/config/config.go:1706 -->|
| `license.key` | `FLEET_LICENSE_KEY` | string | `""` <!-- server/config/config.go:1709 -->|
| `license.enforce_host_limit` | `FLEET_LICENSE_ENFORCE_HOST_LIMIT` | bool | `false` <!-- server/config/config.go:1710 -->|
| `vulnerabilities.databases_path` | `FLEET_VULNERABILITIES_DATABASES_PATH` | string | `"/tmp/vulndbs"` <!-- server/config/config.go:1713 -->|
| `vulnerabilities.periodicity` | `FLEET_VULNERABILITIES_PERIODICITY` | duration | `1h` <!-- server/config/config.go:1715 -->|
| `vulnerabilities.cpe_database_url` | `FLEET_VULNERABILITIES_CPE_DATABASE_URL` | string | `""` <!-- server/config/config.go:1717 -->|
| `vulnerabilities.cpe_translations_url` | `FLEET_VULNERABILITIES_CPE_TRANSLATIONS_URL` | string | `""` <!-- server/config/config.go:1719 -->|
| `vulnerabilities.cve_feed_prefix_url` | `FLEET_VULNERABILITIES_CVE_FEED_PREFIX_URL` | string | `""` <!-- server/config/config.go:1721 -->|
| `vulnerabilities.cisa_known_exploits_url` | `FLEET_VULNERABILITIES_CISA_KNOWN_EXPLOITS_URL` | string | `""` <!-- server/config/config.go:1723 -->|
| `vulnerabilities.current_instance_checks` | `FLEET_VULNERABILITIES_CURRENT_INSTANCE_CHECKS` | string | `"auto"` <!-- server/config/config.go:1725 -->|
| `vulnerabilities.disable_schedule` | `FLEET_VULNERABILITIES_DISABLE_SCHEDULE` | bool | `false` <!-- server/config/config.go:1727 -->|
| `vulnerabilities.disable_data_sync` | `FLEET_VULNERABILITIES_DISABLE_DATA_SYNC` | bool | `false` <!-- server/config/config.go:1729 -->|
| `vulnerabilities.recent_vulnerability_max_age` | `FLEET_VULNERABILITIES_RECENT_VULNERABILITY_MAX_AGE` | duration | `720h` <!-- server/config/config.go:1731 -->|
| `vulnerabilities.disable_win_os_vulnerabilities` | `FLEET_VULNERABILITIES_DISABLE_WIN_OS_VULNERABILITIES` | bool | `false` <!-- server/config/config.go:1733 -->|
| `vulnerabilities.osv_for_vulnerabilities` | `FLEET_VULNERABILITIES_OSV_FOR_VULNERABILITIES` | bool | `true` <!-- server/config/config.go:1738 -->|
| `vulnerabilities.max_concurrency` | `FLEET_VULNERABILITIES_MAX_CONCURRENCY` | int | `1` <!-- server/config/config.go:1743 -->|
| `upgrades.allow_missing_migrations` | `FLEET_UPGRADES_ALLOW_MISSING_MIGRATIONS` | bool | `false` <!-- server/config/config.go:1750 -->|
| `sentry.dsn` | `FLEET_SENTRY_DSN` | string | `""` <!-- server/config/config.go:1754 -->|
| `geoip.database_path` | `FLEET_GEOIP_DATABASE_PATH` | string | `""` <!-- server/config/config.go:1757 -->|
| `prometheus.basic_auth.username` | `FLEET_PROMETHEUS_BASIC_AUTH_USERNAME` | string | `""` <!-- server/config/config.go:1760 -->|
| `prometheus.basic_auth.password` | `FLEET_PROMETHEUS_BASIC_AUTH_PASSWORD` | string | `""` <!-- server/config/config.go:1761 -->|
| `prometheus.basic_auth.disable` | `FLEET_PROMETHEUS_BASIC_AUTH_DISABLE` | bool | `false` <!-- server/config/config.go:1762 -->|
| `packaging.global_enroll_secret` | `FLEET_PACKAGING_GLOBAL_ENROLL_SECRET` | string | `""` <!-- server/config/config.go:1767 -->|
| `packaging.s3.bucket` | `FLEET_PACKAGING_S3_BUCKET` | string | `""` <!-- server/config/config.go:1768 -->|
| `packaging.s3.prefix` | `FLEET_PACKAGING_S3_PREFIX` | string | `""` <!-- server/config/config.go:1769 -->|
| `packaging.s3.region` | `FLEET_PACKAGING_S3_REGION` | string | `""` <!-- server/config/config.go:1770 -->|
| `packaging.s3.endpoint_url` | `FLEET_PACKAGING_S3_ENDPOINT_URL` | string | `""` <!-- server/config/config.go:1771 -->|
| `packaging.s3.access_key_id` | `FLEET_PACKAGING_S3_ACCESS_KEY_ID` | string | `""` <!-- server/config/config.go:1772 -->|
| `packaging.s3.secret_access_key` | `FLEET_PACKAGING_S3_SECRET_ACCESS_KEY` | string | `""` <!-- server/config/config.go:1773 -->|
| `packaging.s3.sts_assume_role_arn` | `FLEET_PACKAGING_S3_STS_ASSUME_ROLE_ARN` | string | `""` <!-- server/config/config.go:1774 -->|
| `packaging.s3.sts_external_id` | `FLEET_PACKAGING_S3_STS_EXTERNAL_ID` | string | `""` <!-- server/config/config.go:1775 -->|
| `packaging.s3.disable_ssl` | `FLEET_PACKAGING_S3_DISABLE_SSL` | bool | `false` <!-- server/config/config.go:1776 -->|
| `packaging.s3.force_s3_path_style` | `FLEET_PACKAGING_S3_FORCE_S3_PATH_STYLE` | bool | `false` <!-- server/config/config.go:1777 -->|
| `mdm.apple_apns_cert` | `FLEET_MDM_APPLE_APNS_CERT` | string | `""` <!-- server/config/config.go:1780 -->|
| `mdm.apple_apns_cert_bytes` | `FLEET_MDM_APPLE_APNS_CERT_BYTES` | string | `""` <!-- server/config/config.go:1781 -->|
| `mdm.apple_apns_key` | `FLEET_MDM_APPLE_APNS_KEY` | string | `""` <!-- server/config/config.go:1782 -->|
| `mdm.apple_apns_key_bytes` | `FLEET_MDM_APPLE_APNS_KEY_BYTES` | string | `""` <!-- server/config/config.go:1783 -->|
| `mdm.apple_scep_cert` | `FLEET_MDM_APPLE_SCEP_CERT` | string | `""` <!-- server/config/config.go:1784 -->|
| `mdm.apple_scep_cert_bytes` | `FLEET_MDM_APPLE_SCEP_CERT_BYTES` | string | `""` <!-- server/config/config.go:1785 -->|
| `mdm.apple_scep_key` | `FLEET_MDM_APPLE_SCEP_KEY` | string | `""` <!-- server/config/config.go:1786 -->|
| `mdm.apple_scep_key_bytes` | `FLEET_MDM_APPLE_SCEP_KEY_BYTES` | string | `""` <!-- server/config/config.go:1787 -->|
| `mdm.apple_bm_server_token` | `FLEET_MDM_APPLE_BM_SERVER_TOKEN` | string | `""` <!-- server/config/config.go:1788 -->|
| `mdm.apple_bm_server_token_bytes` | `FLEET_MDM_APPLE_BM_SERVER_TOKEN_BYTES` | string | `""` <!-- server/config/config.go:1789 -->|
| `mdm.apple_bm_cert` | `FLEET_MDM_APPLE_BM_CERT` | string | `""` <!-- server/config/config.go:1790 -->|
| `mdm.apple_bm_cert_bytes` | `FLEET_MDM_APPLE_BM_CERT_BYTES` | string | `""` <!-- server/config/config.go:1791 -->|
| `mdm.apple_bm_key` | `FLEET_MDM_APPLE_BM_KEY` | string | `""` <!-- server/config/config.go:1792 -->|
| `mdm.apple_bm_key_bytes` | `FLEET_MDM_APPLE_BM_KEY_BYTES` | string | `""` <!-- server/config/config.go:1793 -->|
| `mdm.apple_enable` | `FLEET_MDM_APPLE_ENABLE` | bool | `false` <!-- server/config/config.go:1794 -->|
| `mdm.apple_scep_signer_validity_days` | `FLEET_MDM_APPLE_SCEP_SIGNER_VALIDITY_DAYS` | int | `365` <!-- server/config/config.go:1795 -->|
| `mdm.apple_vpp_app_metadata_api_bearer_token` | `FLEET_MDM_APPLE_VPP_APP_METADATA_API_BEARER_TOKEN` | string | `""` <!-- server/config/config.go:1796 -->|
| `mdm.apple_scep_challenge` | `FLEET_MDM_APPLE_SCEP_CHALLENGE` | string | `""` <!-- server/config/config.go:1797 -->|
| `mdm.apple_dep_sync_periodicity` | `FLEET_MDM_APPLE_DEP_SYNC_PERIODICITY` | duration | `1m` <!-- server/config/config.go:1798 -->|
| `mdm.windows_wstep_identity_cert` | `FLEET_MDM_WINDOWS_WSTEP_IDENTITY_CERT` | string | `""` <!-- server/config/config.go:1799 -->|
| `mdm.windows_wstep_identity_key` | `FLEET_MDM_WINDOWS_WSTEP_IDENTITY_KEY` | string | `""` <!-- server/config/config.go:1800 -->|
| `mdm.windows_wstep_identity_cert_bytes` | `FLEET_MDM_WINDOWS_WSTEP_IDENTITY_CERT_BYTES` | string | `""` <!-- server/config/config.go:1801 -->|
| `mdm.windows_wstep_identity_key_bytes` | `FLEET_MDM_WINDOWS_WSTEP_IDENTITY_KEY_BYTES` | string | `""` <!-- server/config/config.go:1802 -->|
| `mdm.sso_rate_limit_per_minute` | `FLEET_MDM_SSO_RATE_LIMIT_PER_MINUTE` | int | `0` <!-- server/config/config.go:1803 -->|
| `mdm.certificate_profiles_limit` | `FLEET_MDM_CERTIFICATE_PROFILES_LIMIT` | int | `100` <!-- server/config/config.go:1804 -->|
| `mdm.enable_custom_os_updates_and_filevault` | `FLEET_MDM_ENABLE_CUSTOM_OS_UPDATES_AND_FILEVAULT` | bool | `false` <!-- server/config/config.go:1805 -->|
| `mdm.enable_custom_filevault` | `FLEET_MDM_ENABLE_CUSTOM_FILEVAULT` | bool | `false` <!-- server/config/config.go:1806 -->|
| `mdm.enable_custom_disk_encryption` | `FLEET_MDM_ENABLE_CUSTOM_DISK_ENCRYPTION` | bool | `false` <!-- server/config/config.go:1807 -->|
| `mdm.allow_all_declarations` | `FLEET_MDM_ALLOW_ALL_DECLARATIONS` | bool | `false` <!-- server/config/config.go:1808 -->|
| `mdm.android_agent.package` *(hidden)* | `FLEET_MDM_ANDROID_AGENT_PACKAGE` | string | `"com.fleetdm.agent"` <!-- server/config/config.go:1809 -->|
| `mdm.android_agent.signing_sha256` *(hidden)* | `FLEET_MDM_ANDROID_AGENT_SIGNING_SHA256` | string | `"x+IyvrwVbQEBYV/ojWmLavJE0VIZE1RAT2JmxeI5sFw="` <!-- server/config/config.go:1810 -->|
| `mdm.android_batch_size` *(hidden)* | `FLEET_MDM_ANDROID_BATCH_SIZE` | int | `100` <!-- server/config/config.go:1813 -->|
| `calendar.periodicity` | `FLEET_CALENDAR_PERIODICITY` | duration | `0s` <!-- server/config/config.go:1817 -->|
| `partnerships.enable_secureframe` | `FLEET_PARTNERSHIPS_ENABLE_SECUREFRAME` | bool | `false` <!-- server/config/config.go:1823 -->|
| `microsoft_compliance_partner.proxy_uri` | `FLEET_MICROSOFT_COMPLIANCE_PARTNER_PROXY_URI` | string | `"https://fleetdm.com"` <!-- server/config/config.go:1826 -->|
| `partnerships.enable_primo` | `FLEET_PARTNERSHIPS_ENABLE_PRIMO` | bool | `false` <!-- server/config/config.go:1828 -->|
| `conditional_access.cert_serial_format` | `FLEET_CONDITIONAL_ACCESS_CERT_SERIAL_FORMAT` | string | `"hex"` <!-- server/config/config.go:1831 -->|
