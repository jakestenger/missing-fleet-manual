...
The frontmatter records three separate things, and it is worth knowing which is which. **The release** a chapter was written against. **Whether its claims were checked** at that release, in which case the notes behind the check exist in the repository. And **whether an independent reviewer read it** afterwards, which is a different question from whether the sources were checked.

A chapter carries `status: verified` only when the second and third are both true. That combination is deliberate: twelve chapters once carried a verification stamp on the source check alone, and an independent review then found a material defect in every one. Checking a source proves the source says what you quoted. It does not catch a conclusion the source never supported, a heading that claims more than its paragraph, or two sections of the same chapter disagreeing.

A chapter that says it is unverified means exactly that, and is worth more caution than one carrying the full stamp.
