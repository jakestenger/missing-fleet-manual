...
**Requires administrator or maintainer**, globally or on the host's own fleet; GitOps cannot invoke it, the same restriction [above](#eligibility-before-you-find-out-the-hard-way) gives lock, unlock, and wipe.
...
<!-- SCREENSHOT: SS-5.7-02 | Apple MDM command result
     ...
     CAPTURE: Open the host's MDM command history and expand a command result. Show the command name, status, timestamps, and readable response details available in this version. Use an existing NotNow or Error result as a second frame only if naturally available.
