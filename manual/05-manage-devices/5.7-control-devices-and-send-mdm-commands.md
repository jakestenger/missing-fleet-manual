...
![How-to](../_assets/icons/howto.svg) Five things to settle before sending any of these.
...
| **Any of them** | Admin, maintainer or GitOps for the relevant scope |
...
**Send it** with `fleetctl mdm run-command`, or `POST /commands/run` against the API. Apple expects a plist XML command; Windows expects a single `<Exec>` containing one `<Item>`.

**Command identifiers differ by entry point.** The direct API requires and preserves the identifier in your payload; `fleetctl` replaces it with one of its own. Either way, **keep the identifier you get back**, because it is how you retrieve the result later with `fleetctl get mdm-command-results` or `/commands/results`.
...
<!-- IMAGE-TODO: assets/5.7-action-transport-and-reversibility.webp
     ...
     SECOND LAYER, and the reason it earns its place over a plain table: mark REVERSIBILITY BY
     FLEET. Wipe is irreversible everywhere. Android lock is not reversible FROM FLEET, though the
     user can unlock the device locally with its own PIN, so do NOT label it one-way or
     irreversible; label it "not reversible from Fleet". macOS unlock needs a person at the machine.
     Use one restrained accent for the irreversible and not-reversible-from-Fleet cells rather than
     colouring the whole grid.
