...
**This is reactive, not something you can invoke.** Fleet only asks for a rotation when a key it already holds is marked undecryptable; there is no path, including [the reveal action above](#reveal-validate-and-repair-disk-recovery-credentials-safely), that lets you trigger a rotation on a key Fleet can still read.
