...
![Explanation](../_assets/icons/explanation.svg) A useful encryption review answers four questions. Fleet provides signals for the first three; the fourth requires a recovery test:
...
![How-to](../_assets/icons/howto.svg) Fleet delivers the FileVault configuration, macOS asks the user to enable it, and the resulting personal recovery key is escrowed to Fleet. Include the user prompt in your rollout communications so people know what is expected.
...
![How-to](../_assets/icons/howto.svg) On Windows, fleetd performs the BitLocker work after the management prerequisites are satisfied. The agent enables protection and sends the recovery material to Fleet.
...
![How-to](../_assets/icons/howto.svg) A BitLocker startup PIN adds a user-entered step before Windows starts. You enable the requirement in Fleet, the user creates the PIN through Windows, and Fleet reports whether the required protector is present.
...
![How-to](../_assets/icons/howto.svg) Linux escrow adds a recovery credential to Fleet for an existing LUKS-encrypted disk. Plan the user step as part of enrollment so recovery material is available when support staff need it.
...
![Reference](../_assets/icons/reference.svg) Use encryption status to identify hosts that need attention, then examine the underlying signal before choosing a repair.
...
![Troubleshooting](../_assets/icons/troubleshooting.svg) Include escrow in the change plan for enrollment, fleet moves, and certificate maintenance. These operations can affect recovery even when the disk remains encrypted.
...
![Explanation](../_assets/icons/explanation.svg) Recovery Lock protects access to a Mac's recovery environment. Manage it separately from the FileVault recovery key, with its own control, status, and rotation process.
...
![Reference](../_assets/icons/reference.svg) Which encryption mechanism each platform supports, and the licence it requires, are in [a.2](../09-appendices/a.2-platform-capability-matrix.md). Whether an encryption or recovery-credential action is reachable through the UI, fleetctl, the API, or GitOps is in [a.5](../09-appendices/a.5-interface-index.md); the underlying endpoints are in [a.8](../09-appendices/a.8-api-action-and-endpoint-reference.md).
