...
Four states are in play: desired, observed encrypted, credential escrowed, and credential proven. Fleet can show the first three imperfectly and cannot prove the fourth.
...
**A disk can be encrypted while Fleet holds no usable credential for it.** Fleet can also create and escrow a credential for a disk it did not encrypt. Enforcement and escrow are not independent switches you can set, but they are independent *facts* you can be in.

> **The status is a summary of partial signals, not a reading of those four.** The set of values differs by platform: six on macOS, five on Windows, three on Linux. It does not label "the disk is encrypted" and "Fleet has a key" as separate fields, and **it does not mark a credential as archived when it serves you one.** Answer the fourth question yourself.
...
On Windows, encryption and the startup PIN are locked together as one decision.
...
What Fleet escrows is a **numerical password protector**. That is a specific BitLocker construct, not "the BitLocker key".
...
**An already-encrypted Windows disk has its existing recovery credential invalidated and removed from the device.** That is more assertive than the macOS equivalent. Account for it before you switch enforcement on across machines somebody else encrypted.
...
> **On Windows and Linux, no scheduled check runs at all.** A credential is marked usable at the moment it is escrowed. The indicator you are reading on those platforms is a default rather than a result. A reveal still attempts to decrypt the stored value, so a successful reveal tells you more than the indicator does.
...
**To establish that you could actually recover a machine, recover a machine.** Take one host out of production, reveal its credential, and use it. That is the only thing that answers the fourth claim.
...
**Handle the revealed value as a live credential.** Move it through whatever channel your organisation uses for secrets, not through a ticket or a chat message. Rotate or re-escrow after a disclosure that went somewhere it should not have.

**"Repair" is platform-specific.** There is no universal rotate action, and what is possible on one platform may have no equivalent on another.

### The one credential Fleet repairs on its own

A scheduled job re-tries decrypting every macOS key it has not already verified, using the current Apple certificate authority material ([above](#read-compound-status-without-overclaiming-recovery) covers what that verification does and does not prove). When a key comes back undecryptable and the host's own agent has told Fleet it supports Escrow Buddy, Fleet sets a rotation instruction in that host's configuration. **The agent does not act silently:** at the person's next login, it prompts them for their account password, generates a new personal recovery key, and escrows the replacement. If the agent has not declared the Escrow Buddy capability, none of that happens: Fleet sends no instruction, and nothing in the interface tells you a repair was skipped.

**This is reactive, not something you can invoke.** Fleet only asks for a rotation when a key it already holds is marked undecryptable. No path lets you trigger a rotation on a key Fleet can still read, including [the reveal action above](#reveal-validate-and-repair-disk-recovery-credentials-safely).

**Windows and Linux have no equivalent.** BitLocker enforcement reacts to a different signal: whether the host itself reports encryption disabled, not whether Fleet can decrypt what it is holding. So an undecryptable Windows credential stays undecryptable until whatever broke the enrolment certificate is fixed ([above](#certificate-and-key-material)). On Linux, [only the end user can start escrow](#escrow-recovery-material-from-an-already-encrypted-luks-host), and Fleet does not retry that either.
...
![Troubleshooting](../_assets/icons/troubleshooting.svg) Read this before an infrastructure change, not after one.

### Turning enforcement off

**Disabling is read-preserving and write-stopping.** Nothing decrypts, nothing already held is deleted, and you can still retrieve what Fleet has. New escrow stops.
...
**A fleet move deletes the live credential when the destination fleet does not enforce encryption, and preserves it when the destination does.** A fleet that will not escrow does not keep what it will never refresh.

Fleet decides by looking for exactly one thing, its own FileVault configuration profile on the destination fleet, and uses its presence as the answer for **all three platforms**. In a deployment with Windows MDM and no Apple MDM, that profile does not exist on any fleet, so the destination always looks like one that does not enforce encryption, and **every fleet move deletes escrowed BitLocker credentials.** If you run Windows without Apple MDM, treat a fleet move as a de-escrow and confirm the behaviour on one host before doing it in bulk.
...
>
> **Fleet will go on telling you those credentials are available.** Windows escrow is marked usable when it arrives and is excluded from later re-checking, so the status you read afterwards is the one recorded before the change. The indicator says yes and the retrieval fails.
>
...
In both cases the loss is silent, and you discover it when somebody needs a key. Recovery is possible only if the original certificate and key material, or a suitable backup, can be restored. **Before any change to this material, secure what you currently hold**, and plan as though the change were irreversible.

## Manage Recovery Lock as a separate Apple credential

![Explanation](../_assets/icons/explanation.svg) A different credential from the FileVault key, guarding a different thing.
...
That is the opposite of disabling disk-encryption enforcement, which preserves what Fleet holds indefinitely and by design.
