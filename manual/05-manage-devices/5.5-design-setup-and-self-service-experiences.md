...
verified_source: "drafted against fleet-v4.90.0 (7c428c6e46); citation ledger at research/section-notes/5.5-notes.md. Extended 2026-09-08 for Fleet 4.91.0 (campaign IA1), verified at tag fleet-v4.91.0: added a new subsection on the Windows managed local admin account (fleetd creates and hides the _fleetadmin account directly, escrows the password encrypted with the server private key, Premium, requires fleetd 1.60.0+; reveal works but no rotation), corrected the macOS-only caveats in Accounts and Retrieve and rotate; ledger rows recorded in the same notes file. Extended 2026-09-02 (round4 RB3) with a new subsection on Platform SSO account provisioning and password sync (the OAuth IdP configuration, device registration, the ROPG password relay and account-claim mapping), fixing a.1's CAP-003 misroute of the fpsso search term; new ledger rows recorded in the same notes file. Codex-reviewed (gpt-5.6-sol, read-only, same tag) before commit per the round4-fix protocol: 6 MAJOR corrections applied (the four-hour sync window instead of every unlock, the id_token's subject and other identity claims being the IdP's own values rather than something Fleet's re-signing makes independent of it, the full extent of unverified id_token fields, the Fleet-specific extension/profile/TLS prerequisites for device registration, ROPG's Okta/Entra provider restrictions, and GitOps's declarative resupply-on-every-apply behavior), plus 4 MINOR wording fixes; findings and fixes recorded in this notes file"
further_reading:
