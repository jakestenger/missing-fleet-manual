...
**The endpoint carries no licence check; edition decides what can appear in it.** Webhook and ticket automations are Free, so their rows show on any edition. Script, software-install, App Store, calendar and conditional-access automations are Premium, so their rows, and the script and install output with them, appear only there. The endpoint is the same on both; the fleet's edition sets how much it has to show.
