...
verified_source: "drafted against fleet-v4.90.1 (dd0200f062); citation ledger at research/section-notes/5.9-notes.md. Working research is held outside this repository, see research/section-notes/README.md"
further_reading:
...
The chapter is titled "responses" rather than "remediation" deliberately. Two of the seven responses fix the host. The others open a ticket, book a meeting, or stop somebody signing in.
...
> **The structural fact that shapes everything else: there is no cron for the responses that touch a host.** Software installs, App Store installs, script runs and conditional-access evaluation are all decided inside the request that delivers the failing result, one after another, before Fleet replies to the agent. Only webhooks, tickets and calendar events are deferred to a scheduled job.
>
> So the endpoint responses are as timely as the host's check-in, and the external ones are as timely as a cron you may not have looked at. **The webhook and ticket job runs every 24 hours by default.**

<!-- IMAGE-TODO: assets/5.9-what-fires-when.webp
     BRIEF: The chapter's orienting picture. A single host check-in arrives carrying policy results.
     Show the fixed sequence of decisions inside that one request: calendar callback, script,
     conditional access, App Store install, custom package install. Then show the two things that
     leave the request: work placed on the host's queue, and a host id written to a pending set.
     Below, on a separate and much slower timeline, show the scheduled job draining that pending set
     into webhooks and tickets, labelled with its 24-hour default.
     ACCURACY: the five inline decisions are sequential, not parallel. Conditional access is decided
     inline but delivered asynchronously, so draw its delivery leaving the request boundary. Do not
     draw a cron feeding the inline responses; there is none. Do not imply the administrator
     controls the order.
...
|---|---|---|---|
| **Install software** | Endpoint | Installs a custom package you uploaded | Fleet only |
...
**Several can sit on one policy at once**, and they do not coordinate. A policy can install a package, open a ticket and block sign-in for the same failure, in that order, with nothing linking the three.
...
- **Transition-based.** The response fires once. The host is already recorded as failing, so no later result is a transition, and **nothing happens again.** No retry, because retries only cover responses that *failed*. No activity, no warning. It stays that way until something re-arms the policy.
...
> **Write the policy query so that it passes only once the response has actually taken effect, and so that osquery can see the difference.** That is the whole safety argument. Everything else in this chapter is a consequence of it.

### The oscillation nobody warns you about
...
Serialisation has one exception: several App Store installs can activate together rather than strictly one at a time.
...
|---|---|---|---|
| Clears pass/fail history | **Yes, every host** | No | Only if the query or the attached response changed |
...
**Re-arming is not the same as re-firing.** Turning on a webhook or a calendar automation for a policy that hosts are already failing does nothing for those hosts, because there is no new transition to see. Continuous mode acts on the next failing result; attaching a *new* script or package clears history and so does re-fire.

### What re-arms a policy without you asking

Two host operations that an administrator would describe identically have opposite effects. **Re-enrolling a host through osquery deletes all of its policy history**, so every failing policy looks new and its responses fire again. **Re-enrolling through Orbit deletes none of it.** Neither resets retry counters, so a re-armed policy typically fires once with no retries left.
...
Ranked by how fast it takes effect:

1. **Detach the response from the policy.** Stops future triggers. Recalls nothing.
2. **Cancel the queued item on the host.** The only per-host control, and only for work still waiting.
3. **Delete the policy.** Cancels queued package work and discards unfinished script records. **It does not cancel a queued App Store install.** That carries on, having lost its link to the policy.
4. **Move the host out**, which stops future triggers and leaves the queue alone.

There is no pause. There is no off switch for automations as a class.
