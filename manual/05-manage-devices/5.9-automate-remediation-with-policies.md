...
Only the endpoint responses try to change the host. The others open a ticket, book a meeting, or stop somebody signing in.
...
![Explanation](../_assets/icons/explanation.svg) One specific kind of result triggers an automation, and the others do not.
...
| **No answer** | The query did not run, or errored | **No** |

**A no-answer is not a failure.** Results Fleet never received are filtered out before any trigger logic runs.
...
>
> So the endpoint responses are as timely as the host's check-in, and the rest run on clocks of their own. **The webhook and ticket job runs every 24 hours by default; the calendar job runs every five minutes.**
...
**Endpoint responses cannot be attached to an "All fleets" policy.** Neither can conditional access or continuous mode. Fleet rejects them. Calendar is the exception: it is accepted, saved, shown back to you, and **can never fire**, because the job that creates events only ever looks at named fleets. The same is true of a No-team policy.
...
![Explanation](../_assets/icons/explanation.svg) The trigger mode decides whether an automation goes silent or repeats.
...
### Why a successful script leaves the policy still failing
...
What happens when a response succeeds without making the policy pass depends entirely on the trigger mode:
...
> **Three things have to hold together, and Fleet validates none of the relationships between them.** Make the response safe to repeat ([5.3](5.3-run-and-manage-scripts.md) on idempotent scripts, [5.4](5.4-manage-software-and-applications.md) on repeated installs). Choose the trigger mode by deciding which failure you can live with, silence or repetition. Write the policy query so that it passes only once the response has actually taken effect, and so that osquery can see the difference.

### How no-answer re-arms transition mode

There is one way to get repeated firing without continuous mode.
...
![Explanation](../_assets/icons/explanation.svg) A patch policy can hold an update until the user has quit the app, so patching never interrupts an app someone is working in.
...
**The default is the interface's, not the server's.** The Add software and Deploy dialogs preselect patch when closed for a title that has no patch policy yet, so a policy created by clicking through Fleet gets it. Nothing else does: the REST field is a plain boolean and arrives `false` when the body omits it, and the column it lands in is declared `NOT NULL DEFAULT 0`. Write `patch_when_closed: true` explicitly in GitOps and in an API body, or the policy you meant to hold back until the app is closed will not hold back at all.
...
Reading "not running" differs across the two platforms Fleet-maintained apps run on:
...
>
> Conditional access has already read the original failing result and acted on it, because it runs earlier in the same request. The webhook and ticket filter runs afterwards and sees the blank.
>
...
If you run Okta conditional access, one deployment fact is a prerequisite rather than hardening, and it is a security boundary. It is Fleet Premium and needs the server private key configured. The Entra path does not use it; the two share nothing here but the word "certificate".
...
The exposure follows from that. A caller that can set the header can name any serial. A spoofed serial does not let it reach an arbitrary host, because Fleet resolves the serial against its own records and rejects one that is revoked, expired, or unknown, and the host it does resolve to must still pass, or bypass, the health checks that follow. A spoofed serial can name a known, unrevoked, unexpired conditional-access certificate and bind the evaluation to that certificate's host with the owning device absent. The property that prevents this lives entirely in your proxy and your network, not in Fleet.
...
The App Store difference is deliberate and acknowledged in Fleet's own source. **Do not carry the "three attempts" number across to it**: its retries respond to delivery errors rather than to the policy. One case is terminal and never retried: an app the user already installed personally on a BYOD device.
...
One GitOps warning: **creating a fleet from a spec silently drops its failing-policies webhook.** A second apply installs it, because that takes the edit path. Check after the first.

---

An automation can create work on a device, invoke a system outside Fleet, or change whether somebody can sign in. **None of those becomes desired state merely because Fleet initiated it.** The next policy result is the evidence, which is where [5.1](5.1-plan-target-and-govern-device-changes.md) came in.
