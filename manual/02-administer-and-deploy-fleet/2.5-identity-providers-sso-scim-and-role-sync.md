...
**A group inside a group counts from Fleet 4.91, and before that it did not.** Microsoft Entra ID does not flatten a nested group before sending it: it provisions the child as a *member* of the parent, in the same way it would provision a person. Fleet now records those parent-to-child relationships and resolves a user's **effective** membership by walking upward from the groups they belong to directly, so somebody in a child group is a member of every group above it. Everything downstream reads the effective set: the host's IdP groups vital, the group variable available to profiles and scripts, and any host-vitals label built on a group name.

**The practical consequence is that a label scoped to a parent group now matches.** Before this, a host whose owner sat only in a child group did not match a label built on the ancestor's name, and there was nothing on the host or the label to explain why. If you built flattened duplicate groups upstream to work around that, they are no longer doing anything for you.

**A membership change propagates without you re-applying anything.** Adding a child group edge, renaming a group, or moving a person re-queues the profiles that use the group variable for exactly the hosts whose effective membership changed, including the ones that changed only through the nesting.
