...
**Depth is not limited, and a loop does not hang Fleet.** The walk deduplicates as it goes, so a group graph that accidentally contains a cycle terminates rather than spinning. Identity providers that flatten their groups before pushing them, which is the more common behaviour, are unaffected either way: Fleet sees the same flat memberships it always did.
