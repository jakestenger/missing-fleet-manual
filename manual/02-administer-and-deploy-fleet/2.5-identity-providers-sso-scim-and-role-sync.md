...
![How-to](../_assets/icons/howto-light.svg) Aim to leave setup with a working sign-in path, a recovery option, and clear ownership for the account lifecycle:
...
![Reference](../_assets/icons/reference-light.svg) These four terms describe the responsibilities you can connect to your identity provider.
...
![Explanation](../_assets/icons/explanation-light.svg) One SCIM connection supports administrator deprovisioning and device-owner mapping. Consider both uses when choosing the attributes and groups your identity provider will send.
...
![Explanation](../_assets/icons/explanation-light.svg) Start by deciding how an administrator account should come into existence and how it should end, because everything else follows from that.
...
![How-to](../_assets/icons/howto-light.svg) Fleet implements SAML 2.0, and documents Okta, authentik, Google Workspace, and Microsoft Entra ID specifically. Another identity provider that speaks SAML 2.0 should work, but that is a claim about the standard rather than a tested promise, so validate yours during the pilot rather than discovering an incompatibility at rollout. Both IdP-initiated and service-provider-initiated sign-in are supported, so users can start from your application portal or from Fleet's login page.
...
![How-to](../_assets/icons/howto-light.svg) JIT provisioning is enabled by **Create user and sync permissions on login**, on the same SSO settings page. It requires Fleet Premium.
...
![How-to](../_assets/icons/howto-light.svg) SCIM is configured on your identity provider and points at Fleet, rather than the other way round. Fleet's SCIM base URL is `https://<your_fleet_server_url>/api/v1/fleet/scim`.
...
Fleet 4.91 adds support for nested SCIM groups. Microsoft Entra ID can send a child group as a member of its parent. Fleet records those relationships and walks upward from a user’s direct memberships to calculate the effective set. Host IdP-group vitals, profile and script variables, and host-vitals labels all use that effective membership.

A label based on a parent group can therefore match a host whose owner belongs to a child group. Earlier Fleet versions required direct membership, so review any flattened duplicate groups you created as a workaround before maintaining them further.

Adding a child-group relationship, renaming a group, or moving a person re-queues profiles that use the group variable for hosts whose effective membership changes. This includes changes inherited through nesting.

Fleet deduplicates the group traversal, so a cycle terminates safely. It imposes no additional depth limit, although MySQL’s `cte_max_recursion_depth` still bounds the recursive CTE. Identity providers that send flattened groups continue to produce the same flat memberships.
...
![Explanation](../_assets/icons/explanation-light.svg) If you connected SCIM for the second purpose, Fleet collects a host's IdP identity when the end user authenticates during enrollment. That covers automatic enrollment for Apple and Windows devices, and manual enrollment for Apple, Android, Windows, and Linux.
...
![Explanation](../_assets/icons/explanation-light.svg) Device conditional access extends identity integration to the applications employees use. A device presents a client certificate to an Okta or Entra flow so access can depend on its managed state. This Premium feature requires the server private key and is configured separately from administrator SSO.
...
![How-to](../_assets/icons/howto-light.svg) Change one thing at a time and confirm it before the next. The order below is the one that keeps you able to sign in.
...
![Reference](../_assets/icons/reference-light.svg) Verified against Fleet 4.90.0. JIT provisioning and the IdP host-vitals mapping are Fleet Premium capabilities. The `technician` role is available as a role-sync value at this release.
