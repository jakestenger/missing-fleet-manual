...
<!-- IMAGE-REDO: 2.1-setup-sequence.webp
     WHY: reviewed 2026-08-24. The wording fix landed, "managed cloud or self-hosted" is
     right, and the icons are a real improvement. **A legibility problem came with them.**
     The panel headings are set wider than the panels themselves, so "Choose the operating
     model", "Establish administration" and "Connect management services" all run past their
     edges and collide with the neighbouring panel and its arrow. Set the headings to wrap
     inside their panels, at a smaller size if needed, with clear margin on both sides.
     Second, smaller: this diagram and the lifecycle diagram in 1.1 are the same shape, five
     stages left to right, and they should look like siblings. 1.1 gives each stage its own
     dot colour at a light tint; this one alternates two greys. Use the same five tints in
     the same order so the two read as a pair.
