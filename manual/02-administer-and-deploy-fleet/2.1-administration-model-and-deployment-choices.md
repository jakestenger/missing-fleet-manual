...
Before deploying Fleet, decide who runs the servers, who may administer devices, and which operating systems Fleet will manage.
...
Two of the three may already be determined. Eligibility for managed cloud hosting depends on your licence tier and the size of your estate, and the identity provider is an organization-level choice rather than a Fleet one. That leaves how authority is divided and who is accountable for each OS relationship.
...
**Self-hosting is the path for everyone else.** You run the Fleet server, MySQL, Redis, TLS, object storage where you use it, networking, backups, and monitoring. Fleet documents deployments on AWS with Terraform, Kubernetes, GCP, Render, and Docker Compose, and Fleet Premium is available self-hosted with support.

Premium capabilities are identical on both paths. The difference is who carries the pager for MySQL.
...
![Explanation](../_assets/icons/explanation.svg) Both paths split responsibility along the same line. Name it before the first incident lands on it.
...
The reasoning behind that split, and what decides whether a request is allowed, are in [1.4 Identity and roles](../01-foundations/1.4-identity-and-roles.md). This chapter is about writing the answer down.
...
Ongoing, the global administrator group owns the access review and the renewal calendar. Fleet owners own their own device populations. Whoever owns the infrastructure owns the restore test. Run it before you need it.

Expanding past the pilot is one gate; carrying the whole estate in production is the next. [7.7](../07-operate-fleet/7.7-production-readiness-checklist-and-handoff.md#the-go-live-decision) holds that go-live decision and the handover that follows it, and treats it mostly as a review of what this pilot already told you.
