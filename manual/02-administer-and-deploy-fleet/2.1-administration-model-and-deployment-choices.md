...
A small **global administrator** group should own the organization-wide settings that affect everyone: identity-provider configuration, fleet creation, MDM service connections, the Fleet organization settings, and access review. These are the settings where one change reaches every device, and they deserve a group small enough that its members can talk to each other. Audit-log delivery is not one of them: it is set in the server's own configuration rather than through a Fleet role, so it belongs with whoever operates the deployment ([2.8](2.8-activity-audit-logs-and-log-delivery.md)), the same point [1.4](../01-foundations/1.4-identity-and-roles.md) makes.
...
<!-- IMAGE-TODO: assets/2.1-ownership-escalation-map.webp
     ...
     NOTE: Proposed 2026-09-08; editorial brief, not technical re-verification. Use as an
     explanation of why the ownership worksheet exists; reduce the repeated named-person advice.
     Keep the fillable Markdown table and all review cadences. Keep current prose and this TODO
     ...
     CANDIDATE: ../../research/visual-reviews/part-2/assets/2.1-ownership-escalation-map.webp
...
![A shared ownership record connects Fleet administrators, infrastructure owners, external account owners, and their named backups.](assets/2.1-ownership-escalation-map.webp)
-->
...
Expanding past the pilot is one gate; carrying the whole estate in production is the next. [7.7](../07-operate-fleet/7.7-production-readiness-checklist-and-handoff.md#the-go-live-decision) holds that go-live decision and the handover that follows it, and treats it as mostly a review of what this pilot already told you, so the answers you collect here are the ones you revisit there.
