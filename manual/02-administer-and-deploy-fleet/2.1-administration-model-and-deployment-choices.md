...
<!-- IMAGE-REDO: 2.1-setup-sequence.webp
     WHY: reviewed 2026-08-24. Reads well and the tints are right. One wording fix: the
     first panel says "Fleet-hosted or self-hosted", but "Fleet-hosted" is not Fleet's term
     and the chapter deliberately does not use it. Fleet says **managed cloud hosting**.
     Change that panel to "Choose the operating model (managed cloud or self-hosted)". The
     panels are also text-only while the comparable lifecycle diagram in 1.1 carries an icon
     per stage; a simple navy icon in each would make them match.
