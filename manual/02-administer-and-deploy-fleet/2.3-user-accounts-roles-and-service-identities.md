...
<!-- IMAGE-REDO: 2.3-roles-and-reach.webp
     WHY: reviewed 2026-08-24. The caption is right and the six roles are correct, but
     the composition undercuts the point. Fleet scope is drawn nested **inside** Global scope,
     which reads as containment and muddles the very thing the picture exists to say: that
     these are the same six roles with different reach. Draw two panels of equal weight side
     by side instead, each listing the same six, with the difference stated beneath, "reaches
     every fleet" against "reaches one fleet". The role chips are also tinted in four or five
     near-identical greys and blues that carry no meaning; give every chip the same fill,
     since colour here implies a ranking that does not exist.
