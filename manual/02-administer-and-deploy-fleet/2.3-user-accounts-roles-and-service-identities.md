...
**An API-only identity can hold the global admin role**, and it counts toward that check. A deployment whose only global admin is a service token is one expired credential away from having no reachable administrator at all, so keep at least one human in the role.
