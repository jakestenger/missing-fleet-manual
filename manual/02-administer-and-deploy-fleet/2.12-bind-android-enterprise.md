...
You supply nothing to connect Fleet to Android: no certificate, no key, nothing with an expiry date. You bind Fleet to a Google enterprise, Google issues the binding, and it never renews. Your Fleet server URL is the hard thing to change afterwards: Fleet will accept a new one, and Google keeps pushing to the old one.
...
**Two things stop that being the whole picture.** Fleet **polls Google hourly** to reconcile which devices still exist, so a disappearance can be noticed without anyone being told. Every host, on both editions, gets a **narrow Fleet app** added to its policy at enrollment, which enrols with Fleet and reports certificate results back once a certificate template exists ([3.6](../03-connect-devices/3.6-enroll-android-devices.md)); creating that certificate template in the first place is the Premium-gated step, not installing the app. So there is a small amount of Fleet-initiated traffic and a small device-side component.

Android hosts therefore sit outside most of what this manual describes. No live queries, no scheduled reports, no scripts. Policies and inventory on Android come from what the Android Management API chooses to report, and the surface is Google's, not Fleet's.

The single exception is certificates, and the reason is a gap in Google's API rather than a Fleet design choice. The Android Management API has no REST call for installing a client certificate. It does allow delegating the `CERT_INSTALL` scope to a companion app, so Fleet ships one, and that app exists to do that one thing.
...
This is Google's design and Fleet inherits it.

**Only one of Apple's two personal enrollment paths hides the hardware, and the console does not distinguish them.** Both show "On (manual - personal)": **account-driven user enrollment** gives Fleet an enrollment identifier rather than a serial or UDID, while **personal enrollment by link** supplies the real serial and UDID during the over-the-air exchange ([3.5](../03-connect-devices/3.5-enroll-ios-and-ipados-devices.md)). So the status alone does not tell you whether a given Apple host's identifiers are real.
...
![How-to](../_assets/icons/howto.svg) This off switch is not reversible in any ordinary sense.

**Turning Android MDM off deletes your Android Enterprise.** Fleet works through a teardown sequence rather than one atomic action: it deletes the enterprise at Google, drops its own record of it, disables Android MDM, marks every Android host unenrolled so management stops for all of them, fails any pending Android app installs that were queued through Google's managed store, and deletes the stored Android connection secrets (its Pub/Sub notification token and the Fleet server secret). Each step runs in turn and returns on the first error, so a failure partway through can leave Google and Fleet disagreeing about whether the enterprise still exists. If the off switch errors, confirm both sides rather than assuming a clean teardown.

Google, not Fleet, decides what happens on the devices afterwards. On personally owned devices Google's behaviour is that the work profile goes too, and a fully managed company-owned device has no work profile to lose, so it stops being managed. This is the intended behaviour, and turning Android MDM back on does not undo it: that is a fresh binding, and it does not restore the deleted connection secrets or the installs that were marked failed.
...
The binding puts a piece of your Fleet deployment inside somebody else's administrative boundary. The Android enterprise belongs to whoever administers your Google organization, with the consequences described above.
