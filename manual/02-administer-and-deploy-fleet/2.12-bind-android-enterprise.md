...
![Explanation](../_assets/icons/explanation.svg) By default, Fleet reaches Google’s Android Management API through a proxy on fleetdm.com. Enterprise creation, policies, the Pub/Sub subscription, and per-device enrollment tokens all use that path.
...
![Explanation](../_assets/icons/explanation.svg) Android inventory and management primarily use Google’s event stream: `ENROLLMENT` reports a new device and `STATUS_REPORT` reports changes. Android has neither fleetd nor osquery; the small Fleet companion app below handles certificate delivery.
