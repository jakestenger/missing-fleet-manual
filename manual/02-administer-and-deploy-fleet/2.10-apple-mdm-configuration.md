...
![How-to](../_assets/icons/howto-light.svg) **Release from Apple Business**, on a host’s **Actions** menu, removes the device from your Apple Business organization. This Premium action is intended for devices leaving the organization.

Release has no undo action. Re-adding a device requires an eligible purchase-channel or manual Apple enrollment process. Fleet requires a host-specific confirmation before release. Use a fleet transfer or host deletion for changes that do not involve relinquishing the Apple Business assignment.

Enable **Allow this service to release devices** in Apple Business. Otherwise Apple rejects the request and Fleet reports that reason for the host.

If the action is missing, check these six conditions:
...
![Reference](../_assets/icons/reference-light.svg) Verified against Fleet 4.91.0. The annual push certificate cycle and the same-account requirement are Apple's constraints, so they outlast Fleet versions; the screens for managing them are Fleet's and move between releases.
