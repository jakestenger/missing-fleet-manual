...
**And a result-log destination is still not the whole story for reports.** A scheduled query's results are only forwarded when that query has automations enabled, which is off when a query is created, so a correctly configured `osquery_result_log_plugin` will sit empty until somebody turns automations on per query ([1.1](../01-foundations/1.1-what-fleet-is.md)).
