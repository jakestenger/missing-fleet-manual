...
[1.4 Identity and roles](../01-foundations/1.4-identity-and-roles.md) explains why the model is shaped this way. [2.5](2.5-identity-providers-sso-scim-and-role-sync.md) covers connecting an identity provider so accounts are created and removed for you. The comprehensive action-by-action breakdown is [a.4 Roles and permissions matrix](../09-appendices/a.4-roles-and-permissions-matrix.md). **This chapter is the model rather than the matrix**, and it gives you enough to choose correctly.
...
A GitOps identity's read access is deliberately narrow, but narrow is not none. It can read global configuration and global reports, precisely because applying them needs to know what is already there, along with a few specific secrets that [1.4](../01-foundations/1.4-identity-and-roles.md) enumerates. What it cannot do is enumerate your estate: a pipeline credential that leaks should not become a way to list every host. One distinction is easy to lose while checking current state with such a token. A denied read is a `403`, not an empty success; an empty result that came back with a success status means the request was authorized and had nothing to return, which is a different finding from a refusal. Read the status code before concluding the token is broken.
