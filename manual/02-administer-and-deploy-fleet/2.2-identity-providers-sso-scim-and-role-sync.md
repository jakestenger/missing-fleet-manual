...
<!-- IMAGE-REDO: assets/2.2-identity-provider-jobs.webp
     WHY: reviewed 2026-08-24. The vehicles are gone and the green SCIM arrow correctly
     marks the one job not tied to a sign-in. Two composition problems remain. **The arrows
     stop at the braces and never reach the Fleet server box**, so the flow appears to go
     nowhere; extend them to the box. And the frame is mostly empty: two tall blank end boxes,
     arrows confined to the middle third, and a large dead area beneath everything. Tighten it
     so the arrows span the full width and the whole frame carries content. The SCIM label is
     also set in green type; keep all text in the Black ramp and let the arrow carry the
     colour.
