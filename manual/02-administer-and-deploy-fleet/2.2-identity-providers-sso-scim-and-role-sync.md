...
2. **JIT provisioning** creates the account on first successful sign-in. Nobody files a ticket to get access; being in the right group in your identity provider is the access.
3. **Role sync** lets the identity provider decide what that account may do, carried as attributes in the sign-in response. Fleet re-evaluates those attributes on **every** login, not only the first, so a group change upstream becomes a role change in Fleet at the person's next sign-in.
4. **SCIM** closes the loop at the other end, removing the Fleet account when the person is deactivated upstream.

All four together give a joiner-mover-leaver pipeline in which Fleet holds no independent copy of who should have access. Turning them on one at a time keeps each change small enough to test and leaves you a way back into Fleet if one needs adjusting; the verification section at the end tests them in the same order.

**JIT provisioning requires Fleet Premium**, and so does SCIM-based device-owner mapping. SSO and SCIM deprovisioning do not.
...
- Every role and what it can do: [a.4 Roles and permissions matrix](../09-appendices/a.4-roles-and-permissions-matrix.md), **not written yet**. Until it is, [2.3](2.3-user-accounts-roles-and-service-identities.md) is the fullest account
