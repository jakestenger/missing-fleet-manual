...
| **Seventeen rows split by edition**, **eleven** of the first kind and **eight** of the second, with **`apply -f` and `gitops`** in both | Derived, **published as a floor**. The second count was corrected from seven in draft review 1; the first from ten in draft review 2, finding 10 | `a.7-scratch-contracts.md` §2.6.1. The floor is because the Type-1 stub pattern repeats at nine sites in community `teams.go`. **Arithmetic:** nine rows are first-kind only, two are both, six are second-kind only, so 9 + 2 + 6 = 17 distinct rows and the subtotals are 11 and 8, summing to 19. **The reviewer proposed 10 and 9 and that is wrong**: it needs seven second-kind-only rows and the table has six. **Both rows are in both kinds, but not for the reason draft review 1 gave.** `ApplyLabelSpecs` does authorize at `server/service/labels.go:692` before the licence check at `:699`, but that check is conditional on a non-nil, non-zero fleet: `apply` always passes nil (`server/service/client.go:605`), and a Free `gitops` never sends a fleet-scoped label write because the client skips non-global files first (`cmd/fleetctl/fleetctl/gitops.go:290-293`). What actually puts both rows in the second kind is **label-scoped policies** (`server/service/global_policies.go:457-459`, comment "Check authorization first", licence refusal `:493-494`) and **label-scoped configuration profiles** (`server/service/mdm.go:2428` authorizing via `:2852`, licence refusal `:2470-2471`). Conversely a **label-scoped report is first-kind**, not second: `server/service/queries.go` refuses the licence in its validation pass before the authorization pass. `get labels --fleet` and `get software --fleet` remain second-kind (`labels.go:986` then `:990-992`; `software_titles.go:75-79` then `:86-88`) |
