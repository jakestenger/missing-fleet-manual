...
**Four of them cannot be scoped to a fleet at all, and the calendar callback destination cannot be configured globally.** The Google Calendar credentials themselves *are* global; it is only the callback destination that is per fleet.
