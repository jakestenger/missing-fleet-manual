...
GitOps is the closest Fleet comes to declarative configuration. How close it comes decides whether your production practice is a safe one.
...
> **Before your first production run, three things.** Whenever the invocation includes your global file, include an explicit `unassigned.yml`. Check your deployment's effective exception settings rather than assuming them. Know that a dry run validates rather than rehearses: it is not passive, and it does not produce a plan of what would change.

## Understand Fleet's GitOps execution model

![Explanation](../_assets/icons/explanation.svg) A client orchestrating API calls, and the order they run in.
...
Almost nothing. The server sees an ordinary API client that sets a flag meaning "replace rather than merge", records which file a fleet came from, and can ask for validation without persistence. **That is the entire server-side surface.** Everything else that GitOps does happens in the client, on your build runner.
...
**Four repository invariants:**

1. **Every invocation containing the global file also contains an explicit `unassigned.yml`**, even when its desired state is empty.
...
![Reference](../_assets/icons/reference.svg) What GitOps replaces, what it defaults, and what survives omission.
...
> **Omitting a webhook block turns its enable flag off**, because the client defaults that flag. **The destination and the rest of its configuration survive**, inactive, waiting for someone to re-enable it. Your repository will not show that it exists.
...
**The command contract adds its own constraints.** Unknown keys fail by default, and the flag that permits them belongs nowhere near production. `unassigned.yml` is recognised by its basename. The flag that deletes absent fleets belongs only on the canonical complete invocation.
...
> **The secrets exception defaults on, so omitting `secrets:` on a stock Premium deployment preserves your enroll secrets, and *including* the key produces an error.** Turn the exception off and omission deletes them.
>
...
![How-to](../_assets/icons/howto.svg) There is an export command, and what it writes needs repair before it applies.
...
**So the migration takes five steps:**
...
`-f` is required and repeatable: one flag per file, not a comma-separated list. `gitops` rejects any positional argument outright, so a filename typed without its own `-f` fails the command instead of being silently skipped. Point that same set of `-f` flags at a non-production Fleet with `--context`, and run it for real by dropping `--dry-run`:
...
`--context` is not optional here even though it is easy to leave off: `fleetctl` falls back to the `default` context whenever it is omitted, and for anyone who set up `fleetctl` normally that `default` context points at production. Name the non-production context explicitly, every time.

That completes step 4. It is also the canonical complete invocation: every fleet file present, `unassigned.yml` explicit, and, only once you trust it enough to make it authoritative, `--delete-other-fleets` added to remove anything not listed. Leave that flag off everywhere else, including every staged invocation below.

Adopt by boundary rather than all at once, and **use a different invocation to do it.**

A staged migration is not the canonical production workflow and must not borrow its flags. Deleting absent fleets has to be off, because a stage supplies a deliberate subset. Any stage containing the global file still has to carry `unassigned.yml`, for the same reason the empty-Unassigned trap gives. Keep the two invocations separate in your pipeline so nobody can run the destructive one with a partial manifest.

Each boundary you move is also one where you must decide who is now forbidden to use the interface, and that decision has to be enforced socially.
...
![How-to](../_assets/icons/howto.svg) Two mechanisms, and where each one substitutes its value.
...
>
> **Do not enable debug logging in a GitOps pipeline, control the generic `DEBUG` variable in your build environment, and treat any log produced with it as a secret exposure.** Authorization headers and binary bodies are not printed, which limits the exposure without changing the response.

## Validate, review, and apply

![How-to](../_assets/icons/howto.svg) The promotion ladder, and what each rung proves.
...
It is also not passive. It transmits referenced secret values to Fleet, which validates and discards them without storing. It can reach the network to download software or fetch an app manifest when it has no usable cached copy, and it writes transient server state along the way.
...
4. **Apply for real in a non-production environment.** No earlier rung proves the apply will succeed.
5. **Serialise production applies.** One at a time, enforced by your pipeline, because Fleet will not do it for you.
6. **Do not cancel a run in progress.** Cancelling mid-sequence leaves the same partial state a failure does.
...
One failure mode is silent and breaks device enrollment.

**In a Premium run supplying more than one file, where your global configuration assigns an Apple Business Manager or App Store token to a fleet that does not exist yet**, the client cannot assign a token to a fleet it has not created. So it clears those assignment lists first, creates the fleets, and restores the assignments at the end.
...
Then there is the drift GitOps cannot see: an API writer changing a field between runs, an interface action that GitOps mode still permits, or a setting in a block GitOps does not own. **The next apply governs what GitOps sends. It has no opinion about anything else.**
