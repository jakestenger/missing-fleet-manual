...
![Explanation](../_assets/icons/explanation-light.svg) Fleet’s Android Enterprise connection gives administrators a way to manage work profiles and supported company-owned devices alongside the rest of their inventory. Google handles the device-management channel, while Fleet provides the place to configure and review it. Choose a stable, publicly reachable Fleet URL before connecting; Google’s event subscription will continue using that address.

## Purpose and scope

![Explanation](../_assets/icons/explanation-light.svg) This chapter connects Fleet to an Android Enterprise and explains the responsibilities that come with the binding. You will prepare the organization account, verify the connection, and understand what disconnecting it does to devices. Continue to [3.6](../03-connect-devices/3.6-enroll-android-devices.md) for enrollment or [2.9](2.9-mdm-architecture-and-foundations.md) for the shared MDM model.
...
![How-to](../_assets/icons/howto-light.svg) Bind Fleet to an Android Enterprise through Google’s signup flow.
...
![Explanation](../_assets/icons/explanation-light.svg) By default, Fleet reaches Google’s Android Management API through a proxy on fleetdm.com. Enterprise creation, policies, the Pub/Sub subscription, and per-device enrollment tokens all use that path.
...
A network failure during binding can leave an enterprise created at the proxy before Fleet receives its secret key. If Google shows a connection that Fleet does not, investigate that incomplete exchange before retrying setup.
...
![Explanation](../_assets/icons/explanation-light.svg) Android inventory and management primarily use Google’s event stream: `ENROLLMENT` reports a new device and `STATUS_REPORT` reports changes. Android has neither fleetd nor osquery; the small Fleet companion app below handles certificate delivery.
...
Account for these identifiers when reconciling hardware assets. Android work-profile identifiers and account-driven Apple enrollment identifiers represent the managed relationship rather than a reusable hardware serial number.
...
![How-to](../_assets/icons/howto-light.svg) Turning Android MDM off deletes the Android Enterprise and ends management for its devices.
...
<!-- SCREENSHOT: SS-2.12-01 | A completed Android Enterprise connection
...
![How-to](../_assets/icons/howto-light.svg) For Android, the settings page is the confirmation. **Settings > Integrations > MDM** shows Android MDM on once the binding is complete, and on the Google side the free Android Enterprise subscription appears in the Workspace subscriptions list.

Record who administers the Google organization and can delete its Android Enterprise.

## Edge cases and precedence

![Reference](../_assets/icons/reference-light.svg) The behaviours below come from the boundary between Fleet and the platform vendor.
...
![Troubleshooting](../_assets/icons/troubleshooting-light.svg) Android diagnostics are in [8.10](../08-troubleshooting/8.10-android-diagnostics.md), and they are largely a matter of interpreting what Google has and has not pushed.
...
![Reference](../_assets/icons/reference-light.svg) Verified against Fleet 4.90.0. Google's signup flow is vendor-owned and changes on Google's schedule, not Fleet's.
