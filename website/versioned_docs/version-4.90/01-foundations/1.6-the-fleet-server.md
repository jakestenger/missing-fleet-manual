...
![Explanation](../_assets/icons/explanation-light.svg) The Fleet server gives administrators and automated workflows a shared point of coordination. It keeps the configuration you want to apply, receives information from devices, and arranges the work that connects the two. Understanding the services behind it helps you plan capacity, recovery, and the responsibilities of the people who operate Fleet.

Most state lives outside the Fleet process, in its database, Redis, and file stores. This lets you replace or scale Fleet instances while keeping their shared state available.
...
<a id="where-the-state-actually-lives"></a>

## Where Fleet stores state

![Explanation](../_assets/icons/explanation-light.svg) Fleet uses three stores for different workloads. Knowing what each holds helps you choose the right backup and availability arrangements.

**MySQL holds durable records:** hosts, fleets, labels, policies, reports, MDM command queues, activity history, and the configuration you set. Backing up MySQL preserves that central record of the deployment.
...
Each Fleet process caches frequently read values from MySQL. These include application configuration, agent options and features, packs and scheduled queries, fleet and default-fleet MDM configuration, query lookups and result counts, YARA rules, MDM assets, and Fleet-maintained app names.

Entries expire at different intervals, from about a second for configuration to fifteen minutes for the longest-lived entries. Live queries have a separate in-memory cache. These local copies explain why two otherwise healthy Fleet instances can briefly answer with different versions of a recently changed value.
...
The three database-backed fallback stores work across multiple Fleet instances because each instance reaches the same database. Their contents add to database size, with file-carve blocks making the largest contribution.
...
![Explanation](../_assets/icons/explanation-light.svg) Each store serves a different workload.
...
![Reference](../_assets/icons/reference-light.svg) Plan capacity, failover, and shared storage around these constraints.
...
Plan to increase the database writer’s capacity as write demand grows. Additional Fleet instances provide more API and check-in capacity, but they share that writer. Fleet’s reference sizes reflect this relationship by increasing the database instance class as host counts grow.
...
<a id="read-replicas-help-some-deployments-and-not-others"></a>
...
Read replicas are most useful when API polling, automation, and reporting generate substantial read traffic. Host check-ins primarily write data, so a deployment dominated by check-ins benefits less from a read replica.
...
In cluster mode, multi-key operations must use one slot. Operations that cannot target a single node visit every node, and subscriber counts must be added across nodes. Fleet handles that coordination. Its reference architectures still specify standalone Redis for deployments well into six figures of hosts, so size the working set and throughput before choosing a cluster.
...
![Troubleshooting](../_assets/icons/troubleshooting-light.svg) Check the stores separately when diagnosing these symptoms.
...
- **Live reports stop.** Fleet cannot create campaigns or receive their results, and a report being run in the interface fails.
...
- **Error recording stops.** Recorded server errors live in Redis with a TTL. An outage prevents new records, while existing entries can expire; discarding Redis data removes them. Use the server’s own logs to investigate the period Redis could not record.
...
- **Asynchronous host processing can leave observations stale.** When enabled, last-seen, label, policy, and scheduled-query-stat updates pass through Redis. Queue-write failures are logged while the check-in continues, so a successful check-in can coexist with host data that has stopped advancing ([8.14](../08-troubleshooting/8.14-degradation.md)).
...
![Explanation](../_assets/icons/explanation-light.svg) Fleet sends desired state to devices and receives observations from them.
...
![How-to](../_assets/icons/howto-light.svg) When designing or operating a Fleet deployment, establish ownership for:
...
Plan the relationship between the three backups. Fleet provides no supported procedure that captures all stores together, and independent snapshots can represent different points in time. Your backup process needs to arrange consistency where the restored data depends on it.
